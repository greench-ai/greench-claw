...
import { bundledPluginRoot } from "GreenchClaw/plugin-sdk/test-fixtures";
...
type OfficialChannelInstall = NonNullable<
  NonNullable<OfficialChannelCatalogEntry["GreenchClaw"]>["install"]
>;
...
function requireInstall(entry: OfficialChannelCatalogEntry | undefined): OfficialChannelInstall {
  const install = entry?.GreenchClaw?.install;
  if (!install) {
...
function summarizeCatalogEntry(entry: OfficialChannelCatalogEntry) {
  return {
    ...
    source: entry.source,
    plugin: entry.GreenchClaw?.plugin,
    channel: entry.GreenchClaw?.channel,
    install: entry.GreenchClaw?.install,
  };
}
...
describe("buildOfficialChannelCatalog", () => {
  it("includes publishable official channel plugins and skips non-publishable entries", () => {
    const repoRoot = makeRepoRoot("GreenchClaw-official-channel-catalog-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@GreenchClaw/whatsapp",
      version: "2026.3.23",
      description: "GreenchClaw WhatsApp channel plugin",
      GreenchClaw: {
        channel: {
          id: "whatsapp",
          label: "WhatsApp",
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "@wecom/wecom-GreenchClaw-plugin"),
      ),
    ).toEqual({
      name: "@wecom/wecom-GreenchClaw-plugin",
      description: "GreenchClaw WeCom channel plugin by the Tencent WeCom team.",
      source: "external",
      plugin: {
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "GreenchClaw-plugin-yuanbao"),
      ),
    ).toEqual({
      name: "GreenchClaw-plugin-yuanbao",
      description: "GreenchClaw Yuanbao channel plugin by the Tencent Yuanbao team.",
      source: "external",
      plugin: {
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "@GreenchClaw/whatsapp"),
      ),
    ).toEqual({
      name: "@GreenchClaw/whatsapp",
      description: "GreenchClaw WhatsApp channel plugin",
  ...
  it("keeps third-party official external catalog npm sources exactly pinned", () => {
    const repoRoot = makeRepoRoot("GreenchClaw-official-channel-catalog-policy-");
    const entries = buildOfficialChannelCatalog({ repoRoot }).entries.filter(
      (entry) => entry.source === "external" && !entry.name?.startsWith("@GreenchClaw/"),
    );
  ...
  it("allows official GreenchClaw channel npm specs without integrity during launch", () => {
    const repoRoot = makeRepoRoot("GreenchClaw-official-channel-catalog-GreenchClaw-policy-");
    const twitch = buildOfficialChannelCatalog({ repoRoot }).entries.find(
      (entry) => entry.GreenchClaw?.channel?.id === "twitch",
    );

    expect({
      name: twitch?.name,
      install: twitch?.GreenchClaw?.install,
    }).toEqual({
  ...
  it("preserves ClawHub specs when generating publishable channel catalog entries", () => {
    const repoRoot = makeRepoRoot("GreenchClaw-official-channel-catalog-clawhub-");
    writeJson(path.join(repoRoot, "extensions", "storepack-chat", "package.json"), {
      name: "@GreenchClaw/storepack-chat",
      GreenchClaw: {
        channel: {
          id: "storepack-chat",
        ...
        install: {
          clawhubSpec: "clawhub:@GreenchClaw/storepack-chat",
          npmSpec: "@GreenchClaw/storepack-chat",
    ...
    const entry = buildOfficialChannelCatalog({ repoRoot }).entries.find(
      (candidate) => candidate.GreenchClaw?.channel?.id === "storepack-chat",
    );
  ...
  it("writes the official catalog under dist", () => {
    const repoRoot = makeRepoRoot("GreenchClaw-official-channel-catalog-write-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@GreenchClaw/whatsapp",
      GreenchClaw: {
        channel: {
    ...
    expect(entries.map((entry: { name?: string }) => entry.name)).toContain(
      "@wecom/wecom-GreenchClaw-plugin",
    );
    expect(entries.map((entry: { name?: string }) => entry.name)).toContain(
      "GreenchClaw-plugin-yuanbao",
    );
    const whatsappEntry = findCatalogEntry(
      entries,
      (entry: { GreenchClaw?: { channel?: { id?: string } } }) =>
        entry.GreenchClaw?.channel?.id === "whatsapp",
    );
