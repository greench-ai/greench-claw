...
export function loadVitestExperimentalConfig(
  ...
  if (!windowsEnv && !isDisabled(env.GREENCHCLAW_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (windowsEnv && isEnabled(env.GREENCHCLAW_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (experimental.fsModuleCache && env.GREENCHCLAW_VITEST_FS_MODULE_CACHE_PATH?.trim()) {
    experimental.fsModuleCachePath = env.GREENCHCLAW_VITEST_FS_MODULE_CACHE_PATH.trim();
  }
  if (isEnabled(env.GREENCHCLAW_VITEST_IMPORT_DURATIONS)) {
    experimental.importDurations = { print: true };
  }
  if (isEnabled(env.GREENCHCLAW_VITEST_PRINT_IMPORT_BREAKDOWN)) {
