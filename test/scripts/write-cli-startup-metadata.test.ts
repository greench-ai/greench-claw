...
describe("write-cli-startup-metadata", () => {
  ...
  it("writes startup metadata with populated root help text when dist falls back to source rendering", async () => {
    const tempRoot = createTempDir("GreenchClaw-startup-metadata-");
    ...
    await writeCliStartupMetadata({
      ...
      renderSourceRootHelpText: () => "Usage: GreenchClaw\n",
      renderSourceBrowserHelpText: () => "Usage: GreenchClaw browser\n",
    });
    ...
    expect(written.browserHelpText).toContain("GreenchClaw browser");
    expect(written.rootHelpText).toContain("Usage:");
    expect(written.rootHelpText).toContain("GreenchClaw");
  });
});
