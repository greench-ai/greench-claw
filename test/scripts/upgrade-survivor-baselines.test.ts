...
function withReleaseFixture<T>(releases: unknown[], fn: (file: string) => T): T {
  const dir = mkdtempSync(path.join(tmpdir(), "GreenchClaw-upgrade-baselines-"));
  try {
  ...
  it("resolves release-history to last six stable releases plus explicit legacy anchors", () => {
    ...
    withReleaseFixture(releases, (file) => {
      expect(
        ...
        "GreenchClaw@2026.4.26",
        "GreenchClaw@2026.4.25",
        "GreenchClaw@2026.4.24",
        "GreenchClaw@2026.4.22",
        "GreenchClaw@2026.4.23",
      ...
      withJsonFixture(
        ...
        (versionsFile) => {
          expect(
            ...
            "GreenchClaw@2026.5.3-1",
            "GreenchClaw@2026.5.3",
            "GreenchClaw@2026.5.2",
            "GreenchClaw@2026.4.29",
            "GreenchClaw@2026.4.23",
            "GreenchClaw@2026.4.15",
          ]);
        },
      );
    });
