...
const CHECK_SCRIPT = "scripts/check-GreenchClaw-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "GreenchClaw-package-tarball-test-"));
  try {
    ...
    const tarFiles =
      options.includeControlUi === false
        ? files
        : {
            "dist/control-ui/index.html": "<!doctype html><GreenchClaw-app></GreenchClaw-app>",
    ...
    const tarball = join(root, "GreenchClaw.tgz");
...
describe("check-GreenchClaw-package-tarball", () => {
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("GreenchClaw package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
