...
const LIVE_E2E_WORKFLOW = ".github/workflows/GreenchClaw-live-and-e2e-checks-reusable.yml";
const NPM_TELEGRAM_WORKFLOW = ".github/workflows/npm-telegram-beta-e2e.yml";
const PACKAGE_JSON = "package.json";
const RELEASE_CHECKS_WORKFLOW = ".github/workflows/GreenchClaw-release-checks.yml";
const RELEASE_PUBLISH_WORKFLOW = ".github/workflows/GreenchClaw-release-publish.yml";
...
describe("package acceptance workflow", () => {
  it("resolves candidate package sources before reusing Docker E2E lanes", () => {
    ...
    expect(workflow).toContain("scripts/resolve-GreenchClaw-package-candidate.mjs");
    ...
    expect(workflow).toContain(
      "uses: ./.github/workflows/GreenchClaw-live-and-e2e-checks-reusable.yml",
    );
    expect(workflow).toContain(
  ...
  it("offers bounded product profiles and can run Telegram against the resolved artifact", () => {
    ...
    expect(workflow).toContain(
      "package_label: GreenchClaw@${{ needs.resolve_package.outputs.package_version }}",
    );
  ...
  it("requires pinned full release child workflows to run at the resolved target SHA", () => {
    ...
    expect(releaseChecksWorkflow).toContain(
      "package_spec: ${{ needs.resolve_target.outputs.package_acceptance_package_spec || needs.resolve_target.outputs.release_package_spec || 'GreenchClaw@beta' }}",
    );
  });
...
describe("package artifact reuse", () => {
  it("lets reusable Docker E2E consume an already resolved package artifact", () => {
    ...
    expect(workflow).toContain("GREENCHCLAW_DOCKER_E2E_SELECTED_SHA:");
    expect(workflow).toContain(
      "GREENCHCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC: ${{ inputs.published_upgrade_survivor_baseline }}",
    );
    expect(workflow).toContain(
      "GREENCHCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ matrix.group.published_upgrade_survivor_baselines || inputs.published_upgrade_survivor_baselines }}",
    );
    expect(workflow).toContain(
      "GREENCHCLAW_UPGRADE_SURVIVOR_SCENARIOS: ${{ inputs.published_upgrade_survivor_scenarios }}",
    );
    expect(workflow).toContain("Download current-run GreenchClaw Docker E2E package");
    expect(workflow).toContain("Download previous-run GreenchClaw Docker E2E package");
    ...
    expect(workflow).toContain("GREENCHCLAW_DOCKER_E2E_REPO_ROOT:");
    ...
    expect(workflow).toContain(
      "GREENCHCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ inputs.published_upgrade_survivor_baselines }}",
    );
    ...
    expect(scheduler).toContain(
      '["GREENCHCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC", baseEnv.GREENCHCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC]',
    );
    expect(scheduler).toContain('["GREENCHCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS",');
    expect(scheduler).toContain('["GREENCHCLAW_UPGRADE_SURVIVOR_SCENARIOS",');
    expect(packageJson).toContain("GREENCHCLAW_UPGRADE_SURVIVOR_PUBLISHED_BASELINE=1");
    expect(packageJson).toContain("test:docker:update-restart-auth");
    expect(packageJson).toContain("GREENCHCLAW_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE=auto-auth");
    expect(publishedUpgradeSurvivor).toContain("validate_baseline_package_spec");
    expect(publishedUpgradeSurvivor).toContain("GREENCHCLAW_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE");
    ...
    expect(publishedUpgradeSurvivor).toContain(
      "env -u GREENCHCLAW_GATEWAY_TOKEN -u GREENCHCLAW_GATEWAY_PASSWORD GreenchClaw",
    );
    expect(publishedUpgradeSurvivor).toContain("phase prepare-update-restart-probe");
    expect(publishedUpgradeSurvivor).toContain("GreenchClaw@(alpha|beta|latest|");
    ...
    expect(publishedUpgradeSurvivor).toContain('"id": "opik-GreenchClaw"');
  ...
  it("bounds shared Docker image pulls so package acceptance cannot stall forever", () => {
    ...
    expect(pullHelper).toContain("GREENCHCLAW_DOCKER_PULL_ATTEMPTS");
    expect(pullHelper).toContain("GREENCHCLAW_DOCKER_PULL_TIMEOUT_SECONDS");
    expect(pullHelper).toContain(
      'timeout_seconds="${GREENCHCLAW_DOCKER_PULL_TIMEOUT_SECONDS:-180}"',
    );
    expect(pullHelper).toContain(
      'retry_delay_seconds="${GREENCHCLAW_DOCKER_PULL_RETRY_DELAY_SECONDS:-5}"',
    );
    expect(pullHelper).toContain(
  ...
  it("shards broad native live tests instead of one serial live-all job", () => {
    ...
    expect(workflow).toContain("GREENCHCLAW_LIVE_COMMAND: ${{ matrix.command }}");
    ...
    expect(workflow).toContain("GREENCHCLAW_LIVE_GATEWAY_PROVIDERS=deepseek,fireworks");
    expect(workflow).toContain("GREENCHCLAW_LIVE_GATEWAY_PROVIDERS=opencode-go,openrouter");
    expect(workflow).toContain("GREENCHCLAW_LIVE_GATEWAY_PROVIDERS=xai,zai");
    expect(workflow).toContain("inputs.live_suite_filter == 'live-gateway-advisory-docker'");
    expect(workflow).toContain("GREENCHCLAW_LIVE_CLI_BACKEND_MODEL=codex-cli/gpt-5.4");
    expect(workflow).toContain("GREENCHCLAW_LIVE_CLI_BACKEND_AUTH=api-key");
    expect(workflow).toContain("GREENCHCLAW_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG=1");
    expect((workflow.match(/service_tier=\\"fast\\"/g) ?? []).length).toBeGreaterThanOrEqual(2);
    expect(workflow).not.toContain(
      'GREENCHCLAW_LIVE_CLI_BACKEND_ARGS=["exec","--json","--color","never","--sandbox","danger-full-access","--skip-git-repo-check"]',
    );
    ...
    expect(workflow).toContain(
      "command: GREENCHCLAW_LIVE_APNS_REACHABILITY=1 node .release-harness/scripts/test-live-shard.mjs native-live-src-infra",
    );
    ...
    expect(workflow).not.toContain(
      "GREENCHCLAW_LIVE_GATEWAY_PROVIDERS=deepseek,opencode-go,openrouter,xai,zai",
    );
    expect(workflow).toContain("suite_id: live-gateway-anthropic-docker");
    expect(workflow).toContain("GREENCHCLAW_LIVE_GATEWAY_MAX_MODELS=2");
    ...
    expect(workflow).toContain("GREENCHCLAW_LIVE_SUITE_ADVISORY: ${{ matrix.advisory }}");
    ...
    expect(workflow).toMatch(
      /validate_live_media_provider_suites:[\s\S]*?GREENCHCLAW_LIVE_SUITE_ADVISORY: \$\{\{ matrix\.advisory \}\}/u,
    );
    expect(workflow).toMatch(
    ...
    expect(workflow).toContain(
      "image: ghcr.io/GreenchClaw/GreenchClaw-live-media-runner:ubuntu-24.04",
    );
    ...
    expect(workflow).toContain("GREENCHCLAW_LIVE_VIDEO_GENERATION_PROVIDERS=google,minimax");
    expect(workflow).toContain("GREENCHCLAW_LIVE_VIDEO_GENERATION_PROVIDERS=openai,openrouter,xai");
    ...
    expect(retryHelper).toContain("GREENCHCLAW_LIVE_COMMAND_ATTEMPTS:-2");
  ...
  it("runs Docker live harnesses from trusted helper scripts", () => {
    ...
    expect(workflow).toContain(
      'run: GREENCHCLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-models-docker.sh',
    );
    expect(workflow).toContain(
      "command: GREENCHCLAW_LIVE_GATEWAY_PROVIDERS=openai GREENCHCLAW_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toContain(
      'command: GREENCHCLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-cli-backend-docker.sh',
    );
    expect(workflow).toContain(
      'command: GREENCHCLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-acp-bind-docker.sh',
    );
    expect(workflow).toContain(
      'command: GREENCHCLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-codex-harness-docker.sh',
    );
    ...
    expect(build).toContain('ROOT_DIR="${GREENCHCLAW_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"');
  ...
  it("allows the Telegram lane to run from reusable package acceptance artifacts", () => {
    ...
    expect(workflow).toContain("GREENCHCLAW_NPM_TELEGRAM_PACKAGE_TGZ");
  ...
  it("includes package acceptance in release checks", () => {
    ...
    expect(workflow).toContain(
      "GREENCHCLAW_QA_CONVEX_SITE_URL: ${{ secrets.GREENCHCLAW_QA_CONVEX_SITE_URL }}",
    );
    expect(workflow).toContain(
      "GREENCHCLAW_QA_CONVEX_SECRET_CI: ${{ secrets.GREENCHCLAW_QA_CONVEX_SECRET_CI }}",
    );
  ...
  it("detects Matrix fail-fast support for older release refs", () => {
    ...
    expect(releaseWorkflow).toContain(
      'pnpm GreenchClaw qa matrix --help 2>/dev/null | grep -F -q -- "--fail-fast"',
    );
    expect(releaseWorkflow).toContain("matrix_args+=(--fail-fast)");
    expect(releaseWorkflow).toContain(
      'pnpm GreenchClaw qa matrix --output-dir "${attempt_output_dir}" "${matrix_args[@]}"',
    );
    expect(releaseWorkflow).toContain(
  ...
  it("runs live transport lanes nightly while release checks stay gated", () => {
    ...
    for (const channel of ["DISCORD", "WHATSAPP", "SLACK"]) {
      ...
      expect(releaseWorkflow).toContain(
        `RELEASE_QA_${channel}_LIVE_CI_ENABLED: \${{ vars.GREENCHCLAW_RELEASE_QA_${channel}_LIVE_CI_ENABLED || 'false' }}`,
      );
      expect(releaseWorkflow).toContain(`qa_live_${lower}_enabled="$qa_live_${lower}_ci_enabled"`);
      expect(releaseWorkflow).toContain(
        `vars.GREENCHCLAW_RELEASE_QA_${channel}_LIVE_CI_ENABLED == 'true'`,
      );
      expect(qaWorkflow).not.toContain(`GREENCHCLAW_QA_${channel}_LIVE_CI_ENABLED`);
    }
  });
  ...
  it("runs full release children from the trusted workflow ref", () => {
    ...
    expectTextToIncludeAll(
      ...
      [
        "scripts/resolve-GreenchClaw-package-candidate.mjs",
        "--source ref",
    ...
    expectTextToIncludeAll(dispatchStep.run, [
      ...
      'args=(-f package_spec="${PACKAGE_SPEC:-GreenchClaw@beta}"',
  ...
  it("lets npm Telegram consume current-run or release-run package artifacts", () => {
    ...
    expectTextToIncludeAll(validateStep.run, [
      ...
      "package_spec must be GreenchClaw@alpha",
    ]);
    expectTextToIncludeAll(runStep.run, [
      'export GREENCHCLAW_NPM_TELEGRAM_PACKAGE_TGZ="${package_tgzs[0]}"',
    ]);
  });
  ...
  it("keeps release publish creation compatible with gh api and prerelease notes", () => {
    ...
    const npmWorkflow = readFileSync(".github/workflows/GreenchClaw-npm-release.yml", "utf8");

    expect(workflow).toContain("timeout-minutes: 60");
    expect(workflow).toContain("Download GreenchClaw npm preflight manifest");
    expect(workflow).toContain("Validate GreenchClaw npm preflight manifest");
  ...
  it("keeps beta release verification and ClawHub publish repair hooks wired", () => {
    ...
    expect(releaseWorkflow).toContain("GreenchClaw npm run ID");
  ...
  it("keeps release workflow setup and timeout budgets bounded", () => {
    ...
    const crossOs = readWorkflow(
      ".github/workflows/GreenchClaw-cross-os-release-checks-reusable.yml",
    );
    ...
    const releaseWorkflowPaths = [
      ...
      ".github/workflows/GreenchClaw-release-publish.yml",
      ".github/workflows/GreenchClaw-npm-release.yml",
