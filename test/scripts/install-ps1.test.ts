...
describe("install.ps1 failure handling", () => {
  ...
  it("keeps failure termination in the top-level completion handler", () => {
    ...
    expect(completeInstallBody).toMatch(/\bthrow "GreenchClaw installation failed with exit code/);
  });

  it("runs npm install through the resolved command with quiet CI defaults", () => {
    const npmInstallBody = extractFunctionBody(source, "Install-GreenchClaw");
  ...
  it("cleans legacy git submodules only from the selected git checkout", () => {
    const gitInstallBody = extractFunctionBody(source, "Install-GreenchClawFromGit");
    const mainBody = extractFunctionBody(source, "Main");
  ...
  runIfPowerShell("exits non-zero when run as a script file", () => {
    const tempDir = harness.createTempDir("GreenchClaw-install-ps1-");
  ...
  runIfPowerShell("throws without killing the caller when run as a scriptblock", () => {
    ...
    expect(result.stdout).toContain("caught=GreenchClaw installation failed with exit code 1.");
  ...
  runIfPowerShell("keeps npm chatter out of Main's success return value", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "function Check-ExistingGreenchClaw { return $false }",
        "function Add-ToPath { param([string]$Path) }",
        "function Install-GreenchClaw { Write-Output 'npm stdout'; return $true }",
        "function Ensure-GreenchClawOnPath { return $true }",
        "function Refresh-GatewayServiceIfLoaded { }",
        "function Invoke-GreenchClawCommand { return 'GreenchClaw test-version' }",
        "$NoOnboard = $true",
        "$result = Main",
