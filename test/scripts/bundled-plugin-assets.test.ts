...
async function withPluginAssetFixture(run: (rootDir: string) => Promise<void>) {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-plugin-assets-"));
  try {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@GreenchClaw/canvas-plugin",
          GreenchClaw: {
            assetScripts: {
    ...
    fs.writeFileSync(
      path.join(rootDir, "extensions", "canvas", "GreenchClaw.plugin.json"),
  ...
  it("discovers plugin-owned asset scripts by manifest id", async () => {
    await withPluginAssetFixture(async (rootDir) => {
      ...
      expect(hooks).toEqual([
        {
          aliases: ["@GreenchClaw/canvas-plugin", "canvas", "canvas-plugin"],
          command: "node scripts/bundle-a2ui.mjs",
          packageName: "@GreenchClaw/canvas-plugin",
          phase: "build",
