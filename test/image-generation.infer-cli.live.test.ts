...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.GREENCHCLAW_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through GreenchClaw infer", () => {
    const outputBase = path.join(os.tmpdir(), `GreenchClaw-infer-image-${process.pid}.png`);
