...
import {
  ...
  type GreenchClawTestStateOptions,
} from "../../src/test-utils/GreenchClaw-test-state.js";
import { sleep } from "../../src/utils.js";

export type GreenchClawTestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<GreenchClawTestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

export type GreenchClawTestInstanceCommandResult = {
  code: number | null;
...
export type GreenchClawTestInstance = {
  name: string;
  port: number;
  url: string;
  hookToken: string;
  ...
  ) => Promise<GreenchClawTestInstanceCommandResult>;
...
export async function createGreenchClawTestInstance(
  options: GreenchClawTestInstanceOptions,
): Promise<GreenchClawTestInstance> {
  ...
  const instance: GreenchClawTestInstance = {
    name: options.name,
    port,
