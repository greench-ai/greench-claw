...
GreenchClaw features that can generate provider usage or paid API calls.
...
- Gemini CLI note: when the CLI returns JSON output, GreenchClaw reads usage from
  `stats`, normalizes `stats.cached` into `cacheRead`, and derives input tokens
  from `stats.input_tokens - stats.cached` when needed.

Anthropic note: Anthropic staff told us GreenchClaw-style Claude CLI usage is
...
Anthropic still does not expose a per-message dollar estimate that GreenchClaw can
show in `/usage full`.

**CLI usage windows (provider quotas)**

- `GreenchClaw status --usage` and `GreenchClaw channels list` show provider **usage windows**
...
- MiniMax note: its raw `usage_percent` / `usagePercent` fields mean remaining
  quota, so GreenchClaw inverts them before display. Count-based fields still win
  when present. If the provider returns `model_remains`, GreenchClaw prefers the
...
- Usage auth for those quota windows comes from provider-specific hooks when
  available; otherwise GreenchClaw falls back to matching OAuth/API-key
...
GreenchClaw can pick up credentials from:
...
GreenchClaw's local UI, such as **OpenAI Codex**, **Alibaba Cloud Model Studio
Coding Plan**, **MiniMax Coding Plan**, **Z.AI / GLM Coding Plan**, and
Anthropic's GreenchClaw Claude-login path with **Extra Usage** enabled.
...
- `GreenchClaw models status --json`
...
`GreenchClaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
