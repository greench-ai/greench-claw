---
summary: "How GreenchClaw builds prompt context and reports token usage + costs"
read_when:
...
GreenchClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

GreenchClaw assembles its own system prompt on every run. It includes:
...
For images, GreenchClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from GreenchClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `GreenchClaw status --usage` and `GreenchClaw channels list` show
...
available; otherwise GreenchClaw falls back to matching OAuth/API-key credentials
...
GreenchClaw keeps provider usage accounting separate from the current context
...
`cacheWrite`. If pricing is missing, GreenchClaw shows tokens only. OAuth tokens
never show dollar cost.

After sidecars and channels reach the Gateway ready path, GreenchClaw starts an
...
Provider prompt caching only applies within the cache TTL window. GreenchClaw can
...
Anthropic's 1M context window is currently beta-gated. GreenchClaw can inject the
...
GreenchClaw skips the `context-1m-*` beta header because Anthropic currently
