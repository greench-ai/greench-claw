---
summary: "Reference design for the public GreenchClaw App SDK API, event taxonomy, artifacts, approvals, and package structure"
title: "GreenchClaw App SDK API design"
sidebarTitle: "App SDK API design"
read_when:
  - You are implementing the proposed public GreenchClaw app SDK
  - You need the draft namespace, event, result, artifact, approval, or security contract for the app SDK
  - You are comparing Gateway protocol resources with the high-level GreenchClaw App SDK wrapper
---

This page is the detailed API reference design for the public
[GreenchClaw App SDK](/concepts/GreenchClaw-sdk). It is intentionally separate from
the [Plugin SDK](/plugins/sdk-overview).

<Note>
  `@GreenchClaw/sdk` is the external app/client package for talking to the
  Gateway. `GreenchClaw/plugin-sdk/*` is the in-process plugin authoring contract.
...
2. A high-level ergonomic wrapper with `GreenchClaw`, `Agent`, `Session`, `Run`,
...
- source: GreenchClaw, MCP, plugin, channel, runtime, or app
- schema summary
- approval policy
...
| Package                    | Purpose                                                       |
| -------------------------- | ------------------------------------------------------------- |
| `@GreenchClaw/sdk`         | Public high-level SDK and generated low-level Gateway client. |
| `@GreenchClaw/sdk-react`   | Optional React hooks for dashboards and app builders.         |
| `@GreenchClaw/sdk-testing` | Test helpers and fake Gateway server for app integrations.    |

The repo already has `GreenchClaw/plugin-sdk/*` for plugins. Keep that namespace
...
4. High-level `GreenchClaw`, `Agent`, `Session`, `Run`, `Task`, and `Artifact`
   wrappers.
...
- [GreenchClaw App SDK](/concepts/GreenchClaw-sdk)
