...
read_when:
  - You want to use Volcano Engine or Doubao models with GreenchClaw
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    GreenchClaw onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    GreenchClaw models list --provider volcengine
    GreenchClaw models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
Then enable it in `GreenchClaw.json`:
...
For voice-note targets, GreenchClaw asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `GreenchClaw onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, GreenchClaw falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.GreenchClaw/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running GreenchClaw as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about GreenchClaw setup.
  </Card>
</CardGroup>
