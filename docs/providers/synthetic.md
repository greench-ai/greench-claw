---
summary: "Use Synthetic's Anthropic-compatible API in GreenchClaw"
read_when:
...
GreenchClaw registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    GreenchClaw onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
GreenchClaw's Anthropic client appends `/v1` to the base URL automatically, so use
...
`GreenchClaw models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that GreenchClaw appends `/v1` automatically.
