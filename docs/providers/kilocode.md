---
summary: "Use Kilo Gateway's unified API to access many models in GreenchClaw"
title: "Kilo Gateway"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in GreenchClaw
---
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    GreenchClaw onboard --auth-choice kilocode-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    GreenchClaw models list --provider kilocode
    ```
  </Step>
</Steps>
...
<Note>
GreenchClaw treats `kilocode/kilo/auto` as the stable default ref, but does not
...
hard-coded in GreenchClaw.
</Note>

## Built-in catalog

GreenchClaw dynamically discovers available models from the Kilo Gateway at startup. Use
...
<Tip>
At startup, GreenchClaw queries `GET https://api.kilo.ai/api/gateway/models` and merges
...
<AccordionGroup>
  <Accordion title="Transport and compatibility">
    ...
    - Gemini-backed Kilo refs stay on the proxy-Gemini path, so GreenchClaw keeps
  ...
  <Accordion title="Troubleshooting">
    - If model discovery fails at startup, GreenchClaw falls back to the bundled static catalog containing `kilocode/kilo/auto`.
    - Confirm your API key is valid and that your Kilo account has the desired models enabled.
    - When the Gateway runs as a daemon, ensure `KILOCODE_API_KEY` is available to that process (for example in `~/.GreenchClaw/.env` or via `env.shellEnv`).
