---
summary: "CLI reference for `GreenchClaw qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
GreenchClaw qr
GreenchClaw qr --setup-code-only
GreenchClaw qr --json
GreenchClaw qr --remote
GreenchClaw qr --url wss://gateway.example/ws
```
...
- With `--remote`, GreenchClaw requires either `gateway.remote.url` or
