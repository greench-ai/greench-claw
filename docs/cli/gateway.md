---
summary: "GreenchClaw Gateway CLI (`GreenchClaw gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is GreenchClaw's WebSocket server (channels, nodes, sessions, hooks). Subcommands in this page live under `GreenchClaw gateway …`.
...
<CardGroup cols={3}>
  ...
  <Card title="Discovery overview" href="/gateway/discovery">
    How GreenchClaw advertises and finds gateways.
  </Card>
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    - By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.GreenchClaw/GreenchClaw.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
    - `GreenchClaw onboard --mode local` and `GreenchClaw setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
<ParamField path="--token <token>" type="string">
  Token override (also sets `GREENCHCLAW_GATEWAY_TOKEN` for the process).
</ParamField>
...
GreenchClaw gateway restart --safe
GreenchClaw gateway restart --safe --skip-deferral
GreenchClaw gateway restart --force
```

`GreenchClaw gateway restart --safe` asks the running Gateway to preflight active GreenchClaw work before restarting. If queued operations, reply delivery, embedded runs, or task runs are active, the Gateway reports the blockers, coalesces duplicate safe restart requests, and restarts once the active work drains. Plain `restart` keeps the existing service-manager behavior for compatibility. Use `--force` only when you explicitly want the immediate override path.

`GreenchClaw gateway restart --safe --skip-deferral` runs the same GreenchClaw-aware coordinated restart as `--safe`, but bypasses the active-work deferral gate so the Gateway emits the restart immediately even when blockers are reported. Use it as the operator escape hatch when a deferral has been pinned by a stuck task run and `--safe` alone would wait indefinitely. `--skip-deferral` requires `--safe`.
...
- Set `GREENCHCLAW_GATEWAY_STARTUP_TRACE=1` to log phase timings during Gateway startup, including per-phase `eventLoopMax` delay and plugin lookup-table timings for installed-index, manifest registry, startup planning, and owner-map work.
- Set `GREENCHCLAW_DIAGNOSTICS=timeline` with `GREENCHCLAW_DIAGNOSTICS_TIMELINE_PATH=<path>` to write a best-effort JSONL startup diagnostics timeline for external QA harnesses. You can also enable the flag with `diagnostics.flags: ["timeline"]` in config; the path is still env-provided. Add `GREENCHCLAW_DIAGNOSTICS_EVENT_LOOP=1` to include event-loop samples.
...
```bash
GreenchClaw gateway health --url ws://127.0.0.1:18789
```
...
```bash
GreenchClaw gateway usage-cost
GreenchClaw gateway usage-cost --days 7
GreenchClaw gateway usage-cost --json
```
...
<AccordionGroup>
  <Accordion title="Privacy and bundle behavior">
    ...
    - On fatal Gateway exits, shutdown timeouts, and restart startup failures, GreenchClaw writes the same diagnostic snapshot to `~/.GreenchClaw/logs/stability/GreenchClaw-stability-*.json` when the recorder has events. Inspect the newest bundle with `GreenchClaw gateway stability --bundle latest`; `--limit`, `--type`, and `--since-seq` also apply to bundle output.
...
GreenchClaw gateway status --require-rpc
```
...
```bash
GreenchClaw gateway call status
GreenchClaw gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
responsible for eventually exec'ing `GreenchClaw` or Node with those args.

```bash
cat > ~/.local/bin/GreenchClaw-doppler <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec doppler run --project my-project --config production -- GreenchClaw "$@"
EOF
chmod +x ~/.local/bin/GreenchClaw-doppler

GreenchClaw gateway install --wrapper ~/.local/bin/GreenchClaw-doppler --force
GreenchClaw gateway restart
```
...
`GREENCHCLAW_WRAPPER` in the service environment for later forced reinstalls, updates, and doctor
repairs.

```bash
GREENCHCLAW_WRAPPER="$HOME/.local/bin/GreenchClaw-doppler" GreenchClaw gateway install --force
GreenchClaw doctor
```

To remove a persisted wrapper, clear `GREENCHCLAW_WRAPPER` while reinstalling:

```bash
GREENCHCLAW_WRAPPER= GreenchClaw gateway install --force
GreenchClaw gateway restart
```
...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - `gateway restart --safe` asks the running Gateway to preflight active GreenchClaw work and defer the restart until reply delivery, embedded runs, and task runs drain. `--safe` cannot be combined with `--force` or `--wait`.
    ...
    - `gateway restart --safe --skip-deferral` runs the GreenchClaw-aware safe restart but bypasses the deferral gate so the Gateway emits the restart immediately even when blockers are reported. Operator escape hatch for stuck-task-run deferrals; requires `--safe`.
  ...
  <Accordion title="Auth and SecretRefs at install time">
    ...
    - For password auth on `gateway run`, prefer `GREENCHCLAW_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
    - In inferred auth mode, shell-only `GREENCHCLAW_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
...
`gateway discover` scans for Gateway beacons (`_GreenchClaw-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `GreenchClaw.internal.`) and set up split DNS + a DNS server; see [Bonjour](/gateway/bonjour).
...
```bash
GreenchClaw gateway discover --timeout 4000
GreenchClaw gateway discover --json | jq '.beacons[].wsUrl'
```
