...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an GreenchClaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`GreenchClaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to GreenchClaw channel
conversations instead of hosting an ACP harness session, use
[`GreenchClaw mcp serve`](/cli/mcp) instead.

## What this is not

This page is often confused with ACP harness sessions.

`GreenchClaw acp` means:

- GreenchClaw acts as an ACP server
- an IDE or ACP client connects to GreenchClaw
- GreenchClaw forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where GreenchClaw runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to GreenchClaw: use `GreenchClaw acp`
- GreenchClaw should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include GreenchClaw parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                             |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the GreenchClaw gateway or agent instead.                                                                                                                                  |
...
# Remote Gateway
GreenchClaw acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
GreenchClaw acp --url wss://gateway-host:18789 --token-file ~/.GreenchClaw/gateway.token

# Attach to an existing session key
GreenchClaw acp --session agent:main:main

# Attach by label (must already exist)
GreenchClaw acp --session-label "support inbox"

# Reset the session key before the first prompt
GreenchClaw acp --session agent:main:main --reset-session
```
...
```bash
GreenchClaw acp client

# Point the spawned bridge at a remote Gateway
GreenchClaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.GreenchClaw/gateway.token

# Override the server command (default: GreenchClaw)
GreenchClaw acp client --server "node" --server-args GreenchClaw.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run GreenchClaw through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`GreenchClaw acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `GreenchClaw gateway call sessions.list` as the only ACP proof. That
...
it to drive an GreenchClaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `GreenchClaw acp` over stdio.

Example config (persisted):

```bash
GreenchClaw config set gateway.remote.url wss://gateway-host:18789
GreenchClaw config set gateway.remote.token <token>
```
...
```bash
GreenchClaw acp --session agent:main:main
GreenchClaw acp --session agent:design:main
GreenchClaw acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see GreenchClaw plugin tools or selected
...
[GreenchClaw tools MCP bridge](/tools/acp-agents-setup#GreenchClaw-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
GreenchClaw bot over ACP, use `acpx` with its built-in `GreenchClaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx GreenchClaw` at `GreenchClaw acp`.
3. Target the GreenchClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default GreenchClaw ACP session
acpx GreenchClaw exec "Summarize the active GreenchClaw session state."

# Persistent named session for follow-up turns
acpx GreenchClaw sessions ensure --name codex-bridge
acpx GreenchClaw -s codex-bridge --cwd /path/to/repo \
  "Ask my GreenchClaw work agent for recent context relevant to this repo."
```

If you want `acpx GreenchClaw` to target a specific Gateway and session key every
time, override the `GreenchClaw` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "GreenchClaw": {
      "command": "env GREENCHCLAW_HIDE_BANNER=1 GREENCHCLAW_SUPPRESS_NOTES=1 GreenchClaw acp --url ws://127.0.0.1:18789 --token-file ~/.GreenchClaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local GreenchClaw checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env GREENCHCLAW_HIDE_BANNER=1 GREENCHCLAW_SUPPRESS_NOTES=1 node GreenchClaw.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an GreenchClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select "GreenchClaw ACP" to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`GREENCHCLAW_GATEWAY_TOKEN`, `GREENCHCLAW_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`GREENCHCLAW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `GREENCHCLAW_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `GreenchClaw acp client` sets `GREENCHCLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `GreenchClaw`).
