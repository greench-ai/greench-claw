...
read_when:
  - You need a beginner-friendly overview of GreenchClaw logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

GreenchClaw has two main log surfaces:
...
GreenchClaw keeps up to five numbered archives beside the active file, such as
`GreenchClaw-YYYY-MM-DD.1.log`, and keeps writing to a fresh active log instead of
suppressing diagnostics.

You can override this in `~/.GreenchClaw/GreenchClaw.json`:

```json
{
  "logging": {
    "file": "/path/to/GreenchClaw.log"
  }
}
```
...
or times out before `logs.tail` answers, `GreenchClaw logs` falls back to the
...
```bash
GreenchClaw channels logs --channel whatsapp
```
...
GreenchClaw preserves the original structured log arguments alongside these fields
...
`GreenchClaw gateway` also has WebSocket protocol logging for RPC traffic:
...
```bash
GreenchClaw gateway
GreenchClaw gateway --verbose --ws-log compact
GreenchClaw gateway --verbose --ws-log full
```

## Configuring logging

All logging configuration lives under `logging` in `~/.GreenchClaw/GreenchClaw.json`.

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/GreenchClaw/GreenchClaw-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
You can override both via the **`GREENCHCLAW_LOG_LEVEL`** environment variable (e.g. `GREENCHCLAW_LOG_LEVEL=debug`). The env var takes precedence over the config file, so you can raise verbosity for a single run without editing `GreenchClaw.json`. You can also pass the global CLI option **`--log-level <level>`** (for example, `GreenchClaw --log-level debug gateway run`), which overrides the environment variable for that command.
...
```bash
GREENCHCLAW_DEBUG_MODEL_TRANSPORT=1 GreenchClaw gateway
GREENCHCLAW_DEBUG_MODEL_PAYLOAD=tools GREENCHCLAW_DEBUG_SSE=events GreenchClaw gateway
```

Available flags:

- `GREENCHCLAW_DEBUG_MODEL_TRANSPORT=1`: emit request start, fetch response, SDK
  headers, first streaming event, stream completion, and transport errors at
  `info` level.
- `GREENCHCLAW_DEBUG_MODEL_PAYLOAD=summary`: include a bounded request payload
  summary in model request logs.
- `GREENCHCLAW_DEBUG_MODEL_PAYLOAD=tools`: include all model-facing tool names in
  the payload summary.
- `GREENCHCLAW_DEBUG_MODEL_PAYLOAD=full-redacted`: include a redacted, capped JSON
  payload snapshot. Use only while debugging; secrets are redacted but prompts
  and message text may still be present.
- `GREENCHCLAW_DEBUG_SSE=events`: emit first-event and stream-completion timing.
- `GREENCHCLAW_DEBUG_SSE=peek`: also emit the first five redacted SSE event
  payloads, capped per event.
- `GREENCHCLAW_DEBUG_CODE_MODE=1`: emit code-mode model-surface diagnostics,
  including when native provider tools are hidden because code mode owns the
  tool surface.

These flags log through normal GreenchClaw logging, so `GreenchClaw logs --follow`
...
GreenchClaw writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
GreenchClaw can redact sensitive tokens before they hit console output, file logs,
...
policy. GreenchClaw still redacts safety-boundary payloads that can be shown to UI
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `GREENCHCLAW_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `GreenchClaw doctor` first.
