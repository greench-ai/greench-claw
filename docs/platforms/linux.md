...
2. `npm i -g GreenchClaw@latest`
3. `GreenchClaw onboard --install-daemon`
...
GreenchClaw installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `GreenchClaw gateway install` and
`GreenchClaw onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/GreenchClaw-gateway[-<profile>].service`:

```
[Unit]
Description=GreenchClaw Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. GreenchClaw therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, GreenchClaw starts the child through a short
...
- GreenchClaw-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `GREENCHCLAW_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
