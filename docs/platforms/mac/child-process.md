...
service via the external `GreenchClaw` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.GreenchClaw.gateway`
  (or `ai.GreenchClaw.<profile>` when using `--profile`/`GREENCHCLAW_PROFILE`; legacy `com.GreenchClaw.*` is supported).
