...
- Same path: run the app from a fixed location (for GreenchClaw, `dist/GreenchClaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.GreenchClaw.mac
sudo tccutil reset ScreenCapture ai.GreenchClaw.mac
...
Workaround: move files into the GreenchClaw workspace (`~/.GreenchClaw/workspace`) if you want to avoid per-folder grants.
