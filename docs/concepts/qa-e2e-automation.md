...
The private QA stack is meant to exercise GreenchClaw in a more realistic,
...
Every QA flow runs under `pnpm GreenchClaw qa <subcommand>`. Many have `pnpm qa:*`
script aliases; both forms are supported.

| Command                                             | Purpose                                                                                                                                                                                                                                                                 |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `qa run`                                            | Bundled QA self-check; writes a Markdown report.                                                                                                                                                                                                                        |
| `qa suite`                                          | Run repo-backed scenarios against the QA gateway lane. Aliases: `pnpm GreenchClaw qa suite --runner multipass` for a disposable Linux VM.                                                                                                                               |
...
```bash
pnpm GreenchClaw qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`GreenchClaw.run`, `GreenchClaw.harness.run`, `GreenchClaw.model.call`,
`GreenchClaw.context.assembled`, and `GreenchClaw.message.delivery` must be present;
model calls must not export `StreamAbandoned` on successful turns; raw diagnostic IDs and
`GreenchClaw.content.*` attributes must stay out of the trace. It writes
...
```bash
pnpm GreenchClaw qa matrix --profile fast --fail-fast
```
...
CI uses the same command surface in `.github/workflows/qa-live-transports-convex.yml`. Scheduled and default manual runs execute the fast Matrix profile with live frontier credentials, `--fast`, and `GREENCHCLAW_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`. Manual `matrix_profile=all` fans out into the five profile shards so the exhaustive catalog can run in parallel while keeping one artifact directory per shard.

For transport-real Telegram, Discord, and Slack smoke lanes:

```bash
pnpm GreenchClaw qa telegram
pnpm GreenchClaw qa discord
pnpm GreenchClaw qa slack
```
...
```bash
pnpm GreenchClaw qa mantis slack-desktop-smoke \
  --gateway-setup \
...
With `--gateway-setup`, Mantis leaves a persistent GreenchClaw Slack gateway
...
```bash
pnpm GreenchClaw qa mantis visual-task \
...
`visual-driver`, captures `visual-task.png`, runs `GreenchClaw infer image describe`
...
```bash
pnpm GreenchClaw qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds GreenchClaw
...
- `GREENCHCLAW_QA_TELEGRAM_GROUP_ID` - numeric chat id (string).
- `GREENCHCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN`
- `GREENCHCLAW_QA_TELEGRAM_SUT_BOT_TOKEN`

Optional:

- `GREENCHCLAW_QA_TELEGRAM_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts (default redacts).
...
The implicit default set always covers canary, mention gating, native command replies, command addressing, and bot-to-bot group replies. `mock-openai` defaults also include deterministic reply-chain and final-message streaming checks. `telegram-current-session-status-tool` remains opt-in because it is only stable when threaded directly after canary, not after arbitrary native command replies. Use `pnpm GreenchClaw qa telegram --list-scenarios --provider-mode mock-openai` to print the current default/optional split with regression refs.

Output artifacts:

- `telegram-qa-report.md`
- `telegram-qa-summary.json` - includes per-reply RTT (driver send → observed SUT reply) starting with the canary.
- `telegram-qa-observed-messages.json` - bodies redacted unless `GREENCHCLAW_QA_TELEGRAM_CAPTURE_CONTENT=1`.
...
Targets one real private Discord guild channel with two bots: a driver bot controlled by the harness and a SUT bot started by the child GreenchClaw gateway through the bundled Discord plugin. Verifies channel mention handling, that the SUT bot has registered the native `/help` command with Discord, and opt-in Mantis evidence scenarios.

Required env when `--credential-source env`:

- `GREENCHCLAW_QA_DISCORD_GUILD_ID`
- `GREENCHCLAW_QA_DISCORD_CHANNEL_ID`
- `GREENCHCLAW_QA_DISCORD_DRIVER_BOT_TOKEN`
- `GREENCHCLAW_QA_DISCORD_SUT_BOT_TOKEN`
- `GREENCHCLAW_QA_DISCORD_SUT_APPLICATION_ID` - must match the SUT bot user id returned by Discord (the lane fails fast otherwise).

Optional:

- `GREENCHCLAW_QA_DISCORD_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
- `GREENCHCLAW_QA_DISCORD_VOICE_CHANNEL_ID` selects the voice/stage channel for `discord-voice-autojoin`; without it, the scenario picks the first visible voice/stage channel for the SUT bot.
...
- `discord-qa-observed-messages.json` - bodies redacted unless `GREENCHCLAW_QA_DISCORD_CAPTURE_CONTENT=1`.
...
Targets one real private Slack channel with two distinct bots: a driver bot controlled by the harness and a SUT bot started by the child GreenchClaw gateway through the bundled Slack plugin.

Required env when `--credential-source env`:

- `GREENCHCLAW_QA_SLACK_CHANNEL_ID`
- `GREENCHCLAW_QA_SLACK_DRIVER_BOT_TOKEN`
- `GREENCHCLAW_QA_SLACK_SUT_BOT_TOKEN`
- `GREENCHCLAW_QA_SLACK_SUT_APP_TOKEN`

Optional:

- `GREENCHCLAW_QA_SLACK_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
...
- `slack-qa-observed-messages.json` - bodies redacted unless `GREENCHCLAW_QA_SLACK_CAPTURE_CONTENT=1`.
...
{
  "display_information": {
    "name": "GreenchClaw QA Driver",
    "description": "Test driver bot for GreenchClaw QA Slack live lane"
  },
  "features": {
    "bot_user": {
      "display_name": "GreenchClaw QA Driver",
      "always_online": true
    }
  },
  "oauth_config": {
...
{
  "display_information": {
    "name": "GreenchClaw QA SUT",
    "description": "GreenchClaw QA SUT connector for GreenchClaw"
  },
  "features": {
    "bot_user": {
      "display_name": "GreenchClaw QA SUT",
      "always_online": true
    },
    "app_home": {
...
In the QA workspace, create a channel (e.g. `#GreenchClaw-qa`) and invite both bots from inside the channel:

```
/invite @GreenchClaw QA Driver
/invite @GreenchClaw QA SUT
```
...
Two options. Use env vars for single-machine debugging (set the four `GREENCHCLAW_QA_SLACK_*` variables and pass `--credential-source env`), or seed the shared Convex pool so CI and other maintainers can lease them.
...
With `GREENCHCLAW_QA_CONVEX_SITE_URL` and `GREENCHCLAW_QA_CONVEX_SECRET_MAINTAINER` exported in your shell, register and verify:
...
pnpm GreenchClaw qa credentials list --kind slack --status all --json
```
...
```bash
pnpm qa:telegram-user:crabbox -- start --tdlib-url http://artifacts.GreenchClaw.ai/tdlib-v1.8.0-linux-x64.tgz --output-dir .artifacts/qa-e2e/telegram-user-crabbox/pr-review
...
- `mock-openai` is the scenario-aware GreenchClaw mock. It remains the default
...
- the `GreenchClaw qa` command root
...
- how `GreenchClaw qa <runner>` is mounted beneath the shared `qa` root
...
4. Mount the runner as `GreenchClaw qa <runner>` instead of registering a competing root command. Runner plugins should declare `qaRunners` in `GreenchClaw.plugin.json` and export a matching `qaRunnerCliRegistrations` array from `runtime-api.ts`. Keep `runtime-api.ts` light; lazy CLI and runner execution should stay behind separate entrypoints.
...
For the inventory of available scenarios - useful when sizing follow-up work or wiring a new transport - run `pnpm GreenchClaw qa coverage` (add `--json` for machine-readable output).

For character and style checks, run the same scenario across multiple live model
refs and write a judged Markdown report:

```bash
pnpm GreenchClaw qa character-eval \
