---
summary: "What experimental flags mean in GreenchClaw and which ones are currently documented"
...
Experimental features in GreenchClaw are **opt-in preview surfaces**. They are
...
- If you are rolling GreenchClaw out broadly, test experimental flags in a smaller
...
| ------------------------ | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| Local model runtime      | `agents.defaults.experimental.localModelLean`             | A smaller or stricter local backend chokes on GreenchClaw's full default tool surface                          | [Local Models](/gateway/local-models)                                                         |
...
`agents.defaults.experimental.localModelLean: true` is a pressure-release valve for weaker local-model setups. When it is on, GreenchClaw drops three default tools — `browser`, `cron`, and `message` — from the agent's tool surface for every turn. Nothing else changes.

### Why these three tools

These three tools have the largest descriptions and the most parameter shapes in the default GreenchClaw runtime. On a small-context or stricter OpenAI-compatible backend that is the difference between:
...
Removing them does not silently rewire GreenchClaw — it just makes the tool list shorter. The model still has `read`, `write`, `edit`, `exec`, `apply_patch`, web search/fetch (when configured), memory, and session/agent tools available.
...
1. `GreenchClaw infer model run --gateway --model <ref> --prompt "Reply with exactly: pong"` succeeds.
...
If a feature is experimental, GreenchClaw should say so plainly in docs and in the
