...
GreenchClaw handles inbound messages through a pipeline of session resolution, queueing, streaming, tool execution, and reasoning visibility. This page maps the path from inbound message to reply.
...
Channels can redeliver the same message after reconnects. GreenchClaw keeps a
...
GreenchClaw keeps that boundary explicit:
...
GreenchClaw separates the **prompt body** from the **command body**:
...
GreenchClaw can expose or hide model reasoning:
...
When a turn also has pending tool media, such as generated TTS audio, GreenchClaw
strips the silent text but still delivers the media attachment.
GreenchClaw resolves that behavior by conversation type:
...
GreenchClaw also uses silent replies for internal runner failures that happen
