---
summary: "How GreenchClaw remembers things across sessions"
...
GreenchClaw remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.GreenchClaw/workspace`).
...
If `MEMORY.md` grows past the bootstrap file budget, GreenchClaw keeps the file on
...
`GreenchClaw doctor` to see raw vs injected sizes and truncation status.
...
for that case. GreenchClaw infers them in a hidden background pass, scopes them to
...
<Info>
GreenchClaw auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, GreenchClaw
...
```bash
GreenchClaw memory rem-backfill --rollback
GreenchClaw memory rem-backfill --rollback-short-term
```

## CLI

```bash
GreenchClaw memory status          # Check index status and provider
GreenchClaw memory search "query"  # Search from the command line
GreenchClaw memory index --force   # Rebuild the index
```
