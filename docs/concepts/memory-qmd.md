...
alongside GreenchClaw. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, GreenchClaw falls back to the
...
GreenchClaw creates a self-contained QMD home under
`~/.GreenchClaw/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- GreenchClaw creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). `search` is BM25-only, so GreenchClaw skips semantic
  vector readiness probes and embedding maintenance in that mode. If a mode
  fails, GreenchClaw retries with `qmd query`.
- With QMD releases that advertise multi-collection filters, GreenchClaw groups
  same-source collections into one QMD search invocation. Older QMD releases
  keep the compatible per-collection fallback.
- If QMD fails entirely, GreenchClaw falls back to the builtin SQLite engine.
  ...
  `GreenchClaw memory status` and one-shot CLI probes still recheck QMD directly.
...
GreenchClaw keeps the QMD search path compatible with both current and older QMD
installs.

On startup, GreenchClaw checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, GreenchClaw searches all
...
Older QMD builds only accept one collection filter. When GreenchClaw detects one
...
process, so you can tune QMD globally without adding new GreenchClaw config:
...
collection under `~/.GreenchClaw/agents/<id>/qmd/sessions/`.
...
When scope denies a search, GreenchClaw logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If GreenchClaw
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but GreenchClaw still reports
...
with `GreenchClaw memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs GreenchClaw uses.

**Many QMD subprocesses during search?** Update QMD if possible. GreenchClaw uses
...
`memory.qmd.searchMode = "search"`. GreenchClaw treats that mode as lexical-only,
...
the next startup reconciliation. GreenchClaw recreates stale managed collections
...
GreenchClaw's builtin symlink rules. Keep temporary monorepo checkouts under
