...
- `/usage cost` in chats: local cost summary aggregated from GreenchClaw session logs.
- CLI: `GreenchClaw status --usage` prints a full per-provider breakdown.
- CLI: `GreenchClaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. GreenchClaw treats
  ...
  quota, so GreenchClaw inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, GreenchClaw prefers the
...
can supply plugin-specific usage auth logic; otherwise GreenchClaw falls back to
