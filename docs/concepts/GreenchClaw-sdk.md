---
summary: "Public GreenchClaw App SDK for external apps, scripts, dashboards, CI jobs, and IDE extensions"
title: "GreenchClaw App SDK"
sidebarTitle: "App SDK"
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to GreenchClaw
...
The **GreenchClaw App SDK** is the public client API for apps outside the
GreenchClaw process. Use `@GreenchClaw/sdk` when a script, dashboard, CI job, IDE
...
<Note>
  ...
  `@GreenchClaw/sdk` talks to the Gateway from outside GreenchClaw.
  `GreenchClaw/plugin-sdk/*` is only for plugins that run inside GreenchClaw and
  register providers, channels, tools, hooks, or trusted runtimes.
</Note>

## What ships today

`@GreenchClaw/sdk` ships with:

| Surface                   | Status  | What it does                                                                      |
| ------------------------- | ------- | --------------------------------------------------------------------------------- |
| `GreenchClaw`             | Ready   | Main client entry point. Owns transport, connection, requests, and events.        |
...
`AgentRunParams`, `RunResult`, `RunStatus`, `GreenchClawEvent`,
`GreenchClawEventType`, `GatewayEvent`, `GreenchClawTransport`,
...
```typescript
import { GreenchClaw } from "@GreenchClaw/sdk";

const oc = new GreenchClaw({
...
`new GreenchClaw({ gateway: "ws://..." })` is equivalent to `url`. The
...
For tests, pass an object that implements `GreenchClawTransport`:
...
The SDK normalizes raw Gateway events into a stable `GreenchClawEvent` envelope:

```typescript
type GreenchClawEvent = {
  version: 1;
  id: string;
  ts: number;
  type: GreenchClawEventType;
  runId?: string;
  sessionId?: string;
  sessionKey?: string;
  taskId?: string;
...
Task helpers use the durable task ledger that also backs `GreenchClaw tasks`:
...
Use the App SDK when code lives outside GreenchClaw:
...
Use the Plugin SDK when code runs inside GreenchClaw:
...
App SDK code should import from `@GreenchClaw/sdk`. Plugin code should import from
documented `GreenchClaw/plugin-sdk/*` subpaths. Do not mix the two contracts.

## Related

- [GreenchClaw App SDK API design](/reference/GreenchClaw-sdk-api-design)
