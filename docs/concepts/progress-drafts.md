...
read_when:
  ...
  - Explaining how GreenchClaw updates one channel message while work is in progress
...
When progress drafts are enabled, GreenchClaw creates one visible work-in-progress
...
That is usually enough. GreenchClaw will pick an automatic one-word label, wait
...
GreenchClaw sends the final answer normally and cleans up or stops updating the
...
The default label is `auto`, which chooses from GreenchClaw's built-in
...
GreenchClaw uses the same formatter for progress drafts and `/verbose`:
...
GreenchClaw truncates long progress lines by default so repeated draft edits do not
...
With `toolProgress: false`, GreenchClaw still suppresses the older standalone
...
When the final answer is ready, GreenchClaw tries to keep the chat clean:

- If the draft can safely become the final answer, GreenchClaw edits it in place.
- If the channel uses native progress streaming, GreenchClaw finalizes that stream
...
- If the final answer has media, an approval prompt, an explicit reply target,
  too many chunks, or a failed edit/send, GreenchClaw sends the final answer through
...
Check `streaming.progress.toolProgress`. If it is `false`, GreenchClaw keeps the
