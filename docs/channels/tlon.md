...
Tlon is a decentralized messenger built on Urbit. GreenchClaw connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current GreenchClaw releases, so normal packaged
...
```bash
GreenchClaw plugins install @GreenchClaw/tlon
```
...
```bash
GreenchClaw plugins install ./path/to/local/tlon-plugin
```
...
By default, GreenchClaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `GreenchClaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, GreenchClaw replies in-thread.
