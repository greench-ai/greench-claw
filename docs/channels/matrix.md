...
read_when:
  - Setting up Matrix in GreenchClaw
  - Configuring Matrix E2EE and verification
title: "Matrix"
---

Matrix is a downloadable channel plugin for GreenchClaw.
...
```bash
GreenchClaw plugins install @GreenchClaw/matrix
```

Bare plugin specs try ClawHub first, then npm fallback. To force the registry source, use `GreenchClaw plugins install clawhub:@GreenchClaw/matrix` or `GreenchClaw plugins install npm:@GreenchClaw/matrix`.

From a local checkout:

```bash
GreenchClaw plugins install ./path/to/local/matrix-plugin
```

`plugins install` registers and enables the plugin, so no separate `GreenchClaw plugins enable matrix` step is needed. The plugin still does nothing until you configure the channel below. See [Plugins](/tools/plugin) for general plugin behavior and install rules.
...
GreenchClaw configure --section channels
```
...
If matching `MATRIX_*` env vars already exist and the selected account has no saved auth, the wizard offers an env-var shortcut. To resolve room names before saving an allowlist, run `GreenchClaw channels resolve --channel matrix "Project Room"`. When E2EE is enabled, the wizard writes the config and runs the same bootstrap as [`GreenchClaw matrix encryption setup`](#encryption-and-verification).
...
```json5
{
  channels: {
    matrix: {
      ...
      deviceName: "GreenchClaw Gateway",
    },
  },
}
```
...
GreenchClaw cannot tell at invite time whether an invited room is a DM or a group, so all invites - including DM-style invites - go through `autoJoin` first. `dm.policy` only applies later, after the bot has joined and the room has been classified.
...
Matrix stores cached credentials under `~/.GreenchClaw/credentials/matrix/`:

- default account: `credentials.json`
- named accounts: `credentials-<account>.json`

When cached credentials exist there, GreenchClaw treats Matrix as configured even if the access token is not in the config file - that covers setup, `GreenchClaw doctor`, and channel-status probes.
...
Matrix reply streaming is opt-in. `streaming` controls how GreenchClaw delivers the in-flight assistant reply; `blockStreaming` controls whether each completed block is preserved as its own Matrix message.
...
- If a preview grows past Matrix's per-event size limit, GreenchClaw stops preview streaming and falls back to final-only delivery.
- Media replies always send attachments normally. If a stale preview can no longer be reused safely, GreenchClaw redacts it before sending the final media reply.
...
Matrix native approval prompts are normal `m.room.message` events with GreenchClaw-specific custom event content under `com.GreenchClaw.approval`. Matrix permits custom event-content keys, so stock clients still render the text body while GreenchClaw-aware clients can read the structured approval id, kind, state, available decisions, and exec/plugin details.

When an approval prompt is too long for one Matrix event, GreenchClaw chunks the visible text and attaches `com.GreenchClaw.approval` to the first chunk only. Reactions for allow/deny decisions are bound to that first event, so long prompts keep the same approval target as single-event prompts.
...
By default, Matrix messages from other configured GreenchClaw Matrix accounts are ignored.
...
- GreenchClaw still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix does not expose a native bot flag here; GreenchClaw treats "bot-authored" as "sent by another configured Matrix account on this GreenchClaw gateway".
...
All `GreenchClaw matrix` commands accept `--verbose` (full diagnostics), `--json` (machine-readable output), and `--account <id>` (multi-account setups). Output is concise by default with quiet internal SDK logging. The examples below show the canonical form; add the flags as needed.

### Enable encryption

```bash
GreenchClaw matrix encryption setup
```
...
```bash
GreenchClaw matrix account add \
  --homeserver https://matrix.example.org \
...
GreenchClaw matrix verify status --include-recovery-key --json
```
...
The literal-key form `GreenchClaw matrix verify device "<recovery-key>"` is also accepted, but the key ends up in your shell history.
...
If the homeserver requires UIA to upload cross-signing keys, GreenchClaw tries no-auth first, then `m.login.dummy`, then `m.login.password` (requires `channels.matrix.password`).
...
```bash
GreenchClaw matrix verify list
```

Lists pending verification requests for the selected account.

```bash
GreenchClaw matrix verify request --own-user
GreenchClaw matrix verify request --user-id @ops:example.org --device-id ABCDEF
```

Sends a verification request from this GreenchClaw account. `--own-user` requests self-verification (you accept the prompt in another Matrix client of the same user); `--user-id`/`--device-id`/`--room-id` target someone else. `--own-user` cannot be combined with the other targeting flags.
...
| Command                                       | Purpose                                                             |
| --------------------------------------------- | ------------------------------------------------------------------- |
| `GreenchClaw matrix verify accept <id>`       | Accept an inbound request                                           |
| `GreenchClaw matrix verify start <id>`        | Start the SAS flow                                                  |
| `GreenchClaw matrix verify sas <id>`          | Print the SAS emoji or decimals                                     |
| `GreenchClaw matrix verify confirm-sas <id>`  | Confirm that the SAS matches what the other client shows            |
| `GreenchClaw matrix verify mismatch-sas <id>` | Reject the SAS when the emoji or decimals do not match              |
| `GreenchClaw matrix verify cancel <id>`       | Cancel; takes optional `--reason <text>` and `--code <matrix-code>` |
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    ...
    Startup also runs a conservative crypto bootstrap pass that reuses the current secret storage and cross-signing identity. If bootstrap state is broken, GreenchClaw attempts a guarded repair even without `channels.matrix.password`; if the homeserver requires password UIA, startup logs a warning and stays non-fatal. Already-owner-signed devices are preserved.
  ...
  <Accordion title="Verification notices">
    ...
    Incoming requests from another Matrix client are tracked and auto-accepted. For self-verification, GreenchClaw starts the SAS flow automatically and confirms its own side once emoji verification is available - you still need to compare and confirm "They match" in your Matrix client.
  ...
  <Accordion title="Deleted or invalid Matrix device">
    If `verify status` says the current device is no longer listed on the homeserver, create a new GreenchClaw Matrix device. For password login:
...
GreenchClaw matrix account add \
  ...
  --password '<password>' \
  --device-name GreenchClaw-Gateway
```

    For token auth, create a fresh access token in your Matrix client or admin UI, then update GreenchClaw:
...
```
  ...
  <Accordion title="Device hygiene">
    Old GreenchClaw-managed devices can accumulate. List and prune:

```bash
GreenchClaw matrix devices list
GreenchClaw matrix devices prune-stale
```
  ...
  <Accordion title="Crypto store">
    ...
    Encrypted runtime state lives under `~/.GreenchClaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/` and includes the sync store, crypto store, recovery key, IDB snapshot, thread bindings, and startup verification state. When the token changes but the account identity stays the same, GreenchClaw reuses the best existing root so prior state remains visible.
...
```bash
GreenchClaw matrix profile set --name "GreenchClaw Assistant"
GreenchClaw matrix profile set --avatar-url https://cdn.example.org/avatar.png
```

You can pass both options in one call. Matrix accepts `mxc://` avatar URLs directly; when you pass `http://` or `https://`, GreenchClaw uploads the file first and stores the resolved `mxc://` URL into `channels.matrix.avatarUrl` (or the per-account override).
...
`dm.sessionScope` decides how Matrix DM rooms map to GreenchClaw sessions:
...
- DM user-target reuse only kicks in when the current session metadata proves the same DM peer on the same Matrix account; otherwise GreenchClaw falls back to normal user-scoped routing.
...
When GreenchClaw detects a Matrix DM room colliding with another DM room on the same shared session, it posts a one-time `m.notice` in that room pointing to the `/focus` escape hatch and suggesting a `dm.sessionScope` change. The notice only appears when thread bindings are enabled.
...
- `threadBindings.spawnSessions` gates `/acp spawn --thread auto|here`, where GreenchClaw needs to create or bind a child Matrix thread.
...
- Matrix room history is pending-only: GreenchClaw buffers room messages that did not trigger a reply yet, then snapshots that window when a mention or other trigger arrives.
...
```bash
GreenchClaw pairing list matrix
GreenchClaw pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging you before approval, GreenchClaw reuses the same pending pairing code and may send a reminder reply after a short cooldown instead of minting a new code.

See [Pairing](/channels/pairing) for the shared DM pairing flow and storage layout.

## Direct room repair

If direct-message state drifts out of sync, GreenchClaw can end up with stale `m.direct` mappings that point at old solo rooms instead of the live DM. Inspect the current mapping for a peer:

```bash
GreenchClaw matrix direct inspect --user-id @alice:example.org
```

Repair it:

```bash
GreenchClaw matrix direct repair --user-id @alice:example.org
```
...
- If you have multiple accounts and one is literally named `default`, GreenchClaw uses it implicitly even when `defaultAccount` is unset.
...
- When GreenchClaw promotes a single-account config to multi-account during repair or setup, it preserves the existing named account if one exists or `defaultAccount` already points at one. Only Matrix auth/bootstrap keys move into the promoted account; shared delivery-policy keys stay at the top level.

See [Configuration reference](/gateway/config-channels#multi-account-all-channels) for the shared multi-account pattern.

## Private/LAN homeservers

By default, GreenchClaw blocks private/internal Matrix homeservers for SSRF protection unless you
...
GreenchClaw uses the same proxy setting for runtime Matrix traffic and account status probes.

## Target resolution

Matrix accepts these target forms anywhere GreenchClaw asks you for a room or user target:
...
GreenchClaw keeps internal session keys canonical for storage, so those lowercase
