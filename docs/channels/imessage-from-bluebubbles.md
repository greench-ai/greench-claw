...
BlueBubbles support was removed. GreenchClaw supports iMessage through `imsg` only. This guide is for migrating old `channels.bluebubbles` configs to `channels.imessage`; there is no other supported migration path.
...
5. With the Gateway stopped, enable `channels.imessage`, then run `GreenchClaw channels status --probe --channel imessage`.
...
`imsg` is a local macOS CLI for Messages. GreenchClaw starts `imsg rpc` as a child process and talks JSON-RPC over stdin/stdout. There is no HTTP server, webhook URL, background daemon, launch agent, or port to expose.
...
- Linux builds can inspect a copied `chat.db`, but cannot send, watch the live Mac database, or drive Messages.app. For GreenchClaw iMessage, run `imsg` on the signed-in Mac or through an SSH wrapper to that Mac.
...
2. Verify the read, watch, send, and RPC surfaces before changing GreenchClaw config:

   ```bash
   imsg chats --limit 10 --json | jq -s
   imsg history --chat-id 42 --limit 10 --attachments --json | jq -s
   imsg watch --chat-id 42 --reactions --json
   imsg send --chat-id 42 --text "GreenchClaw imsg test"
   imsg rpc --help
   ```

   Replace `42` with a real chat id from `imsg chats`. Sending requires Automation permission for Messages.app. If GreenchClaw will run through SSH, run these commands through the same SSH wrapper or user context that GreenchClaw will use.
...
4. After you add an enabled `channels.imessage` config, verify the bridge through GreenchClaw:
...
5. Snapshot your config:
   ...
   ```bash
   cp ~/.GreenchClaw/GreenchClaw.json5 ~/.GreenchClaw/GreenchClaw.json5.bak
   ```
...
7. **Remove the BlueBubbles server and config** once iMessage DMs, groups, and actions are verified. GreenchClaw will not use `channels.bluebubbles`.
...
| Threaded reply (`reply_to_guid`)                           | ✅                                  | ✅ (closes [#51892](https://github.com/GreenchClaw/GreenchClaw/issues/51892))                                                 |
| Tapback (`react`)                                          | ✅                                  | ✅                                                                                                                            |
| Edit / unsend (macOS 13+ recipients)                       | ✅                                  | ✅                                                                                                                            |
| Send with screen effect                                    | ✅                                  | ✅ (closes part of [#9394](https://github.com/GreenchClaw/GreenchClaw/issues/9394))                                           |
...
| Catchup of inbound messages received while gateway is down | ✅ (webhook replay + history fetch) | ✅ (opt-in via `channels.imessage.catchup.enabled`; closes [#78649](https://github.com/GreenchClaw/GreenchClaw/issues/78649)) |
...
The reply cache lives at `~/.GreenchClaw/state/imessage/reply-cache.jsonl` (mode `0600`, parent dir `0700`). It is safe to delete if you want a clean slate.
