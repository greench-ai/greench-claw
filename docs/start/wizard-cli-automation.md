---
summary: "Scripted onboarding and agent setup for the GreenchClaw CLI"
read_when:
...
Use `--non-interactive` to automate `GreenchClaw onboard`.
...
<AccordionGroup>
  ...
  <Accordion title="Custom provider example">
    ...
    GreenchClaw marks common vision model IDs as image-capable automatically. Add `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
...
Anthropic setup-token remains available as a supported onboarding token path, but GreenchClaw now prefers Claude CLI reuse when available.
For production, prefer an Anthropic API key.

## Add another agent

Use `GreenchClaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.GreenchClaw/workspace-<agentId>`.
