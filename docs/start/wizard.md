...
CLI onboarding is the **recommended** way to set up GreenchClaw on macOS,
...
`GreenchClaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`GreenchClaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.GreenchClaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `GreenchClaw doctor` first.
</Note>
...
- CLI command reference: [`GreenchClaw onboard`](/cli/onboard)
