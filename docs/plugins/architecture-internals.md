...
At startup, GreenchClaw does roughly this:
...
The manifest is the control-plane source of truth. GreenchClaw uses it to:
...
channels, slots, and auto-enable rules. If that derived set is empty, GreenchClaw
...
GreenchClaw does not cache plugin discovery results or direct manifest registry
...
GreenchClaw still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, GreenchClaw calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that GreenchClaw no longer calls, such as
...
| --  | _(built-in model lookup)_         | GreenchClaw tries the normal registry/catalog path first                                                       | _(not a plugin hook)_                                                                                                                         |
...
that still runs on GreenchClaw's normal inference loop.
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of GreenchClaw's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as GreenchClaw adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  product plugins own their routes and schemas while GreenchClaw owns the
...
- GreenchClaw only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "GreenchClaw plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-GreenchClaw-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-GreenchClaw-scopes` only when the header is explicitly present
  - if `x-GreenchClaw-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-GreenchClaw-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `GreenchClaw/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                                | Purpose                                                  |
| -------------------------------------- | -------------------------------------------------------- |
| `GreenchClaw/plugin-sdk/plugin-entry`  | Plugin registration primitives                           |
| `GreenchClaw/plugin-sdk/channel-core`  | Channel entry/build helpers                              |
| `GreenchClaw/plugin-sdk/core`          | Generic shared helpers and umbrella contract             |
| `GreenchClaw/plugin-sdk/config-schema` | Root `GreenchClaw.json` Zod schema (`GreenchClawSchema`) |
...
<Info>
`GreenchClaw/plugin-sdk/channel-runtime`, `GreenchClaw/plugin-sdk/config-runtime`,
and `GreenchClaw/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `GreenchClaw/plugin-sdk/*` subpaths. Never
...
`GreenchClaw/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape GreenchClaw writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to GreenchClaw's
...
- if both `catalog` and `discovery` are registered, GreenchClaw uses `catalog`
...
- Read-only command paths such as `GreenchClaw status`, `GreenchClaw status --all`,
  `GreenchClaw channels status`, `GreenchClaw channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `GreenchClaw.extensions`:
...
Security guardrail: every `GreenchClaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `GreenchClaw plugins install` installs plugin dependencies with a
...
Optional: `GreenchClaw.setupEntry` can point at a lightweight setup-only module.
When GreenchClaw needs setup surfaces for a disabled channel plugin, or
...
Optional: `GreenchClaw.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let GreenchClaw load the
...
Channel plugins can advertise setup/discovery metadata via `GreenchClaw.channel` and
install hints via `GreenchClaw.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@GreenchClaw/nextcloud-talk",
  "GreenchClaw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@GreenchClaw/nextcloud-talk",
...
Useful `GreenchClaw.channel` fields beyond the minimal example:
...
GreenchClaw can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.GreenchClaw/mpm/plugins.json`
- `~/.GreenchClaw/mpm/catalog.json`
- `~/.GreenchClaw/plugins/catalog.json`

Or point `GREENCHCLAW_PLUGIN_CATALOG_PATHS` (or `GREENCHCLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "GreenchClaw": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `GreenchClaw.install` block. The
...
```ts
import { buildMemorySystemPromptAddition } from "GreenchClaw/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `GreenchClawPluginApi` and/or `api.runtime` with the smallest useful
...
This is how GreenchClaw stays opinionated without becoming hardcoded to one
