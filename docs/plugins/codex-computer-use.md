---
summary: "Set up Codex Computer Use for Codex-mode GreenchClaw agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode GreenchClaw agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. GreenchClaw
...
Use this page when GreenchClaw is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## GreenchClaw.app and Peekaboo

GreenchClaw.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want GreenchClaw.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode GreenchClaw agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an GreenchClaw node and exposes mobile
...
GreenchClaw-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through GreenchClaw's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the GreenchClaw command:

```bash
cua-driver mcp-config --client GreenchClaw
```

or register the stdio server yourself:

```bash
GreenchClaw mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'
```
...
available as a normal GreenchClaw MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. GreenchClaw
...
With this config, GreenchClaw checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, GreenchClaw asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, GreenchClaw also tries to
...
plugin command surface is available. These are GreenchClaw chat/runtime commands,
not `GreenchClaw codex ...` CLI subcommands:
...
GreenchClaw uses the same app-server API that Codex itself exposes. The
...
During install, GreenchClaw polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, GreenchClaw prefers
...
`computer-use` is registered, GreenchClaw tries to add the standard bundled
...
## What GreenchClaw checks

GreenchClaw reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If GreenchClaw says Computer Use
...
GreenchClaw intentionally fails closed when `computerUse.enabled` is true. A
...
fresh GreenchClaw session.

**A Computer Use tool says `Native hook relay unavailable`.** The Codex-native
tool hook could not reach an active GreenchClaw relay through the local bridge or
Gateway fallback. Start a fresh GreenchClaw session with `/new` or `/reset`. If it
