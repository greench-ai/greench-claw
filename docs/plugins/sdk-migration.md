...
read_when:
  - You see the GREENCHCLAW_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the GREENCHCLAW_EXTENSION_API_DEPRECATED warning
  - You used api.registerEmbeddedExtensionFactory before GreenchClaw 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external GreenchClaw plugin
---

GreenchClaw has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`GreenchClaw/plugin-sdk/compat`** - a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`GreenchClaw/plugin-sdk/infra-runtime`** - a broad runtime helper barrel that
...
- **`GreenchClaw/plugin-sdk/config-runtime`** - a broad config compatibility barrel
  that still carries deprecated direct load/write helpers during the migration
  window.
- **`GreenchClaw/extension-api`** - a bridge that gave plugins direct access to
...
GreenchClaw does not remove or reinterpret documented plugin behavior in the same
...
The modern plugin SDK fixes this: each import path (`GreenchClaw/plugin-sdk/\<subpath\>`)
...
`GreenchClaw/plugin-sdk/realtime-voice`. The new controller owns the common Talk
...
`talk.providers` should be repaired with `GreenchClaw doctor --fix`; runtime Talk
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    `GreenchClaw/plugin-sdk/config-runtime` compatibility barrel. Use the narrow
    SDK subpath that matches the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `GreenchClawConfig` | `GreenchClaw/plugin-sdk/config-contracts` |
    | Already-loaded config assertions and plugin-entry config lookup | `GreenchClaw/plugin-sdk/plugin-config-runtime` |
    | Current runtime snapshot reads | `GreenchClaw/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `GreenchClaw/plugin-sdk/config-mutation` |
    | Session store helpers | `GreenchClaw/plugin-sdk/session-store-runtime` |
    | Markdown table config | `GreenchClaw/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `GreenchClaw/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `GreenchClaw/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `GreenchClaw/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `GreenchClaw/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `GreenchClaw/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "GreenchClaw/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "GreenchClaw/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "GreenchClaw/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "GreenchClaw/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "GreenchClaw/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedPiAgent } from "GreenchClaw/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `GreenchClaw/plugin-sdk/infra-runtime` still exists for external
    compatibility, but new code should import the focused helper surface it
    actually needs:

    | Need | Import |
    | --- | --- |
    | System event queue helpers | `GreenchClaw/plugin-sdk/system-event-runtime` |
    | Heartbeat wake, event, and visibility helpers | `GreenchClaw/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `GreenchClaw/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `GreenchClaw/plugin-sdk/channel-activity-runtime` |
    | In-memory dedupe caches | `GreenchClaw/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `GreenchClaw/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `GreenchClaw/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `GreenchClaw/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `GreenchClaw/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `GreenchClaw/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `GreenchClaw/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `GreenchClaw/plugin-sdk/error-runtime` |
    | Transport readiness waits | `GreenchClaw/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `GreenchClaw/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `GreenchClaw/plugin-sdk/concurrency-runtime` |
    | Numeric coercion | `GreenchClaw/plugin-sdk/number-runtime` |
    | Process-local async lock | `GreenchClaw/plugin-sdk/async-lock-runtime` |
    | File locks | `GreenchClaw/plugin-sdk/file-lock` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code should use `GreenchClaw/plugin-sdk/channel-route`.
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `GreenchClawSchema` |
  ...
  | `plugin-sdk/bundled-channel-config-schema` | Bundled config schemas | GreenchClaw-maintained bundled plugins only; new plugins must define plugin-local schemas |
...
`@GreenchClaw/discord@2026.3.13` package. Owner-specific helpers live inside the
...
<AccordionGroup>
  <Accordion title="command-auth help builders → command-status">
    **Old (`GreenchClaw/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`GreenchClaw/plugin-sdk/command-status`)**: same signatures, same
    exports - just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "GreenchClaw/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "GreenchClaw/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers → resolveInboundMentionDecision">
    **Old**: `resolveInboundMentionRequirement({ facts, policy })` and
    ...
    `GreenchClaw/plugin-sdk/channel-inbound` or
    `GreenchClaw/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `GreenchClaw/plugin-sdk/channel-runtime` is a compatibility shim for older
    channel plugins. Do not import it from new code; use
    `GreenchClaw/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    `channelActions*` helpers in `GreenchClaw/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Web search provider tool() helper → createTool() on the plugin">
    **Old**: `tool()` factory from `GreenchClaw/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    GreenchClaw no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. GreenchClaw downgrades stale stored values by profile
    rank automatically.
  ...
  <Accordion title="GreenchClawSchemaType alias → GreenchClawConfig">
    `GreenchClawSchemaType` re-exported from `GreenchClaw/plugin-sdk` is now a
    one-line alias for `GreenchClawConfig`. Prefer the canonical name.

    ```typescript
    // Before
    import type { GreenchClawSchemaType } from "GreenchClaw/plugin-sdk";
    // After
    import type { GreenchClawConfig } from "GreenchClaw/plugin-sdk/config-schema";
    ```
...
```bash
GREENCHCLAW_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 GreenchClaw gateway run
GREENCHCLAW_SUPPRESS_EXTENSION_API_WARNING=1 GreenchClaw gateway run
```
