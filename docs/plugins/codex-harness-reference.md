...
By default, GreenchClaw starts the managed Codex binary shipped with the bundled
plugin:
...
| `clearEnv`                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after GreenchClaw builds its inherited environment.                                                          |
| `requestTimeoutMs`            | `60000`                                                | Timeout for app-server control-plane calls.                                                                                                                                                     |
| `turnCompletionIdleTimeoutMs` | `60000`                                                | Quiet window after a turn-scoped app-server request while GreenchClaw waits for `turn/completed`.                                                                                               |
...
unattended GreenchClaw turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, GreenchClaw treats the implicit default as guardian
...
1. An explicit GreenchClaw Codex auth profile for the agent.
...
Stdio app-server launches inherit GreenchClaw's process environment by default, but
GreenchClaw owns the Codex app-server account bridge and sets both `CODEX_HOME` and
`HOME` to per-agent directories under that agent's GreenchClaw state. Codex's own
...
skills, plugins, config, accounts, and thread state scoped to the GreenchClaw agent
instead of leaking in from the operator's personal Codex CLI home.

GreenchClaw plugins and GreenchClaw skill snapshots still flow through GreenchClaw's own
plugin registry and skill loader. Personal Codex CLI assets do not. If you have
useful Codex CLI skills or plugins that should become part of an GreenchClaw agent,
...
`CODEX_HOME` and `HOME` remain reserved for GreenchClaw's per-agent Codex
...
Remaining GreenchClaw integration tools, such as messaging, sessions, media, cron,
browser, nodes, gateway, `heartbeat_respond`, and `web_search`, are available
through Codex tool search under the `GreenchClaw` namespace. This keeps the initial
...
Dynamic tool budgets are capped at 600000 ms. On timeout, GreenchClaw aborts the
...
availability is owned by Codex app-server, so the list can change when GreenchClaw
...
If discovery fails or times out, GreenchClaw uses a bundled fallback catalog for:
...
Codex handles `AGENTS.md` itself through native project-doc discovery. GreenchClaw
...
For GreenchClaw workspace parity, the Codex harness resolves the other bootstrap
