...
An **agent harness** is the low level executor for one prepared GreenchClaw agent
...
runtime and the normal GreenchClaw provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the GreenchClaw
  session transcript
...
Before a harness is selected, GreenchClaw has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the GreenchClaw transcript/session file
...
The prepared attempt also includes `params.runtimePlan`, an GreenchClaw-owned
...
**Import:** `GreenchClaw/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "GreenchClaw/plugin-sdk/agent-harness";
...
GreenchClaw chooses a harness after provider/model resolution:
...
3. `auto` asks registered harnesses if they support the resolved
   provider/model.
4. If no registered harness matches, GreenchClaw uses PI unless PI fallback is
   disabled.
...
provider/model. Once a plugin harness has claimed a run, GreenchClaw does not
...
`agents.list[].agentRuntime`, and `GREENCHCLAW_AGENT_RUNTIME`. `/status` shows the
...
GreenchClaw. The harness then claims that provider in `supports(...)`.
...
- app-server request: GreenchClaw sends the bare model id to Codex and lets the
...
GreenchClaw requires Codex app-server `0.125.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
GreenchClaw only runs against the protocol surface it has been tested with. The
`0.125.0` floor includes the native MCP hook payload support that landed in
Codex `0.124.0`, while pinning GreenchClaw to the newer tested stable line.
...
`GreenchClaw/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so GreenchClaw's fallback policy can decide whether to retry on a
...
The bundled `codex` harness is the native Codex mode for embedded GreenchClaw
...
`GreenchClaw doctor --fix`, and legacy `codex/*` model refs remain compatibility
aliases for the native harness.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. GreenchClaw still owns the chat channel,
...
By default, GreenchClaw uses `auto` provider/model runtime policy: registered
...
Keep that binding explicitly associated with the GreenchClaw session, and keep
mirroring user-visible assistant/tool output into the GreenchClaw transcript.

The GreenchClaw transcript remains the compatibility layer for:
...
If your harness stores a sidecar binding, implement `reset(...)` so GreenchClaw can
clear it when the owning GreenchClaw session is reset.

## Tool and media results

Core constructs the GreenchClaw tool list and passes it into the prepared attempt.
