...
when GreenchClaw loads an installed package and let npm packages avoid runtime
...
an installed package only declares a TypeScript source entry, GreenchClaw will use a
...
**Import:** `GreenchClaw/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                               | No       | -                   |
| `configSchema` | `GreenchClawPluginConfigSchema \| () => GreenchClawPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: GreenchClawPluginApi) => void`                                  | Yes      | -                   |

- `id` must match your `GreenchClaw.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- GreenchClaw resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `GreenchClaw/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: GreenchClawPluginApi) => void`                                  | No       | -                   |
| `registerFull`        | `(api: GreenchClawPluginApi) => void`                                  | No       | -                   |
...
- Discovery registration is non-activating, not import-free. GreenchClaw may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and GreenchClaw
...
- For plugin-owned root CLI commands, prefer `api.registerCli(..., { descriptors: [...] })`
  ...
  `api.registerNodeCliFeature(...)` so the command lands under `GreenchClaw nodes`.
  For other nested plugin commands, add `parentPath` and register commands on
  the `program` object passed to the registrar; GreenchClaw resolves it to the
...
GreenchClaw loads this instead of the full entry when a channel is disabled,
...
- `GreenchClaw/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
  import-safe setup patch adapters, lookup-note output,
  `promptResolvedAllowFrom`, `splitSetupEntries`, and delegated setup proxies
- `GreenchClaw/plugin-sdk/channel-setup` for optional-install setup surfaces
- `GreenchClaw/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`GreenchClaw/plugin-sdk/channel-entry-contract` instead. That contract lets the
...
the plugin entry and the channel plugin object so GreenchClaw can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want GreenchClaw to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; GreenchClaw rejects descriptor names outside
...
GreenchClaw classifies loaded plugins by their registration behavior:
...
Use `GreenchClaw plugins inspect <id>` to see a plugin's shape.
