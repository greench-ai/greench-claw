---
summary: "Run GreenchClaw on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for GreenchClaw
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the GreenchClaw Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install GreenchClaw on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep GreenchClaw bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/GreenchClaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `GREENCHCLAW_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for a stable startup path:
  ...
  - `NODE_COMPILE_CACHE=/var/tmp/GreenchClaw-compile-cache`
...
For the standard `GreenchClaw onboard --install-daemon` path, edit the user unit:
...
`GreenchClaw-gateway.service` via `sudo systemctl edit GreenchClaw-gateway.service`.
