...
## Config (`~/.GreenchClaw/GreenchClaw.json`)
...
- `consultThinkingLevel`: optional thinking level override for the full GreenchClaw agent run behind realtime `GreenchClaw_agent_consult` calls.
- `consultFastMode`: optional fast-mode override for realtime `GreenchClaw_agent_consult` calls.
...
- `realtime.instructions`: appends provider-facing system instructions to GreenchClaw's built-in realtime prompt. Use it for voice style and tone; GreenchClaw keeps the default `GreenchClaw_agent_consult` guidance.
...
- Browser realtime Talk uses `talk.client.toolCall` for `GreenchClaw_agent_consult` instead of exposing `chat.send` to provider-owned browser sessions.
...
- macOS local MLX playback uses the bundled `GreenchClaw-mlx-tts` helper when present, or an executable on `PATH`. Set `GREENCHCLAW_MLX_TTS_BIN` to point at a custom helper binary during development.
