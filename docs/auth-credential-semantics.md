...
Explicit copy flows, such as `GreenchClaw agents add`, use this portability policy:
...
has such a marker, `GreenchClaw doctor --fix` moves it to `auth.profiles` and
...
- Probe targets can come from auth profiles, environment credentials, or
  `models.json`.
- If a provider has credentials but GreenchClaw cannot resolve a probeable model
