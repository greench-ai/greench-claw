...
GreenchClaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.GreenchClaw/GreenchClaw.json`.
The active config path must be a regular file. Symlinked `GreenchClaw.json`
layouts are unsupported for GreenchClaw-owned writes; an atomic write may replace
the path instead of preserving the symlink. If you keep config outside the
default state directory, point `GREENCHCLAW_CONFIG_PATH` directly at the real file.

If the file is missing, GreenchClaw uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `GreenchClaw onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    GreenchClaw onboard       # full onboarding flow
    GreenchClaw configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.GreenchClaw/GreenchClaw.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
GreenchClaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

`GreenchClaw config schema` prints the canonical JSON Schema used by Control UI
...
- Only diagnostic commands work (`GreenchClaw doctor`, `GreenchClaw logs`, `GreenchClaw health`, `GreenchClaw status`)
- Run `GreenchClaw doctor` to see exact issues
- Run `GreenchClaw doctor --fix` (or `--yes`) to apply repairs

The Gateway keeps a trusted last-known-good copy after each successful startup,
but startup and hot reload do not restore it automatically. If `GreenchClaw.json`
...
Run `GreenchClaw doctor --fix` (or `--yes`) to repair prefixed/clobbered config or
...
<AccordionGroup>
  ...
  <Accordion title="Choose and configure models">
    ...
    - Use `GreenchClaw config set agents.defaults.models '<json>' --strict-json --merge` to add allowlist entries without removing existing models. Plain replacements that would remove entries are rejected unless you pass `--replace`.
  ...
  <Accordion title="Tune gateway WebSocket handshake timeout">
    ...
    - `GREENCHCLAW_HANDSHAKE_TIMEOUT_MS` still takes precedence for one-off service or shell overrides.
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `GreenchClaw.json`.
    ...
    ```bash
    GreenchClaw config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `GREENCHCLAW_APNS_RELAY_BASE_URL` and `GREENCHCLAW_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `GREENCHCLAW_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-GreenchClaw-token`); query-string tokens are rejected.
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **GreenchClaw-owned writes**: when a write changes only one top-level section
      backed by a single-file include such as `plugins: { $include: "./plugins.json5" }`,
      GreenchClaw updates that included file and leaves `GreenchClaw.json` intact
    - **Unsupported write-through**: root includes, include arrays, and includes
      with sibling overrides fail closed for GreenchClaw-owned writes instead of
      flattening the config
    - **Confinement**: `$include` paths must resolve under the directory holding
      `GreenchClaw.json`. To share a tree across machines or users, set
      `GREENCHCLAW_INCLUDE_ROOTS` to a path-list (`:` on POSIX, `;` on Windows) of
...
The Gateway watches `~/.GreenchClaw/GreenchClaw.json` and applies changes automatically - no manual restart needed for most settings.

Direct file edits are treated as untrusted until they validate. The watcher waits
for editor temp-write/rename churn to settle, reads the final file, and rejects
invalid external edits without rewriting `GreenchClaw.json`. GreenchClaw-owned config
...
config`, inspect the config, run `GreenchClaw config validate`, then run `GreenchClaw
...
When you edit a source file that is referenced through `$include`, GreenchClaw plans
...
```bash
GreenchClaw gateway call config.get --params '{}'  # capture payload.hash
GreenchClaw gateway call config.patch --params '{
...
GreenchClaw reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.GreenchClaw/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, GreenchClaw runs your login shell and imports only the missing keys:
...
Env var equivalent: `GREENCHCLAW_LOAD_SHELL_ENV=1`
</Accordion>
