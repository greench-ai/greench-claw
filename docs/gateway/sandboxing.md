---
summary: "How GreenchClaw sandboxing works: modes, scopes, workspace access, and images"
title: "Sandboxing"
...
GreenchClaw can run **tools inside sandbox backends** to reduce blast radius. This is **optional** and controlled by configuration (`agents.defaults.sandbox` or `agents.list[].sandbox`). If sandboxing is off, tools run on the host. The Gateway stays on the host; tool execution runs in an isolated sandbox when enabled.
...
<AccordionGroup>
  <Accordion title="Sandboxed browser details">
    ...
    - By default, sandbox browser containers use a dedicated Docker network (`GreenchClaw-sandbox-browser`) instead of the global `bridge` network. Configure with `agents.defaults.sandbox.browser.network`.
    ...
    - noVNC observer access is password-protected by default; GreenchClaw emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
Sandboxing is off by default. If you enable sandboxing and do not choose a backend, GreenchClaw uses the Docker backend. It executes tools and sandbox browsers locally via the Docker daemon socket (`/var/run/docker.sock`). Sandbox container isolation is determined by Docker namespaces.
...
If you deploy the GreenchClaw Gateway itself as a Docker container, it orchestrates sibling sandbox containers using the host's Docker socket (DooD). This introduces a specific path mapping constraint:

- **Config requires host paths**: The `GreenchClaw.json` `workspace` configuration MUST contain the **Host's absolute path** (e.g. `/home/user/.GreenchClaw/workspaces`), not the internal Gateway container path. When GreenchClaw asks the Docker daemon to spawn a sandbox, the daemon evaluates paths relative to the Host OS namespace, not the Gateway namespace.
- **FS bridge parity (identical volume map)**: The GreenchClaw Gateway native process also writes heartbeat and bridge files to the `workspace` directory. Because the Gateway evaluates the exact same string (the host path) from within its own containerized environment, the Gateway deployment MUST include an identical volume map linking the host namespace natively (`-v /home/user/.GreenchClaw:/home/user/.GreenchClaw`).
- **Codex code mode**: When an GreenchClaw sandbox is active, GreenchClaw constrains Codex app-server turns to Codex `workspace-write` sandboxing even if the Codex plugin default is `danger-full-access`. Do not mount the host Docker socket into agent sandbox containers or custom Codex sandboxes.

If you map paths internally without absolute host parity, GreenchClaw natively throws an `EACCES` permission error attempting to write its heartbeat inside the container environment because the fully qualified path string doesn't exist natively.
</Warning>

### SSH backend

Use `backend: "ssh"` when you want GreenchClaw to sandbox `exec`, file tools, and media reads on an arbitrary SSH-accessible machine.
...
<AccordionGroup>
  <Accordion title="How it works">
    - GreenchClaw creates a per-scope remote root under `sandbox.ssh.workspaceRoot`.
    - On first use after create or recreate, GreenchClaw seeds that remote workspace from the local workspace once.
    ...
    - GreenchClaw does not sync remote changes back to the local workspace automatically.
  ...
  <Accordion title="Authentication material">
    ...
    - `identityData`, `certificateData`, `knownHostsData`: use inline strings or SecretRefs. GreenchClaw resolves them through the normal secrets runtime snapshot, writes them to temp files with `0600`, and deletes them when the SSH session ends.
  ...
  <Accordion title="Remote-canonical consequences">
    ...
    - Host-local edits made outside GreenchClaw after the seed step are not visible remotely until you recreate the sandbox.
    - `GreenchClaw sandbox recreate` deletes the per-scope remote root and seeds again from local on next use.
...
Use `backend: "openshell"` when you want GreenchClaw to sandbox tools in an OpenShell-managed remote environment. For the full setup guide, configuration reference, and workspace mode comparison, see the dedicated [OpenShell page](/gateway/openshell).
...
- `mirror` (default): local workspace stays canonical. GreenchClaw syncs local files into OpenShell before exec and syncs the remote workspace back after exec.
- `remote`: OpenShell workspace is canonical after the sandbox is created. GreenchClaw seeds the remote workspace once from the local workspace, then file tools and exec run directly against the remote sandbox without syncing changes back.

<AccordionGroup>
  <Accordion title="Remote transport details">
    - GreenchClaw asks OpenShell for sandbox-specific SSH config via `openshell sandbox ssh-config <name>`.
...
<Tabs>
  <Tab title="mirror (local canonical)">
    ...
    - you edit files locally outside GreenchClaw and want those changes to show up in the sandbox automatically
  ...
  <Tab title="remote (OpenShell canonical)">
    ...
    - When the sandbox is first created, GreenchClaw seeds the remote workspace from the local workspace once.
    - After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate directly against the remote OpenShell workspace.
    - GreenchClaw does **not** sync remote changes back into the local workspace after exec.
    ...
    - If you edit files on the host outside GreenchClaw after the seed step, the remote sandbox will **not** see those changes automatically.
...
- `GreenchClaw sandbox list` shows OpenShell runtimes as well as Docker runtimes
- `GreenchClaw sandbox recreate` deletes the current runtime and lets GreenchClaw recreate it on next use
...
<Tabs>
  <Tab title="none (default)">
    Tools see a sandbox workspace under `~/.GreenchClaw/sandboxes`.
  </Tab>
  <Tab title="ro">
...
<Note>
**Skills note:** the `read` tool is sandbox-rooted. With `workspaceAccess: "none"`, GreenchClaw mirrors eligible skills into the sandbox workspace (`.../skills`) so they can be read. With `"rw"`, workspace skills are readable from `/workspace/skills`.
</Note>
...
- GreenchClaw blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
- GreenchClaw also blocks common home-directory credential roots such as `~/.aws`, `~/.cargo`, `~/.config`, `~/.docker`, `~/.gnupg`, `~/.netrc`, `~/.npm`, and `~/.ssh`.
- Bind validation is not just string matching. GreenchClaw normalizes the source path, then resolves it again through the deepest existing ancestor before re-checking blocked paths and allowed roots.
...
Default Docker image: `GreenchClaw-sandbox:bookworm-slim`

<Note>
**Source checkout vs npm install**

The `scripts/sandbox-setup.sh`, `scripts/sandbox-common-setup.sh`, and `scripts/sandbox-browser-setup.sh` helper scripts are only available when running from a [source checkout](https://github.com/GreenchClaw/GreenchClaw). They are not included in the npm package.

If you installed GreenchClaw via `npm install -g GreenchClaw`, use the inline `docker build` commands shown below instead.
</Note>
...
<Steps>
  <Step title="Build the default image">
    ...
    ```bash
    docker build -t GreenchClaw-sandbox:bookworm-slim - <<'DOCKERFILE'
    ...
    GreenchClaw does not silently substitute plain `debian:bookworm-slim` when `GreenchClaw-sandbox:bookworm-slim` is missing. Sandbox runs that target the default image fail fast with a build instruction until you build it, because the bundled image carries `python3` for sandbox write/edit helpers.
  ...
  <Step title="Optional: build the common image">
    ...
    From an npm install, build the default image first (see above), then build the common image on top using the [`scripts/docker/sandbox/Dockerfile.common`](https://github.com/GreenchClaw/GreenchClaw/blob/main/scripts/docker/sandbox/Dockerfile.common) from the repository.

    Then set `agents.defaults.sandbox.docker.image` to `GreenchClaw-sandbox-common:bookworm-slim`.
  ...
  <Step title="Optional: build the sandbox browser image">
    ...
    From an npm install, build using the [`scripts/docker/sandbox/Dockerfile.browser`](https://github.com/GreenchClaw/GreenchClaw/blob/main/scripts/docker/sandbox/Dockerfile.browser) from the repository.
...
<AccordionGroup>
  <Accordion title="Sandbox browser Chromium defaults">
    ...
    - `--remote-debugging-port=<derived from GREENCHCLAW_BROWSER_CDP_PORT>`
    ...
    - The three graphics hardening flags (`--disable-3d-apis`, `--disable-software-rasterizer`, `--disable-gpu`) are optional and are useful when containers lack GPU support. Set `GREENCHCLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if your workload requires WebGL or other 3D/browser features.
    - `--disable-extensions` is enabled by default and can be disabled with `GREENCHCLAW_BROWSER_DISABLE_EXTENSIONS=0` for extension-reliant flows.
    - `--renderer-process-limit=2` is controlled by `GREENCHCLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>`, where `0` keeps Chromium's default.
...
For Docker gateway deployments, `scripts/docker/setup.sh` can bootstrap sandbox config. Set `GREENCHCLAW_SANDBOX=1` (or `true`/`yes`/`on`) to enable that path. You can override socket location with `GREENCHCLAW_DOCKER_SOCKET`. Full setup and env reference: [Docker](/install/docker#agent-sandbox).
...
- Use `GreenchClaw sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
