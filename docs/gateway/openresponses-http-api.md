...
GreenchClaw's Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`GreenchClaw agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-GreenchClaw-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-GreenchClaw-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "GreenchClaw"`, `model: "GreenchClaw/default"`, `model: "GreenchClaw/<agentId>"`, or `x-GreenchClaw-agent-id`
- use `x-GreenchClaw-model` when you want to override the selected agent's backend model
- use `x-GreenchClaw-session-key` for explicit session routing
- use `x-GreenchClaw-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: GreenchClaw reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
GreenchClaw normalizes common OpenAI-style aliases before those counters reach
