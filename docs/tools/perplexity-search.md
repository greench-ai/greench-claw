...
GreenchClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, GreenchClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `GreenchClaw configure --section web`. It stores the key in
`~/.GreenchClaw/GreenchClaw.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.GreenchClaw/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
