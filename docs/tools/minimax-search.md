...
GreenchClaw supports MiniMax as a `web_search` provider through the MiniMax
...
GreenchClaw also accepts `MINIMAX_CODING_API_KEY`, `MINIMAX_OAUTH_TOKEN`, and
...
If `plugins.entries.minimax.config.webSearch.region` is unset, GreenchClaw resolves
...
| `count`   | integer | 1-10        | Number of results to return. GreenchClaw trims the returned list to this size. |
