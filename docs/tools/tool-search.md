...
read_when:
  ...
  - You want GreenchClaw tools, MCP tools, and client tools exposed through one compact PI surface
  - You are implementing or debugging tool discovery for PI runs
---

Tool Search is an experimental GreenchClaw PI-agent feature. It gives PI agents one
...
This page documents GreenchClaw PI Tool Search. It is not the Codex-native tool
...
`GreenchClaw.tools` bridge:

```js
const hits = await GreenchClaw.tools.search("create a GitHub issue");
const tool = await GreenchClaw.tools.describe(hits[0].id);
return await GreenchClaw.tools.call(tool.id, {
  title: "Crash on startup",
  body: "Steps to reproduce...",
});
```

The catalog can include GreenchClaw tools, plugin tools, MCP tools, and
...
needs the exact schema, and calls that tool through GreenchClaw.

Codex harness runs do not receive these experimental GreenchClaw Tool Search
controls. GreenchClaw passes product capabilities to Codex as dynamic tools, and
...
2. List eligible GreenchClaw and plugin tools.
...
At execution time every real tool call returns to GreenchClaw. The isolated Node
runtime does not hold plugin implementations, MCP client objects, or secrets.
`GreenchClaw.tools.call(...)` crosses the bridge back into the Gateway, where the
...
catalog includes eligible GreenchClaw, MCP, and client tools after normal policy
filtering.
...
`GreenchClaw.tools.search(query, options?)`

Searches the effective catalog for the current run. Results are compact and safe
to put back into prompt context.

```js
const hits = await GreenchClaw.tools.search("calendar event", { limit: 5 });
```

`GreenchClaw.tools.describe(id)`

Loads full metadata for one search result, including the exact input schema.

```js
const calendarCreate = await GreenchClaw.tools.describe("mcp:calendar:create_event");
```

`GreenchClaw.tools.call(id, args)`

Calls a selected tool through GreenchClaw.

```js
await GreenchClaw.tools.call(calendarCreate.id, {
  summary: "Planning",
...
network grants, and no child-process or worker grants. GreenchClaw enforces a
...
- `GreenchClaw.tools.search`
- `GreenchClaw.tools.describe`
- `GreenchClaw.tools.call`

Normal GreenchClaw behavior still applies to final calls:
...
```bash
GreenchClaw config set tools.toolSearch true
```
...
- final tool calls executed through GreenchClaw
...
- whether the result came from GreenchClaw, MCP, or a client tool
