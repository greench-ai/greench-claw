...
GreenchClaw supports Gemini models with built-in
...
For a gateway install, put env keys in `~/.GreenchClaw/.env`.
...
`pd`, `pw`, `pm`, and `py`. GreenchClaw converts these values, or an explicit
