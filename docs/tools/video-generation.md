...
GreenchClaw agents can generate videos from text prompts, reference images, or
...
GreenchClaw treats video generation as three runtime modes:
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ```bash
    GreenchClaw config set agents.defaults.videoGenerationModel.primary "google/veo-3.1-fast-generate-preview"
    ```
  </Step>
...
1. GreenchClaw submits the request to the provider and immediately returns a task id.
...
3. When the video is ready, GreenchClaw wakes the same session with an internal completion event.
4. The agent tells the user and attaches the finished video. In group/channel
   ...
   result through the message tool instead of GreenchClaw posting it directly.
...
generation. Use `GreenchClaw tasks list` or `GreenchClaw tasks show <taskId>` to
...
Generated video files are saved under GreenchClaw-managed media storage when
...
larger renders. When a provider also returns a hosted output URL, GreenchClaw
...
<ParamField path="aspectRatio" type="string">
  Aspect-ratio hint such as `1:1`, `16:9`, `9:16`, `adaptive`, or a provider-specific value. GreenchClaw normalizes or ignores unsupported values per provider.
</ParamField>
<ParamField path="resolution" type="string">Resolution hint such as `480P`, `720P`, `768P`, `1080P`, `4K`, or a provider-specific value. GreenchClaw normalizes or ignores unsupported values per provider.</ParamField>
...
<ParamField path="timeoutMs" type="number">Optional provider operation timeout in milliseconds. When omitted, GreenchClaw uses `agents.defaults.videoGenerationModel.timeoutMs` if configured.</ParamField>
...
<Note>
Not all providers support all parameters. GreenchClaw normalizes duration to
...
GreenchClaw resolves the model in this order:
...
<AccordionGroup>
  ...
  <Accordion title="BytePlus Seedance 1.5">
    Requires the [`@GreenchClaw/byteplus-modelark`](https://www.npmjs.com/package/@GreenchClaw/byteplus-modelark)
  ...
  <Accordion title="fal">
    Uses a queue-backed flow for long-running jobs. GreenchClaw waits up to 20
  ...
  <Accordion title="OpenRouter">
    Uses OpenRouter's asynchronous `/videos` API. GreenchClaw submits the
...
```bash
GREENCHCLAW_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts
```
...
- Per-provider operation cap from
  `GREENCHCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default).
...
Set `GREENCHCLAW_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared
...
Set the default video-generation model in your GreenchClaw config:
...
```bash
GreenchClaw config set agents.defaults.videoGenerationModel.primary "qwen/wan2.6-t2v"
```
