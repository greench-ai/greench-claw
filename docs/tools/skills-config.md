...
`~/.GreenchClaw/GreenchClaw.json`. Agent-specific skill visibility lives under
...
- Built-in skill roots always include `~/.GreenchClaw/skills`, `~/.agents/skills`,
...
- `install.nodeManager`: node installer preference (`npm` | `pnpm` | `yarn` | `bun`, default: npm).
  ...
  - `GreenchClaw setup --node-manager` is narrower and currently accepts `npm`,
...
GreenchClaw skips it and logs `Skipping escaped skill path outside its configured
root`.
...
- Keys under `entries` map to the skill name by default. If a skill defines
  `metadata.GreenchClaw.skillKey`, use that key instead.
- Load precedence is `<workspace>/skills` → `<workspace>/.agents/skills` →
  `~/.agents/skills` → `~/.GreenchClaw/skills` → bundled skills →
