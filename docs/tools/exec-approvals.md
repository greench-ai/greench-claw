...
host-local `ask: "always"` in `~/.GreenchClaw/exec-approvals.json` keeps
...
| ------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `GreenchClaw approvals get` / `--gateway` / `--node <id\|name\|ip>` | Requested policy, host policy sources, and the effective result.                       |
| `GreenchClaw exec-policy show`                                      | Local-machine merged view.                                                             |
| `GreenchClaw exec-policy set` / `preset`                            | Synchronize the local requested policy with the local host approvals file in one step. |
...
- **Gateway host** → `GreenchClaw` process on the gateway machine.
...
- For shell scripts and direct interpreter/runtime file invocations, GreenchClaw also tries to bind one concrete local file operand. If that bound file changes after approval but before execution, the run is denied instead of executing drifted content.
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.GreenchClaw/exec-approvals.sock",
...
<ParamField path="strictInlineEval" type="boolean">
  When `true`, GreenchClaw treats inline code-eval forms as approval-only
...
<ParamField path="commandHighlighting" type="boolean" default="false">
  ...
  GreenchClaw may attach parser-derived command spans so Web approval
...
**both** policy layers - requested exec policy in GreenchClaw config
...
- In YOLO mode, GreenchClaw does **not** add a separate heuristic command-obfuscation approval gate or script-preflight rejection layer on top of the configured host exec policy.
...
`--permission-mode bypassPermissions` when GreenchClaw's requested exec
...
<Steps>
  ...
  <Step title="Match the host approvals file">
    ```bash
    GreenchClaw approvals set --stdin <<'EOF'
    {
      version: 1,
      defaults: {
        security: "full",
...
```bash
GreenchClaw exec-policy preset yolo
```

That local shortcut updates both:

- Local `tools.exec.host/security/ask`.
- Local `~/.GreenchClaw/exec-approvals.json` defaults.

It is intentionally local-only. To change gateway-host or node-host
approvals remotely, use `GreenchClaw approvals set --gateway` or
`GreenchClaw approvals set --node <id|name|ip>`.
...
```bash
GreenchClaw approvals set --node <id|name|ip> --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `GreenchClaw exec-policy` does not synchronize node approvals.
- `GreenchClaw exec-policy set --host node` is rejected.
- Node exec approvals are fetched from the node at runtime, so node-targeted updates must use `GreenchClaw approvals --node ...`.
...
specific argument shape. GreenchClaw evaluates the regular expression
...
entries instead of hand-editing the encoded value. If GreenchClaw cannot
...
edit its local `~/.GreenchClaw/exec-approvals.json` directly.

CLI: `GreenchClaw approvals` supports gateway or node editing - see
...
When an async exec approval is denied, GreenchClaw prevents the agent from
