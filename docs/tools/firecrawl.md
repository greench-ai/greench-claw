...
GreenchClaw can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `GreenchClaw configure --section web` enables the bundled Firecrawl plugin automatically.
...
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `GreenchClaw doctor --fix`.
...
when you run Firecrawl yourself. GreenchClaw accepts `http://` only for loopback,
...
GreenchClaw always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, GreenchClaw
