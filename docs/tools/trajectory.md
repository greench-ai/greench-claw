---
summary: "Export redacted trajectory bundles for debugging an GreenchClaw agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an GreenchClaw session
...
Trajectory capture is GreenchClaw's per-session flight recorder. It records a
...
GreenchClaw writes the bundle under the workspace:

```text
.GreenchClaw/trajectory-exports/GreenchClaw-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.GreenchClaw/trajectory-exports/`. Absolute
...
create the bundle; do not use allow-all. In group chats, GreenchClaw sends the
...
Trajectory capture is on by default for GreenchClaw agent runs.
...
| `metadata.json`       | GreenchClaw version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata  |
...
GreenchClaw also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `GREENCHCLAW_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export GREENCHCLAW_TRAJECTORY_DIR=/var/lib/GreenchClaw/trajectories
```

When this variable is set, GreenchClaw writes one JSONL file per session id in that
directory.
...
Set `GREENCHCLAW_TRAJECTORY=0` before starting GreenchClaw:

```bash
export GREENCHCLAW_TRAJECTORY=0
```
...
GreenchClaw redacts sensitive values before writing export files:
...
- confirm GreenchClaw was started without `GREENCHCLAW_TRAJECTORY=0`
- check whether `GREENCHCLAW_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.GreenchClaw/trajectory-exports/`
