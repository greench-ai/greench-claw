---
summary: "Install GreenchClaw - installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs GreenchClaw, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://GreenchClaw.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want GreenchClaw and Node kept under a local prefix such as
`~/.GreenchClaw`, without depending on a system-wide Node install:

```bash
curl -fsSL https://GreenchClaw.ai/install-cli.sh | bash
```
...
`GreenchClaw update --channel dev` and `GreenchClaw update --channel stable`. See
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g GreenchClaw@latest
```
...
Or skip the link and use `pnpm GreenchClaw ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:GreenchClaw/GreenchClaw#main
```
...
```bash
GreenchClaw --version      # confirm the CLI is available
GreenchClaw doctor         # check for config issues
GreenchClaw gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `GreenchClaw onboard --install-daemon` or `GreenchClaw gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy GreenchClaw on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove GreenchClaw completely.
  </Card>
</CardGroup>

## Troubleshooting: `GreenchClaw` not found

If the install succeeded but `GreenchClaw` is not found in your terminal:
