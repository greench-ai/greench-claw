...
GreenchClaw ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/GreenchClaw`, override with
  `GREENCHCLAW_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
GreenchClaw update --tag 2026.4.1-beta.1
...
GreenchClaw update --tag GreenchClaw@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `GreenchClaw update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  GreenchClaw prompts for confirmation (skip with `--yes`).
...
Preview what `GreenchClaw update` would do without making changes:

```bash
GreenchClaw update --dry-run
GreenchClaw update --channel beta --dry-run
GreenchClaw update --tag 2026.4.1-beta.1 --dry-run
GreenchClaw update --dry-run --json
```
...
When you switch channels with `GreenchClaw update`, GreenchClaw also syncs plugin
sources:
