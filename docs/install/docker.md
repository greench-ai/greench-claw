---
summary: "Optional Docker-based setup and onboarding for GreenchClaw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run GreenchClaw on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export GREENCHCLAW_IMAGE="ghcr.io/GreenchClaw/GreenchClaw:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/GreenchClaw/GreenchClaw/pkgs/container/GreenchClaw).
  ...
  <Step title="Complete onboarding">
    ...
    `GreenchClaw-gateway` directly. `GreenchClaw-cli` is for commands you run after
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm GreenchClaw-cli channels login

    # Telegram
    docker compose run --rm GreenchClaw-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm GreenchClaw-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t GreenchClaw:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node GreenchClaw-gateway \
...
<Note>
Run `docker compose` from the repo root. If you enabled `GREENCHCLAW_EXTRA_MOUNTS`
or `GREENCHCLAW_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `GreenchClaw-cli` shares `GreenchClaw-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d GreenchClaw-gateway`, run onboarding
and setup-time config writes through `GreenchClaw-gateway` with
...
| Variable                                      | Purpose                                                         |
| --------------------------------------------- | --------------------------------------------------------------- |
| `GREENCHCLAW_IMAGE`                           | Use a remote image instead of building locally                  |
| `GREENCHCLAW_DOCKER_APT_PACKAGES`             | Install extra apt packages during build (space-separated)       |
| `GREENCHCLAW_EXTENSIONS`                      | Include selected bundled plugin helpers at build time           |
| `GREENCHCLAW_EXTRA_MOUNTS`                    | Extra host bind mounts (comma-separated `source:target[:opts]`) |
| `GREENCHCLAW_HOME_VOLUME`                     | Persist `/home/node` in a named Docker volume                   |
| `GREENCHCLAW_SANDBOX`                         | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)          |
| `GREENCHCLAW_SKIP_ONBOARDING`                 | Skip the interactive onboarding step (`1`, `true`, `yes`, `on`) |
| `GREENCHCLAW_DOCKER_SOCKET`                   | Override Docker socket path                                     |
| `GREENCHCLAW_DISABLE_BONJOUR`                 | Disable Bonjour/mDNS advertising (defaults to `1` for Docker)   |
| `GREENCHCLAW_DISABLE_BUNDLED_SOURCE_OVERLAYS` | Disable bundled plugin source bind-mount overlays               |
...
| `GREENCHCLAW_OTEL_PRELOADED`                  | Skip starting a second OpenTelemetry SDK when one is preloaded  |
...
`GREENCHCLAW_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`.
...
```bash
export GREENCHCLAW_EXTENSIONS="diagnostics-otel"
export OTEL_EXPORTER_OTLP_ENDPOINT="http://otel-collector:4318"
export OTEL_SERVICE_NAME="GreenchClaw-gateway"
./scripts/docker/setup.sh
```

Install the official `@GreenchClaw/diagnostics-otel` plugin from ClawHub in
packaged Docker installs before enabling export. Custom source-built images can
still include the local plugin source with
`GREENCHCLAW_EXTENSIONS=diagnostics-otel`. To enable export, allow and enable the
...
`clawhub:@GreenchClaw/diagnostics-prometheus`, enable the
...
```bash
docker compose exec GreenchClaw-gateway node dist/index.js health --token "$GREENCHCLAW_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `GREENCHCLAW_GATEWAY_BIND=lan` so host access to
...
When GreenchClaw runs in Docker, `127.0.0.1` inside the container is the container
...
`GREENCHCLAW_DISABLE_BONJOUR=1` so the Gateway does not crash-loop or repeatedly
...
Set `GREENCHCLAW_DISABLE_BONJOUR=0` only when running with host networking, macvlan,
...
Docker Compose bind-mounts `GREENCHCLAW_CONFIG_DIR` to `/home/node/.GreenchClaw`,
`GREENCHCLAW_WORKSPACE_DIR` to `/home/node/.GreenchClaw/workspace`, and
`GREENCHCLAW_AUTH_PROFILE_SECRET_DIR` to `/home/node/.config/GreenchClaw`, so those
...
That mounted config directory is where GreenchClaw keeps:

- `GreenchClaw.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `GREENCHCLAW_GATEWAY_TOKEN`
...
but separate from `GREENCHCLAW_CONFIG_DIR`.

Installed downloadable plugins store their package state under the mounted
GreenchClaw home, so plugin install records and package roots survive container
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export GREENCHCLAW_SANDBOX=1
    export GREENCHCLAW_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
    ...
    `workspace-write` while the GreenchClaw sandbox is active; do not mount the
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm GreenchClaw-cli gateway probe
    docker compose run -T --rm GreenchClaw-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `GreenchClaw-cli` uses `network_mode: "service:GreenchClaw-gateway"` so CLI
    ...
    `no-new-privileges` on both `GreenchClaw-gateway` and `GreenchClaw-cli`.
  </Accordion>

  <Accordion title="Docker Desktop DNS failures in GreenchClaw-cli">
    Some Docker Desktop setups fail DNS lookups from the shared-network
    `GreenchClaw-cli` sidecar after `NET_RAW` is dropped, which shows up as
    `EAI_AGAIN` during npm-backed commands such as `GreenchClaw plugins install`.
    ...
    docker compose -f docker-compose.yml -f docker-compose.cli-no-dropped-caps.local.yml run --rm GreenchClaw-cli plugins install <package>
    ```

    If you already created a long-running `GreenchClaw-cli` container, recreate it
  ...
  <Accordion title="Permissions and EACCES">
    ...
    `/home/node/.GreenchClaw`, make sure your host bind mounts are owned by uid 1000:
    ...
    `/path/to/GreenchClaw-config/npm` to `root:root` if you intentionally run
    GreenchClaw as root long term.
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export GREENCHCLAW_HOME_VOLUME="GreenchClaw_home"`
    2. **Bake system deps**: `export GREENCHCLAW_DOCKER_APT_PACKAGES="git curl jq"`
    3. **Bake Playwright Chromium**: `export GREENCHCLAW_INSTALL_BROWSER=1`
    4. **Or install Playwright browsers into a persisted volume**:
       ```bash
       docker compose run --rm GreenchClaw-cli \
         node /app/node_modules/playwright-core/cli.js install chromium
       ```
    5. **Persist browser downloads**: use `GREENCHCLAW_HOME_VOLUME` or
       `GREENCHCLAW_EXTRA_MOUNTS`. GreenchClaw auto-detects the Docker image's
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/GreenchClaw/GreenchClaw/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    GreenchClaw runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm GreenchClaw-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm GreenchClaw-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping GreenchClaw up to date
