...
export type GreenchClawTransport = {
  request<T = unknown>(
    method: string,
    params?: unknown,
...
export type ConnectableGreenchClawTransport = GreenchClawTransport & {
...
export type GreenchClawEventType =
  | "run.created"
  | "run.queued"
  | "run.started"
  | "run.completed"
...
export type GreenchClawEvent<TData = unknown> = {
  version: 1;
  id: string;
  ts: number;
