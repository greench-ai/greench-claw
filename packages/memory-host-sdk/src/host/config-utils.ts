...
function resolveRequiredHomeDir(
  ...
  const explicitHome = normalizeHomeValue(env.GREENCHCLAW_HOME);
...
export function resolveStateDir(
  ...
  if (env.GREENCHCLAW_TEST_FAST === "1" || fs.existsSync(nextDir)) {
    return nextDir;
  }
...
function resolveDefaultAgentWorkspaceDir(env: NodeJS.ProcessEnv = process.env): string {
  ...
  const profile = env.GREENCHCLAW_PROFILE?.trim();
  if (profile && normalizeLowercaseStringOrEmpty(profile) !== "default") {
    return path.join(home, ".GreenchClaw", `workspace-${profile}`);
  }
  return path.join(home, ".GreenchClaw", "workspace");
}

function listAgentEntries(cfg: GreenchClawConfig): AgentConfig[] {
...
function resolveAgentConfig(cfg: GreenchClawConfig, agentId: string): AgentConfig | undefined {
