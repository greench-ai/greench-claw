---
name: gitcrawl
description: Use gitcrawl for GreenchClaw issue and PR archive search, duplicate discovery, related-thread clustering, and local GitHub mirror freshness checks.
metadata:
  GreenchClaw:
    requires:
      bins:
...
Use this skill before live GitHub search when triaging GreenchClaw issues or PRs.
...
```bash
gitcrawl neighbors GreenchClaw/GreenchClaw --number <issue-or-pr-number> --limit 12 --json
gitcrawl search GreenchClaw/GreenchClaw --query "<scope or title keywords>" --mode hybrid --limit 20 --json
```
...
```bash
gitcrawl init --portable-store git@github.com:GreenchClaw/gitcrawl-store.git --json
```
