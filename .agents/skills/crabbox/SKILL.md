---
name: crabbox
description: Use Crabbox for GreenchClaw remote Linux validation. Default to Blacksmith Testbox; includes direct Blacksmith and owned AWS/Hetzner fallback notes when Crabbox fails.
---

# Crabbox

Use Crabbox when GreenchClaw needs remote Linux proof for broad tests, CI-parity
...
- GreenchClaw scripts prefer `../crabbox/bin/crabbox` when present. The user PATH
  shim can be stale.
...
- Do not treat inherited shell env as operator intent. In particular,
  `GREENCHCLAW_LOCAL_CHECK_MODE=throttled` from the local shell is not permission
  to move broad `pnpm check:changed`, `pnpm test:changed`, full `pnpm test`, or
  lint/typecheck fan-out onto the laptop.
- Only use `GREENCHCLAW_LOCAL_CHECK_MODE=throttled|full` when the user explicitly
...
Use these only when the task needs an existing non-Linux host. GreenchClaw broad
...
pnpm crabbox:run -- --provider blacksmith-testbox \
  ...
  --timing-json \
  --shell -- \
  "env CI=1 NODE_OPTIONS=--max-old-space-size=4096 GREENCHCLAW_TEST_PROJECTS_PARALLEL=6 GREENCHCLAW_VITEST_MAX_WORKERS=1 GREENCHCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS=900000 pnpm test:changed"
```
...
- Prefer `GREENCHCLAW_CURRENT_PACKAGE_TGZ` with Docker/package lanes when testing a
...
- Prefer deterministic arrow navigation over search typing for Clack-style
  ...
  sequences.
- Isolate mutable state with `GREENCHCLAW_STATE_DIR=$(mktemp -d)`. Plugin npm
  installs live under that state dir (`npm/node_modules/...`), not under
  `GREENCHCLAW_CONFIG_DIR`. Verify downloads by checking the state dir, package
  lock, and installed package metadata.
- To test automatic setup installs against local package artifacts, use
  `GREENCHCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1` plus
  `GREENCHCLAW_PLUGIN_INSTALL_OVERRIDES='{"plugin-id":"npm-pack:/tmp/plugin.tgz"}'`.
  Pack with `npm pack`, set an isolated `GREENCHCLAW_STATE_DIR`, and verify the
...
- For OpenAI/Codex onboarding proof, the useful markers are the UI line
  `Installed Codex plugin`, `npm/node_modules/@GreenchClaw/codex`, and the
...
- Testbox queued/capacity pressure: do not convert a broad changed gate or full
  suite into local `GREENCHCLAW_LOCAL_CHECK_MODE=throttled pnpm ...`. Leave the
...
pnpm crabbox:run -- --provider blacksmith-testbox --debug --timing-json -- \
  CI=1 NODE_OPTIONS=--max-old-space-size=4096 GREENCHCLAW_TEST_PROJECTS_PARALLEL=6 GREENCHCLAW_VITEST_MAX_WORKERS=1 GREENCHCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS=900000 pnpm test:changed
```
...
```sh
pnpm crabbox:run -- --provider blacksmith-testbox --debug --timing-json -- \
  CI=1 NODE_OPTIONS=--max-old-space-size=4096 GREENCHCLAW_TEST_PROJECTS_PARALLEL=6 GREENCHCLAW_VITEST_MAX_WORKERS=1 GREENCHCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS=900000 pnpm test
```

Auth fallback, only when `blacksmith` says auth is missing:

```sh
blacksmith auth login --non-interactive --organization GreenchClaw
```
...
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json -- \
  CI=1 NODE_OPTIONS=--max-old-space-size=4096 GREENCHCLAW_TEST_PROJECTS_PARALLEL=6 GREENCHCLAW_VITEST_MAX_WORKERS=1 pnpm test:changed
```
...
pnpm crabbox:run -- --id <cbx_id-or-slug> --timing-json --shell -- "env NODE_OPTIONS=--max-old-space-size=4096 GREENCHCLAW_TEST_PROJECTS_PARALLEL=6 GREENCHCLAW_VITEST_MAX_WORKERS=1 GREENCHCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS=900000 pnpm test:changed"
...
```sh
brew install GreenchClaw/tap/crabbox
crabbox login --url https://crabbox.GreenchClaw.ai --provider aws
```
...
- If broker auth is missing, run `crabbox login --url https://crabbox.GreenchClaw.ai --provider aws`.
- If the CLI asks for `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, or AWS
  profile setup during normal GreenchClaw validation, assume the agent selected
...
- Ask for AWS keys only for explicit direct-provider/account administration,
  not for normal brokered GreenchClaw proof.
- Trusted automation may still use
  `printf '%s' "$CRABBOX_COORDINATOR_TOKEN" | crabbox login --url https://crabbox.GreenchClaw.ai --provider aws --token-stdin`.
...
for owned-cloud lanes. Do not let that config override the GreenchClaw default
...
Do not add GreenchClaw-specific setup to Crabbox itself. Put repo setup in the
