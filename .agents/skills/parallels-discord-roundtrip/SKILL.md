...
```bash
export GREENCHCLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.GreenchClaw/GreenchClaw.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env GREENCHCLAW_PARALLELS_DISCORD_TOKEN \
...
- Use the `GreenchClaw` wrapper for guest `message send/read`; `node GreenchClaw.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/GreenchClaw-parallels-smoke.*`
...
- host inbound nonce appears in `GreenchClaw message read` output
