...
description: Triage GreenchClaw security advisories, drafts, and GHSA reports with shipped-tag and trust-model proof.
---

# Security Triage

Use when reviewing GreenchClaw security advisories, drafts, or GHSA reports.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/GreenchClaw/GreenchClaw/security-advisories/<GHSA>`.
3. Inspect the exact implicated code paths.
4. Verify shipped state:
   - `git tag --sort=-creatordate | head`
   - `npm view GreenchClaw version --userconfig "$(mktemp)"`
...
```bash
gh api /repos/GreenchClaw/GreenchClaw/security-advisories/<GHSA>
gh api /repos/GreenchClaw/GreenchClaw/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view GreenchClaw version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo GreenchClaw/GreenchClaw --match title,body,comments -- "<terms>"
gh search prs --repo GreenchClaw/GreenchClaw --match title,body,comments -- "<terms>"
```
