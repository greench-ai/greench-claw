...
vi.mock("../infra/tmp-GreenchClaw-dir.js", () => ({
  resolvePreferredGreenchClawTmpDir: () => "/tmp/GreenchClaw-browser-test",
}));
...
import {
  ...
  isChromeCdpReady,
  isChromeReachable,
  launchGreenchClawChrome,
  resolveGreenchClawUserDataDir,
 ...
 * not exercise: launchGreenchClawChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopGreenchClawChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildGreenchClawChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveGreenchClawUserDataDir", () => {
    it("falls back to the default profile name when none is supplied", () => {
      const dir = resolveGreenchClawUserDataDir();
      expect(dir.endsWith(path.join("GreenchClaw", "user-data"))).toBe(true);
    });

    it("respects an explicit profile name", () => {
      const dir = resolveGreenchClawUserDataDir("my-profile");
      expect(dir.endsWith(path.join("my-profile", "user-data"))).toBe(true);
    });
  });

  describe("buildGreenchClawChromeLaunchArgs branches", () => {
    ...
    it("lets a request headless override beat env and profile headed settings", () => {
      const args = buildGreenchClawChromeLaunchArgs({
        ...
        env: { GREENCHCLAW_BROWSER_HEADLESS: "0" },
      });
      expect(args).toContain("--headless=new");
      expect(args).toContain("--disable-gpu");
    });
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchGreenchClawChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchGreenchClawChrome(resolved, profile);
  ...
  describe("launchGreenchClawChrome", () => {
    let tmpDir = "";

    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-launch-"));
    });
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchGreenchClawChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchGreenchClawChrome(makeResolved(), profile);
          expect(running.pid).toBe(4242);
    ...
    it("clears stale singleton locks and retries once after profile-in-use launch failure", async () => {
      ...
      try {
        const running = await launchGreenchClawChrome(
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchGreenchClawChrome(resolved, profile)).rejects.toThrow(
  ...
  describe("stopGreenchClawChrome SIGKILL fallback", () => {
    it("escalates to SIGKILL when CDP keeps reporting reachable past the deadline", async () => {
      ...
      await stopGreenchClawChrome(
        { proc, cdpPort: 12345 } as unknown as Parameters<typeof stopGreenchClawChrome>[0],
        1,
      );
  ...
  describe("launchGreenchClawChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-decorated-"));
      try {
        const profileName = path.basename(stageDir);
        const colorHex = "#FF4500";
        const colorInt = ((0xff << 24) | 0xff4500) >> 0;
        const userDataDir = path.join(resolveGreenchClawUserDataDir(profileName));
        ...
        const staged = resolveGreenchClawUserDataDir(path.basename(stageDir));
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_GREENCHCLAW_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      await expect(launchGreenchClawChrome(resolved, profile)).rejects.toThrow(/Chrome stderr:/);
    });
    ...
    it("omits the sandbox hint on non-linux platforms", async () => {
      ...
      try {
        ...
        let caught: unknown;
        try {
          await launchGreenchClawChrome(resolved, profile);
        } catch (e) {
          caught = e;
        }
    ...
    it("logs a warning when decorateGreenchClawProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateGreenchClawProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
      ...
      const decorationSpy = vi
        .spyOn({ decorateGreenchClawProfile }, "decorateGreenchClawProfile")
        .mockImplementation(() => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".GreenchClaw-profile-decorated") || s.endsWith("Preferences")) {
