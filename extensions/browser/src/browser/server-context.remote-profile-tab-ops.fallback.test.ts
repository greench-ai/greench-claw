...
describe("browser remote profile fallback and attachOnly behavior", () => {
  it("uses profile-level attachOnly when global attachOnly is false", async () => {
    const state = deps.makeState("GreenchClaw");
    ...
    const launchMock = vi.mocked(deps.chromeModule.launchGreenchClawChrome);
  ...
  it("does not enforce managed tab cap for remote GreenchClaw profiles", async () => {
  ...
  it("uses remote-class tab-open timeouts for attachOnly loopback CDP profiles", async () => {
    ...
    const opened = await ctx.forProfile("GreenchClaw").openTab("https://example.com");
  ...
  it("keeps managed loopback tab opens on local CDP defaults", async () => {
    ...
    await ctx.forProfile("GreenchClaw").openTab("http://127.0.0.1:3000");
