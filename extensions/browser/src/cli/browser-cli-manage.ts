...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "GreenchClaw";
...
async function runBrowserDoctor(parent: BrowserParentOpts, profile?: string, deep?: boolean) {
  ...
  checks.push({
    name: "profile",
    ok: true,
    detail: `${status.profile ?? "GreenchClaw"} (${usesChromeMcpTransport(status) ? "chrome-mcp" : (status.transport ?? "cdp")})`,
  });
  checks.push({
    name: "browser",
    ok: status.running,
    detail: status.running
      ...
      : "not running; run `GreenchClaw browser start`",
  });
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "GreenchClaw"}`,
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "GreenchClaw" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option(
      "--driver <driver>",
      "Profile driver (GreenchClaw|existing-session). Default: GreenchClaw",
    )
    .action(
      async (
        opts: {
