...
import { runFfmpeg } from "GreenchClaw/plugin-sdk/media-runtime";
import {
...
describe("transcribeSenseAudioAudio", () => {
  ...
  it("can transcribe generated speech in live mode", async () => {
    if (process.env.GREENCHCLAW_LIVE_TEST !== "1" || !process.env.SENSEAUDIO_API_KEY) {
      return;
    }
    ...
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-senseaudio-live-"));
    try {
