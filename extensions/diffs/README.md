...
Read-only diff viewer plugin for **GreenchClaw** agents.
...
Before:
GreenchClaw supports plugins.

After:
GreenchClaw supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/GreenchClaw-diffs`).
