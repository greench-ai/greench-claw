...
describe("renderDiffDocument", () => {
  ...
  it("renders before/after input into a complete viewer document", async () => {
    ...
    expect(rendered.html).toContain("data-GreenchClaw-diff-root");
  ...
  it("resolves viewer assets under an optional base path", async () => {
    ...
    expect(
      new URL(loaderSrc ?? "", "https://example.com/GreenchClaw/plugins/diffs/view/id/token")
        .pathname,
    ).toBe("/GreenchClaw/plugins/diffs/assets/viewer.js");
  });
  ...
  it("downgrades invalid language hints to plain text", async () => {
    ...
    const payloads = [...html.matchAll(/data-GreenchClaw-diff-payload>(.*?)<\/script>/g)].map(
