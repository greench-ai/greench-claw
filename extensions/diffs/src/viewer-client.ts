...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-GreenchClaw-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-GreenchClaw-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.GreenchClawDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.GreenchClawDiffsError = "true";
