...
import type { JsonSchemaObject } from "GreenchClaw/plugin-sdk/config-schema";
...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    expect(
      resolveDiffsPluginViewerBaseUrl({
        viewerBaseUrl: "https://example.com/GreenchClaw/",
      }),
    ).toBe("https://example.com/GreenchClaw");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/GreenchClaw?x=1" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/GreenchClaw#frag" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/GreenchClaw/" })).toBe(true);
  });
  ...
  it("preserves defaults and security for direct safeParse callers", () => {
    ...
    expect(data.viewerBaseUrl).toBe("https://example.com/GreenchClaw");
...
describe("diffs viewer URL helpers", () => {
  ...
  it("joins viewer path under baseUrl pathname", () => {
    expect(
      buildViewerUrl({
        config: {},
        baseUrl: "https://example.com/GreenchClaw",
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("https://example.com/GreenchClaw/plugins/diffs/view/id/token");
  });
...
describe("viewer assets", () => {
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("GreenchClawDiffsReady");
