import { createProviderUsageFetch, makeResponse } from "GreenchClaw/plugin-sdk/test-env";
...
vi.mock("GreenchClaw/plugin-sdk/json-store", () => ({
  loadJsonFile: jsonStoreMocks.loadJsonFile,
  saveJsonFile: jsonStoreMocks.saveJsonFile,
}));

vi.mock("GreenchClaw/plugin-sdk/state-paths", () => ({
  resolveStateDir: () => "/tmp/GreenchClaw-state",
}));

import type { ProviderResolveDynamicModelContext } from "GreenchClaw/plugin-sdk/core";
...
describe("github-copilot token", () => {
  const cachePath = "/tmp/GreenchClaw-state/credentials/github-copilot.token.json";
