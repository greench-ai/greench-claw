...
vi.mock("GreenchClaw/plugin-sdk/channel-feedback", () => ({
  DEFAULT_TIMING: {
    doneHoldMs: 0,
    errorHoldMs: 0,
  },
...
vi.mock("GreenchClaw/plugin-sdk/channel-streaming", () => ({
...
vi.mock("GreenchClaw/plugin-sdk/outbound-runtime", () => ({
...
vi.mock("GreenchClaw/plugin-sdk/reply-payload", () => ({
...
vi.mock("GreenchClaw/plugin-sdk/security-runtime", () => ({
...
vi.mock("../config.runtime.js", () => ({
  resolveStorePath: () => "/tmp/GreenchClaw-store.json",
...
describe("dispatchPreparedSlackMessage preview fallback", () => {
  ...
  it("updates non-main DM last-route metadata on the prepared thread session", async () => {
    ...
    expect(updateLastRouteMock).toHaveBeenCalledWith({
      storePath: "/tmp/GreenchClaw-store.json",
      sessionKey: "agent:main:slack:direct:u1:thread:500.000",
      deliveryContext: {
        channel: "slack",
        to: "user:U1",
