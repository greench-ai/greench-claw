...
vi.mock("GreenchClaw/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: GreenchClaw", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "GreenchClaw" });
  });

  it("runTavilyExtract sends X-Client-Source: GreenchClaw", async () => {
