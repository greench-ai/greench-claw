...
type Registered = {
  ...
  tools: unknown[];
  service?: Parameters<GreenchClawPluginApi["registerService"]>[0];
};
...
describe("voice-call plugin", () => {
  ...
  afterEach(() => {
    ...
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("GreenchClaw.voice-call.runtime")
    ];
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("GreenchClaw.voice-call.runtimePromise")
    ];
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("GreenchClaw.voice-call.runtimeStopPromise")
    ];
  });
  ...
  it("still starts the webhook runtime for gateway CLI processes", async () => {
    ...
    vi.stubEnv("GREENCHCLAW_CLI", "1");
    process.argv = ["node", "GreenchClaw", "gateway", "run"];
    const { service } = setup({ provider: "mock" });
  ...
  it("normalizes legacy config through runtime creation and warns to run doctor", async () => {
    ...
    expectWarningIncludes('Run "GreenchClaw doctor --fix"');
  });
  ...
  it("CLI smoke can place a live notify call with --yes", async () => {
    ...
    try {
      ...
      expect(runtimeStub.manager.initiateCall).toHaveBeenCalledWith("+15550009999", undefined, {
        message: "GreenchClaw voice call smoke test.",
        mode: "notify",
      });
