...
describe("openai tts", () => {
  ...
  describe("openaiTTS diagnostics", () => {
    it("adds GreenchClaw attribution headers to native OpenAI speech requests", async () => {
      ...
      expect(headers?.originator).toBe("GreenchClaw");
      expect(headers?.version).toBe("2026.3.22");
      expect(headers?.["User-Agent"]).toBe("GreenchClaw/2026.3.22");
    });
    ...
    it("records TTS exchanges in debug proxy capture mode", async () => {
      ...
      process.env.GREENCHCLAW_DEBUG_PROXY_SESSION_ID = "tts-session";
    ...
    it("does not double-capture TTS exchanges when the global fetch patch is installed", async () => {
      ...
      process.env.GREENCHCLAW_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
      process.env.GREENCHCLAW_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
      process.env.GREENCHCLAW_DEBUG_PROXY_SESSION_ID = "tts-patched-session";
