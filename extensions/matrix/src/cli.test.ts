...
describe("matrix CLI verification commands", () => {
  ...
  it("prints recovery-key and identity-trust diagnostics for device verification failures", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Recovery key can unlock the room-key backup, but full Matrix identity trust is still incomplete. Run GreenchClaw matrix verify self, accept the request in another verified Matrix client, and confirm the SAS only if it matches.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
  ...
  it("requests Matrix self-verification and prints the follow-up SAS commands", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Then run GreenchClaw matrix verify start --account ops -- txn-1 to start SAS verification.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Run GreenchClaw matrix verify sas --account ops -- txn-1 to display the SAS emoji or decimals.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- When the SAS matches, run GreenchClaw matrix verify confirm-sas --account ops -- txn-1.",
    );
  });
  ...
  it("prints DM lookup details in Matrix verification follow-up commands", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Then run GreenchClaw matrix verify start --user-id @alice:example.org --room-id '!room-'\\''$(x):example.org' -- txn-dm to start SAS verification.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Run GreenchClaw matrix verify sas --user-id @alice:example.org --room-id '!room-'\\''$(x):example.org' -- txn-dm to display the SAS emoji or decimals.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- When the SAS matches, run GreenchClaw matrix verify confirm-sas --user-id @alice:example.org --room-id '!room-'\\''$(x):example.org' -- txn-dm.",
    );
  });
  ...
  it("terminates options before remote Matrix verification ids in follow-up commands", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Then run GreenchClaw matrix verify start --account ops -- --account=evil to start SAS verification.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Run GreenchClaw matrix verify sas --account ops -- --account=evil to display the SAS emoji or decimals.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- When the SAS matches, run GreenchClaw matrix verify confirm-sas --account ops -- --account=evil.",
    );
  });
  ...
  it("lists Matrix verification requests", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith("Initiated by GreenchClaw: no");
  });
  ...
  it("shell-quotes Matrix verification ids in follow-up command guidance", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Then run GreenchClaw matrix verify start -- 'txn-'\\''$(touch /tmp/pwn)' to start SAS verification.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Run GreenchClaw matrix verify sas -- 'txn-'\\''$(touch /tmp/pwn)' to display the SAS emoji or decimals.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- When the SAS matches, run GreenchClaw matrix verify confirm-sas -- 'txn-'\\''$(touch /tmp/pwn)'.",
    );
  });
  ...
  it("shows Matrix SAS diagnostics and confirm/mismatch guidance", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- If they match, run GreenchClaw matrix verify confirm-sas -- self-1.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- If they do not match, run GreenchClaw matrix verify mismatch-sas -- self-1.",
    );
  });
  ...
  it("passes DM lookup details through Matrix verification follow-up commands", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- If they match, run GreenchClaw matrix verify confirm-sas --user-id @alice:example.org --room-id '!dm:example.org' --account ops -- txn-dm.",
    );
  });
  ...
  it("prints stable transaction ids in follow-up commands after accepting verification", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Run GreenchClaw matrix verify start -- txn-stable to start SAS verification.",
    );
  });
  ...
  it("lists matrix devices", async () => {
    listMatrixOwnDevicesMock.mockResolvedValue([
      {
        ...
        displayName: "GreenchClaw\u001B[2J Gateway",
    ...
    expect(console.log).toHaveBeenCalledWith("- A7hWrQ70ea (current, GreenchClaw Gateway)");
    expect(console.log).toHaveBeenCalledWith("  Last IP: 127.0.0.1");
    expect(console.log).toHaveBeenCalledWith("- BritdXC6iL (GreenchClaw Gateway)");
  });
  ...
  it("prunes stale matrix gateway devices", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith("Deleted stale GreenchClaw devices: BritdXC6iL");
  ...
  it("adds a matrix account and prints a binding hint", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "Bind this account to an agent: GreenchClaw agents bind --agent <id> --bind matrix:ops",
    );
  });
  ...
  it("bootstraps verification for newly added encrypted accounts", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "Matrix device hygiene warning: stale GreenchClaw devices detected (BritdXC6iL). Run GreenchClaw matrix devices prune-stale --account ops.",
    );
  });
  ...
  it("uses --name as fallback account id and prints account-scoped config path", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "Bind this account to an agent: GreenchClaw agents bind --agent <id> --bind matrix:main-bot",
    );
  });
  ...
  it("shows explicit backup issue in default status output", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "- Backup key is not loaded on this device. Run GreenchClaw matrix verify backup restore to load it and restore old room keys. If restore still cannot load the key, run the shown printf pipeline with the Matrix recovery key env var for this account: printf '%s\\n' \"$MATRIX_RECOVERY_KEY\" | GreenchClaw matrix verify backup restore --recovery-key-stdin.",
    );
    expect(console.log).not.toHaveBeenCalledWith(
      "- Backup is present but not trusted for this device. Re-run 'GreenchClaw matrix verify device <key>'.",
    );
  });
  ...
  it("fails status with re-login guidance when the current Matrix device is missing on the server", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "- This Matrix device is no longer listed on the homeserver. Create a new GreenchClaw Matrix device with GreenchClaw matrix account add --homeserver '<url>' --user-id '<@user:server>' --password '<password>' --device-name GreenchClaw-Gateway. If you use token auth, create a fresh Matrix access token in your Matrix client or admin UI, then run GreenchClaw matrix account add --homeserver '<url>' --access-token '<token>'.",
    );
  });
  ...
  it("includes backup reset guidance when the backup key does not match this device", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "- If you want a fresh backup baseline and accept losing unrecoverable history, run GreenchClaw matrix verify backup reset --yes. Add --rotate-recovery-key only when the old recovery key should stop unlocking the fresh backup.",
    );
  });
  ...
  it("requires --yes before resetting the Matrix room-key backup", async () => {
    ...
    expect(console.error).toHaveBeenCalledWith(
      "Backup reset failed: Refusing to reset Matrix room-key backup without --yes. If you accept losing unrecoverable history, re-run GreenchClaw matrix verify backup reset --yes.",
    );
  });
  ...
  it("prints resolved account-aware guidance when a named Matrix account is selected implicitly", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "- Run the shown printf pipeline with the Matrix recovery key env var for this account: printf '%s\\n' \"$MATRIX_RECOVERY_KEY_ASSISTANT\" | GreenchClaw matrix verify device --recovery-key-stdin --account assistant. If you do not have the recovery key but still have another verified Matrix client, run GreenchClaw matrix verify self --account assistant instead.",
    );
    expect(console.log).toHaveBeenCalledWith(
      "- Run GreenchClaw matrix verify bootstrap --account assistant to create a room key backup.",
    );
  });
