...
import { createReplyToFanout } from "GreenchClaw/plugin-sdk/outbound-runtime";
import { resolvePayloadMediaUrls } from "GreenchClaw/plugin-sdk/reply-payload";
...
const MATRIX_GREENCHCLAW_PRESENTATION_KEY = "com.GreenchClaw.presentation" as const;
const MATRIX_GREENCHCLAW_PRESENTATION_TYPE = "message.presentation" as const;
...
function buildMatrixPresentationContent(presentation: MessagePresentation) {
  return {
    ...presentation,
    version: 1,
    type: MATRIX_GREENCHCLAW_PRESENTATION_TYPE,
  };
}
...
function resolveMatrixPresentationContent(
  ...
  const presentation = toRecord(extraContent?.[MATRIX_GREENCHCLAW_PRESENTATION_KEY]);
  if (
    !presentation ||
    presentation.version !== 1 ||
    presentation.type !== MATRIX_GREENCHCLAW_PRESENTATION_TYPE
  ) {
    return undefined;
  }
  return presentation;
  ...
  return {
    ...
    channelData: {
      ...
      matrix: {
        ...matrixData,
        extraContent: {
          [MATRIX_GREENCHCLAW_PRESENTATION_KEY]: buildMatrixPresentationContent(
            params.presentation,
          ),
        },
      },
...
function resolveMatrixExtraContent(payload: ReplyPayload): MatrixExtraContentFields | undefined {
  ...
  return presentation ? { [MATRIX_GREENCHCLAW_PRESENTATION_KEY]: presentation } : undefined;
}
