...
function baseCfg(): GreenchClawConfig {
  return {
    channels: {
      imessage: {
        dmPolicy: "open",
...
describe("imessage monitor gating + envelope builders", () => {
  ...
  it("blocks group messages when imessage.groups is set without a wildcard", async () => {
    ...
    const decision = await resolveIMessageInboundDecision({
      ...
      message: {
        ...
        sender: "+15550001111",
        is_from_me: false,
        text: "@GreenchClaw hello",
        is_group: true,
      },
      opts: {},
      messageText: "@GreenchClaw hello",
      bodyText: "@GreenchClaw hello",
      allowFrom: ["*"],
      groupAllowFrom: [],
      groupPolicy: "open",
      dmPolicy: "open",
  ...
  it("honors group allowlist and ignores pairing-store senders in groups", async () => {
    ...
    const denied = await resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@GreenchClaw hi",
      bodyText: "@GreenchClaw hi",
      allowFrom: ["*"],
    ...
    const allowed = await resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@GreenchClaw ok",
      bodyText: "@GreenchClaw ok",
      allowFrom: ["*"],
      groupAllowFrom: ["chat_id:101"],
      groupPolicy: "allowlist",
      dmPolicy: "pairing",
