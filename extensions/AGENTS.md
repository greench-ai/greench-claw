...
- Extension production code should import from `GreenchClaw/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `GreenchClaw.plugin.json` and the package
  `GreenchClaw` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `GreenchClaw/plugin-sdk/*`.
