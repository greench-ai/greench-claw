...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-test-web-logout-"));
    previousOAuthDir = process.env.GREENCHCLAW_OAUTH_DIR;
    process.env.GREENCHCLAW_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  afterAll(async () => {
    if (previousOAuthDir === undefined) {
      ...
      process.env.GREENCHCLAW_OAUTH_DIR = previousOAuthDir;
    }
  ...
  it("does not delete through symlinked auth dirs inside the GreenchClaw auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the GreenchClaw auth root", async () => {
