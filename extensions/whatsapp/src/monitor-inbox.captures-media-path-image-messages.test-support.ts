...
vi.mock("GreenchClaw/plugin-sdk/logging-core", async () => {
  const actual = await vi.importActual<typeof import("GreenchClaw/plugin-sdk/logging-core")>(
    "GreenchClaw/plugin-sdk/logging-core",
  );
  return {
    ...actual,
    getChildLogger: () => ({
...
describe("web monitor inbox", () => {
  ...
  it("detaches inbound listeners and ends the socket on close()", async () => {
    ...
    expect(closeError?.message).toBe("GreenchClaw WhatsApp listener close");
