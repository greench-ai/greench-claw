...
import { redactIdentifier } from "GreenchClaw/plugin-sdk/logging-core";
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS } from "GreenchClaw/plugin-sdk/media-runtime";
...
let resetLogger: typeof import("GreenchClaw/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("GreenchClaw/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: GreenchClawConfig = {
  channels: { whatsapp: {} },
};
...
vi.mock("GreenchClaw/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("GreenchClaw/plugin-sdk/media-runtime")>(
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("GreenchClaw/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `GreenchClaw-outbound-${crypto.randomUUID()}.log`);
