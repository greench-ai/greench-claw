...
The ACPX extension is a thin GreenchClaw wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `GreenchClaw/acpx`, not in this extension.
...
Use this flow when GreenchClaw needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `GreenchClaw/acpx` repo first.
2. In GreenchClaw, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild GreenchClaw and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for GreenchClaw ACP validation.
...
If a change feels like shared ACP runtime behavior instead of GreenchClaw-specific glue, move it to `GreenchClaw/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
