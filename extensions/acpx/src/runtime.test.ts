...
const DOCUMENTED_GREENCHCLAW_BRIDGE_COMMAND =
  "env GREENCHCLAW_HIDE_BANNER=1 GREENCHCLAW_SUPPRESS_NOTES=1 GreenchClaw acp --url ws://127.0.0.1:18789 --token-file ~/.GreenchClaw/gateway.token --session agent:main:main";
const CODEX_ACP_COMMAND = "npx @zed-industries/codex-acp@0.13.0";
const CODEX_ACP_WRAPPER_COMMAND = `node "/tmp/GreenchClaw/acpx/codex-acp-wrapper.mjs"`;
const CODEX_ACP_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_ACP_WRAPPER_COMMAND} ${GREENCHCLAW_ACPX_LEASE_ID_ARG} lease-close ${GREENCHCLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
  ...
  const runtime = new AcpxRuntime(
    {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "GreenchClaw" ? "GreenchClaw acp" : agentName,
        list: () => ["codex", "GreenchClaw"],
      },
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("normalizes GreenchClaw Codex model ids for ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("injects Codex ACP startup config into the scoped registry", () => {
    ...
    expect(__testing.isCodexAcpCommand("GreenchClaw acp")).toBe(false);
  });
  ...
  it("cleans up GreenchClaw-owned ACPX process trees after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/GreenchClaw/acpx/codex-acp-wrapper.mjs"',
        pid: 900,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate } = makeRuntime(
      baseStore,
      {
        GreenchClawWrapperRoot: "/tmp/GreenchClaw/acpx",
      },
      {
        GreenchClawProcessCleanup: {
          listProcesses: vi.fn(async () => [
            {
              pid: 900,
              ppid: 1,
              command: 'node "/tmp/GreenchClaw/acpx/codex-acp-wrapper.mjs"',
            },
            {
              pid: 901,
              ppid: 900,
  ...
  it("records ACPX process leases without persisting lease-specific agent commands", async () => {
    ...
    const { runtime, delegate, wrappedStore } = makeRuntime(baseStore, {
      GreenchClawGatewayInstanceId: "gateway-test",
      GreenchClawProcessLeaseStore: leaseStore.store,
      GreenchClawWrapperRoot: "/tmp/GreenchClaw/acpx",
      agentRegistry: {
        resolve: (agentName: string) =>
    ...
    expect(lease?.wrapperPath).toBe("/tmp/GreenchClaw/acpx/codex-acp-wrapper.mjs");
    expect(launchCommands[0]).toContain("GREENCHCLAW_ACPX_LEASE_ID=");
    expect(launchCommands[0]).toContain("GREENCHCLAW_GATEWAY_INSTANCE_ID=gateway-test");
    expect(savedRecords[0]?.agentCommand).toBe(CODEX_ACP_WRAPPER_COMMAND);
    expect(savedRecords[0]?.GreenchClawGatewayInstanceId).toBe("gateway-test");
    expect(savedRecords[0]?.GreenchClawLeaseId).toBe(lease?.leaseId);
  });
  ...
  it("merges sidecar lease ids into loaded ACPX session records", async () => {
    ...
    leaseStore.leases.set("lease-loaded", {
      ...
      sessionKey: "agent:codex:acp:binding:test",
      wrapperRoot: "/tmp/GreenchClaw/acpx",
      wrapperPath: "/tmp/GreenchClaw/acpx/codex-acp-wrapper.mjs",
      rootPid: 777,
      commandHash: "hash",
      startedAt: 1,
      state: "open",
    ...
    expect(loadedRecord?.GreenchClawGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.GreenchClawLeaseId).toBe("lease-loaded");
  });
  ...
  it("merges the lease for the current ACPX session process when old leases exist", async () => {
    ...
    const loadedRecord = await wrappedStore.load("agent:codex:acp:binding:test");
    expect(loadedRecord?.GreenchClawGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.GreenchClawLeaseId).toBe("lease-current");
  });
  ...
  it("uses matching leases before legacy pid cleanup on close", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/GreenchClaw/acpx/codex-acp-wrapper.mjs"',
        GreenchClawLeaseId: "lease-close",
        pid: 930,
      })),
      save: vi.fn(async () => {}),
    };
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 930,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${GREENCHCLAW_ACPX_LEASE_ID_ARG} lease-old ${GREENCHCLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
            {
              pid: 940,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${GREENCHCLAW_ACPX_LEASE_ID_ARG} lease-current ${GREENCHCLAW_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
  ...
  it("routes GreenchClaw ensureSession through the bridge-safe delegate when MCP servers are configured", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const bridgeEnsure = vi.spyOn(bridgeSafeDelegate, "ensureSession").mockResolvedValue({
      sessionKey: "agent:GreenchClaw:acp:test",
      backend: "acpx",
  ...
  it("routes non-GreenchClaw sessions through the default delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes handle-based follow-up calls for GreenchClaw sessions through the bridge-safe delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const handle: Parameters<NonNullable<AcpRuntime["getStatus"]>>[0]["handle"] = {
      sessionKey: "agent:GreenchClaw:acp:test",
      backend: "acpx",
      runtimeSessionName: "GreenchClaw-session-handle",
    };
  ...
  it("keeps MCP-enabled routing when the GreenchClaw agent is overridden to a non-bridge adapter", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "GreenchClaw" ? "codex" : agentName),
        list: () => ["codex", "GreenchClaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for any agent mapped to the GreenchClaw bridge command", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "codex" ? "GreenchClaw acp" : agentName),
        list: () => ["codex", "GreenchClaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for documented env-wrapped GreenchClaw bridge commands", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "GreenchClaw" ? DOCUMENTED_GREENCHCLAW_BRIDGE_COMMAND : agentName,
        list: () => ["codex", "GreenchClaw"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for local node GreenchClaw entrypoints", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "GreenchClaw"
            ? "env GREENCHCLAW_HIDE_BANNER=1 node GreenchClaw.mjs acp"
            : agentName,
        list: () => ["codex", "GreenchClaw"],
      },
    });
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:GreenchClaw:acp:test",
        agentCommand: DOCUMENTED_GREENCHCLAW_BRIDGE_COMMAND,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const status = await runtime.getStatus({
      handle: {
        sessionKey: "agent:GreenchClaw:acp:test",
        backend: "acpx",
        runtimeSessionName: "agent:GreenchClaw:acp:test",
      },
    });
  ...
  it("probes through the bridge-safe delegate when probeAgent resolves to GreenchClaw bridge", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
