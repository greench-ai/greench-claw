...
import { isTruthyEnvValue, type RuntimeEnv } from "GreenchClaw/plugin-sdk/runtime-env";
import { resolveStateDir } from "GreenchClaw/plugin-sdk/state-paths";
import {
  lowercasePreservingWhitespace,
  normalizeOptionalString,
} from "GreenchClaw/plugin-sdk/string-coerce-runtime";
import { ensureDir, resolveUserPath } from "GreenchClaw/plugin-sdk/text-utility-runtime";
...
<title>GreenchClaw Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.GreenchClawCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.GreenchClawCanvasA2UIAction &&
        typeof window.GreenchClawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.GreenchClawSendUserAction === "function";
  ...
  window.addEventListener("GreenchClaw:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android GreenchClaw node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.GreenchClawSendUserAction === "function"
        ? window.GreenchClawSendUserAction
        : undefined;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>GreenchClaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
