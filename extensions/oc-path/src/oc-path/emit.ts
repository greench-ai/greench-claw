 ...
 * `__GREENCHCLAW_REDACTED__` literal anywhere in the output throws
 ...
 *
 * @module @GreenchClaw/oc-path/emit
 */
...
export function emitMd(ast: MdAst, opts: EmitOptions = {}): string {
  ...
  if (mode === "roundtrip") {
    ...
    if (!acceptPreExisting && ast.raw.includes("__GREENCHCLAW_REDACTED__")) {
      guardSentinel("__GREENCHCLAW_REDACTED__", `${guardPath}/[raw]`);
    }
    return ast.raw;
  }
