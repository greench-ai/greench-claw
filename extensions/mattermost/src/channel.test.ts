...
vi.mock("GreenchClaw/plugin-sdk/ssrf-runtime", async () => {
  const original = (await vi.importActual("GreenchClaw/plugin-sdk/ssrf-runtime")) as Record<
    string,
    unknown
  >;
...
function getDescribedActions(cfg: GreenchClawConfig, accountId?: string): string[] {
