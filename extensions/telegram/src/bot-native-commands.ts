...
import { resolveChannelStreamingBlockEnabled } from "GreenchClaw/plugin-sdk/channel-streaming";
...
import { getRuntimeConfigSnapshot } from "GreenchClaw/plugin-sdk/runtime-config-snapshot";
...
type TelegramChunkMode = ReturnType<
  typeof import("GreenchClaw/plugin-sdk/reply-dispatch-runtime").resolveChunkMode
>;
type TelegramNativeReplyPayload =
  import("GreenchClaw/plugin-sdk/reply-dispatch-runtime").ReplyPayload;
...
export const registerTelegramNativeCommands = ({
  ...
  const loadFreshRuntimeConfig = (): GreenchClawConfig => telegramDeps.getRuntimeConfig();
  const resolveFreshTelegramConfig = (runtimeCfg: GreenchClawConfig): TelegramAccountConfig => {
    try {
