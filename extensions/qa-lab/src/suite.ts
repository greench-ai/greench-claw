...
import { disposeRegisteredAgentHarnesses } from "GreenchClaw/plugin-sdk/agent-harness";
...
function shouldLogQaSuiteProgress(env: NodeJS.ProcessEnv = process.env) {
  const override = parseQaSuiteBooleanEnv(env.GREENCHCLAW_QA_SUITE_PROGRESS);
...
function resolveQaSuiteTransportReadyTimeoutMs(
  ...
  const raw = env.GREENCHCLAW_QA_TRANSPORT_READY_TIMEOUT_MS;
  if (!raw) {
    return 120_000;
  }
...
async function runScenario(name: string, steps: QaSuiteStep[]): Promise<QaSuiteScenarioResult> {
  ...
  for (const step of steps) {
    try {
      if (process.env.GREENCHCLAW_QA_DEBUG === "1") {
...
async function writeQaSuiteArtifacts(params: {
  ...
  const report = renderQaMarkdownReport({
    title: "GreenchClaw QA Scenario Suite",
...
export async function runQaSuite(params?: QaSuiteRunParams): Promise<QaSuiteResult> {
  ...
  const gateway = await startQaGatewayChild({
    ...
    mutateConfig: gatewayConfigPatch
      ? (cfg) => applyQaMergePatch(cfg, gatewayConfigPatch) as GreenchClawConfig
      : undefined,
  });
  writeQaSuiteProgress(
    progressEnabled,
  ...
  try {
    ...
    const keepTemp = process.env.GREENCHCLAW_QA_KEEP_TEMP === "1" || false;
    await gateway.stop({
      keepTemp,
