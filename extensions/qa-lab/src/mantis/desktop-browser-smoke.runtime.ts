...
const DEFAULT_BROWSER_URL = "https://GreenchClaw.ai";
...
const BROWSER_PROFILE_ARCHIVE_ENV = "GREENCHCLAW_MANTIS_BROWSER_PROFILE_TGZ_B64";
const BROWSER_PROFILE_DIR_ENV = "GREENCHCLAW_MANTIS_BROWSER_PROFILE_DIR";
...
if [ -n "$profile_archive_b64" ]; then
  profile_archive="$profile/GreenchClaw-mantis-browser-profile.tgz"
...
export async function runMantisDesktopBrowserSmoke(
  ...
  const browserProfileArchiveEnv =
    ...
    trimToValue(env.GREENCHCLAW_MANTIS_BROWSER_PROFILE_ARCHIVE_ENV) ??
  ...
  const remoteOutputDir = `/tmp/GreenchClaw-mantis-desktop-${startedAt
    .toISOString()
