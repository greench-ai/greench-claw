  ...
  it("supports advanced QA memory and subagent recovery prompts", async () => {
    ...
    const threadMemorySearch = await fetch(`${server.baseUrl}/v1/responses`, {
      ...
      body: JSON.stringify({
        stream: true,
        instructions:
          "@GreenchClaw Thread memory check: what is the hidden thread codename stored only in memory? Use memory tools first and reply only in this thread.",
        input: [
          {
            role: "user",
            content: [
      ...
      body: JSON.stringify({
        ...
        input: [
          ...
          },
          {
            type: "function_call_output",
            output:
              "QA mission: Understand this GreenchClaw repo from source + docs before acting.",
          },
        ],
      }),
    });
  ...
  it("scripts a reasoning-only recovery sequence after a replay-safe read", async () => {
    ...
    const reasoningPayload = await expectResponsesJson<{
      ...
      input: [
        ...
        {
          type: "function_call_output",
          output: "QA mission: Understand this GreenchClaw repo from source + docs before acting.",
        },
      ],
    });
