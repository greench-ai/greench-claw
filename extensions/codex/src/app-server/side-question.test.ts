...
vi.mock("GreenchClaw/plugin-sdk/agent-harness", () => ({
  createGreenchClawCodingTools: (...args: unknown[]) => createGreenchClawCodingToolsMock(...args),
}));
...
describe("runCodexAppServerSideQuestion", () => {
  ...
  it("forks an ephemeral side thread and returns the completed assistant text", async () => {
    ...
    const [toolOptions] = mockCall(createGreenchClawCodingToolsMock);
  ...
  it("bridges side-thread dynamic tool requests to GreenchClaw tools", async () => {
    const client = createFakeClient();
  ...
  it("cleans up notification handlers when side tool setup fails", async () => {
    const client = createFakeClient();
    createGreenchClawCodingToolsMock.mockImplementation(() => {
