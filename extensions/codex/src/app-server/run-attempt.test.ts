...
import {
  CODEX_GREENCHCLAW_DYNAMIC_TOOL_NAMESPACE,
...
function threadStartResult(threadId = "thread-1") {
  return {
    thread: {
      ...
      status: { type: "idle" },
      path: null,
      cwd: tempDir || "/tmp/GreenchClaw-codex-test",
      cliVersion: "0.125.0",
      source: "unknown",
      agentNickname: null,
      agentRole: null,
...
describe("runCodexAppServerAttempt", () => {
  beforeEach(async () => {
    resetAgentEventsForTest();
    vi.stubEnv("GREENCHCLAW_TRAJECTORY", "0");
    vi.stubEnv("CODEX_API_KEY", "");
    vi.stubEnv("OPENAI_API_KEY", "");
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-codex-run-"));
  });

  afterEach(async () => {
    __testing.resetCodexAppServerClientFactoryForTests();
    __testing.resetGreenchClawCodingToolsFactoryForTests();
  ...
  it("starts Codex threads without duplicate GreenchClaw workspace tools by default", async () => {
  ...
  it("does not expose GreenchClaw Tool Search controls through Codex dynamic tools", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
  ...
  it("starts Codex threads with searchable GreenchClaw dynamic tools by default", async () => {
    __testing.setGreenchClawCodingToolsFactoryForTests(() => [
    ...
    expect(webSearch?.namespace).toBe(CODEX_GREENCHCLAW_DYNAMIC_TOOL_NAMESPACE);
    expect(webSearch?.deferLoading).toBe(true);
    expect(heartbeat?.namespace).toBe(CODEX_GREENCHCLAW_DYNAMIC_TOOL_NAMESPACE);
  ...
  it("keeps searchable Codex dynamic tools canonical in mirrored transcript snapshots", async () => {
    ...
    const toolResult = (await harness.handleServerRequest({
      ...
      params: {
        ...
        namespace: CODEX_GREENCHCLAW_DYNAMIC_TOOL_NAMESPACE,
        tool: "wiki_status",
  ...
  it("passes the live run session key to Codex dynamic tools when sandbox policy uses another key", () => {
    ...
    expect(
      __testing.resolveGreenchClawCodingToolsSessionKeys(
        params,
    ...
    expect(__testing.resolveGreenchClawCodingToolsSessionKeys(params, "agent:main:main")).toEqual({
  ...
  it("returns a failed dynamic tool response when an app-server tool call exceeds the deadline", async () => {
    ...
    await expect(response).resolves.toEqual({
      ...
      contentItems: [
        {
          type: "inputText",
          text: "GreenchClaw dynamic tool call timed out after 1ms while running tool message.",
        },
      ],
    });
  ...
  it("logs process poll timeout context separately from session idle", async () => {
    ...
    await expect(response).resolves.toEqual({
      ...
      contentItems: [
        {
          type: "inputText",
          text: "GreenchClaw dynamic tool call timed out after 1ms while waiting for process action=poll sessionId=rapid-crustacean. This is a tool RPC timeout, not a session idle timeout.",
        },
      ],
    });
  ...
  it("emits normalized tool progress around app-server dynamic tool requests", async () => {
    ...
    expect(toolResult.contentItems?.[0]?.text).toMatch(
      /^(Unknown GreenchClaw tool: message|Action send requires a target\.)$/u,
    );
  ...
  it("projects mirrored history when starting Codex without a native thread binding", async () => {
    ...
    const harness = createStartedThreadHarness();
    const params = createParams(sessionFile, workspaceDir);
    params.prompt = "make the default webpage GreenchClaw";
    ...
    expect(inputText).toContain("GreenchClaw assembled context for this turn:");
    ...
    expect(inputText).toContain("make the default webpage GreenchClaw");
  });

  it("passes GreenchClaw bootstrap files through Codex developer instructions", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
  ...
  it("releases completion when a projector callback throws during turn/completed", async () => {
    // Regression for GreenchClaw/GreenchClaw#67996: a throw inside the projector's
  ...
  it("clamps Codex danger-full-access when GreenchClaw sandboxing is active", () => {
    const appServer = resolveCodexAppServerRuntimeOptions({
      pluginConfig: {
        appServer: {
          approvalPolicy: "never",
    ...
    const sandboxed = __testing.restrictCodexAppServerSandboxForGreenchClawSandbox(appServer, {
      enabled: true,
    } as never);
    ...
    expect(__testing.restrictCodexAppServerSandboxForGreenchClawSandbox(appServer, null)).toBe(
      appServer,
    );
    expect(
      __testing.restrictCodexAppServerSandboxForGreenchClawSandbox(
  ...
  it("builds resume and turn params from the currently selected GreenchClaw model", () => {
  ...
  it("uses turn-scoped collaboration instructions for heartbeat Codex turns", () => {
    ...
    expect(heartbeatCollaborationMode.settings.developer_instructions).toContain(
      "This is an GreenchClaw heartbeat turn. Apply these instructions only to this heartbeat wake",
    );
    expect(heartbeatCollaborationMode.settings.developer_instructions).toContain(
