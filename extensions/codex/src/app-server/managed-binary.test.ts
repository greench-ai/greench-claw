...
describe("managed Codex app-server binary", () => {
  ...
  it("resolves the plugin-local bundled Codex binary", async () => {
    const pluginRoot = path.join("/tmp", "GreenchClaw", "extensions", "codex");
  ...
  it("resolves Windows Codex command shims", () => {
    const pluginRoot = path.win32.join("C:\\", "GreenchClaw", "dist", "extensions", "codex");
  ...
  it("uses the package root when the resolver is bundled into a dist chunk", () => {
    expect(__testing.resolveDefaultCodexPluginRoot("/repo/GreenchClaw/dist")).toBe(
      "/repo/GreenchClaw",
    );
    expect(__testing.resolveDefaultCodexPluginRoot("/repo/GreenchClaw/dist-runtime")).toBe(
      "/repo/GreenchClaw",
    );
    expect(
      __testing.resolveDefaultCodexPluginRoot("/repo/GreenchClaw/extensions/codex/src/app-server"),
    ).toBe("/repo/GreenchClaw/extensions/codex");
  });

  it("finds Codex in the package install root used by packaged plugins", async () => {
    const installRoot = path.join("/tmp", "GreenchClaw-plugin-package", "codex");
  ...
  it("falls back to the resolved Codex package bin when no command shim exists", async () => {
    const installRoot = await mkdtemp(path.join(os.tmpdir(), "GreenchClaw-codex-package-"));
    const pluginRoot = path.join(installRoot, "dist", "extensions", "codex");
  ...
  it("fails clearly when the managed Codex binary is missing", async () => {
    await expect(
      resolveManagedCodexAppServerStartOptions(startOptions("managed"), {
        platform: "darwin",
        pluginRoot: path.join("/tmp", "GreenchClaw", "extensions", "codex"),
