import type { AnyAgentTool } from "GreenchClaw/plugin-sdk/agent-harness";
import { wrapToolWithBeforeToolCallHook } from "GreenchClaw/plugin-sdk/agent-harness-runtime";
import {
  installCodexToolResultMiddleware,
  installGreenchClawOwnedToolHooks,
  mediaToolResult,
  resetGreenchClawOwnedToolHooks,
  textToolResult,
...
describe("GreenchClaw-owned tool runtime contract — Codex app-server adapter", () => {
  afterEach(() => {
    resetGreenchClawOwnedToolHooks();
  });
  ...
  it("wraps unwrapped dynamic tools with before/after tool hooks", async () => {
    ...
    const hooks = installGreenchClawOwnedToolHooks({ adjustedParams });
  ...
  it("fails closed when before_tool_call blocks a dynamic tool", async () => {
    const hooks = installGreenchClawOwnedToolHooks({ blockReason: "blocked by policy" });
  ...
  it("records successful Codex messaging text, media, and target telemetry", async () => {
    const hooks = installGreenchClawOwnedToolHooks();
