...
import * as memoryCoreHostRuntimeCoreModule from "GreenchClaw/plugin-sdk/memory-core-host-runtime-core";
import * as runtimeConfigSnapshotModule from "GreenchClaw/plugin-sdk/runtime-config-snapshot";
import * as sessionStoreRuntimeModule from "GreenchClaw/plugin-sdk/session-store-runtime";
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("GreenchClaw:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("GreenchClaw-dreaming-backfill-");
    ...
    expect(content.match(/GreenchClaw:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("GreenchClaw-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- GreenchClaw:dreaming:diary:start -->");
    expect(content).toContain("<!-- GreenchClaw:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- GreenchClaw:dreaming:diary:start -->");
    const end = content.indexOf("<!-- GreenchClaw:dreaming:diary:end -->");
  ...
  it("prepends diary before existing managed blocks", async () => {
    ...
    await fs.writeFile(
      dreamsPath,
      "## Light Sleep\n<!-- GreenchClaw:dreaming:light:start -->\n- Candidate: test\n<!-- GreenchClaw:dreaming:light:end -->\n",
      "utf-8",
    );
    await appendNarrativeEntry({
      workspaceDir,
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("GreenchClaw-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("GreenchClaw-dreaming-narrative-");
    const stateDir = await createTempWorkspace("GreenchClaw-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("GreenchClaw-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("GreenchClaw-dreaming-narrative-");
...
describe("runDetachedDreamNarrative", () => {
  ...
  it("caps the number of in-flight detached narratives at 3", async () => {
    ...
    const workspaceDir = await createTempWorkspace("GreenchClaw-dreaming-detach-");
    const logger = createMockLogger();
