...
describe("memory index", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-mem-fixtures-"));
  ...
  beforeEach(async () => {
    ...
    vi.stubEnv("GREENCHCLAW_TEST_MEMORY_UNSAFE_REINDEX", "1");
    clearRegistry();
    registerBuiltInMemoryEmbeddingProviders({ registerMemoryEmbeddingProvider: registerAdapter });
    embedBatchCalls = 0;
  ...
  async function getFtsSessionManager(params: {
    ...
    vi.stubEnv("GREENCHCLAW_STATE_DIR", path.join(workspaceDir, params.stateDirName));
    const cfg = createCfg({
  ...
  it("streams embedding cache rows during safe reindex", async () => {
    vi.stubEnv("GREENCHCLAW_TEST_MEMORY_UNSAFE_REINDEX", "0");
