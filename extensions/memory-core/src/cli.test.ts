...
vi.mock("./cli.host.runtime.js", async () => {
  const [runtimeCli, runtimeCore, runtimeFiles] = await Promise.all([
    import("GreenchClaw/plugin-sdk/memory-core-host-runtime-cli"),
    import("GreenchClaw/plugin-sdk/memory-core-host-runtime-core"),
    import("GreenchClaw/plugin-sdk/memory-core-host-runtime-files"),
  ]);
  return {
...
let defaultRuntime: typeof import("GreenchClaw/plugin-sdk/memory-core-host-runtime-cli").defaultRuntime;
let isVerbose: typeof import("GreenchClaw/plugin-sdk/memory-core-host-runtime-cli").isVerbose;
let setVerbose: typeof import("GreenchClaw/plugin-sdk/memory-core-host-runtime-cli").setVerbose;
let fixtureRoot = "";
...
beforeAll(async () => {
  ...
  ({ defaultRuntime, isVerbose, setVerbose } =
    await import("GreenchClaw/plugin-sdk/memory-core-host-runtime-cli"));
...
describe("memory cli", () => {
  ...
  it("documents memory help examples", () => {
    ...
    expect(helpText).toContain("GreenchClaw memory status --fix");
    expect(helpText).toContain("Repair stale recall locks and normalize promotion metadata.");
    expect(helpText).toContain("GreenchClaw memory status --deep");
    expect(helpText).toContain("Probe embedding provider readiness.");
    expect(helpText).toContain('GreenchClaw memory search "meeting notes"');
    expect(helpText).toContain("Quick search using positional query.");
    expect(helpText).toContain('GreenchClaw memory search --query "deployment" --max-results 20');
    expect(helpText).toContain("Limit results for focused troubleshooting.");
    expect(helpText).toContain("GreenchClaw memory promote --apply");
    expect(helpText).toContain("Append top-ranked short-term candidates into MEMORY.md.");
    expect(helpText).toContain('GreenchClaw memory promote-explain "router vlan"');
    expect(helpText).toContain("Explain why a specific candidate would or would not promote.");
    expect(helpText).toContain("GreenchClaw memory rem-harness --json");
  ...
  it("shows the fix hint only before --fix has been run", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      const log = spyRuntimeLogs(defaultRuntime);
      await runMemoryCli(["status"]);
      expectLogged(log, "Fix: GreenchClaw memory status --fix --agent main");
      ...
      await runMemoryCli(["status", "--fix"]);
      expectNotLogged(log, "Fix: GreenchClaw memory status --fix --agent main");
    });
  });
  ...
  it("writes grounded rem backfill entries into DREAMS.md", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      expect(dreams).toContain("GreenchClaw:dreaming:backfill-entry");
  ...
  it("prefers persistence-relevant evidence over narrated operational logs in grounded what happened", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await fs.writeFile(
        historyPath,
        [
          "## GreenchClaw / runtime / workflow preferences and corrections",
    ...
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await fs.writeFile(
        ...
        [
          "# Dream Diary",
          "",
          "<!-- GreenchClaw:dreaming:diary:start -->",
          "---",
          "",
          ...
          "*January 1, 2025*",
          "",
          "<!-- GreenchClaw:dreaming:backfill-entry day=2025-01-01 source=memory/2025-01-01.md -->",
          "",
          "What Happened",
          "1. Remove this entry.",
          "",
          "<!-- GreenchClaw:dreaming:diary:end -->",
          "",
        ].join("\n"),
        "utf-8",
      );
  ...
  it("applies top promote candidates into MEMORY.md", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      expect(memoryText).toContain("GreenchClaw-memory-promotion:");
