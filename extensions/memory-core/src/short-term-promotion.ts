...
import { formatMemoryDreamingDay } from "GreenchClaw/plugin-sdk/memory-core-host-status";
...
const PROMOTION_MARKER_PREFIX = "GreenchClaw-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*GreenchClaw-memory-promotion:/i.test(snippet) ||
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*GreenchClaw-memory-promotion:([^\n]+?)\s*-->/gi);
