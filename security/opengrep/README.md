...
`precise.yml` is GreenchClaw's shipped precise OpenGrep rulepack. Each rule is tied
...
```bash
# from the GreenchClaw repo root
...
Some GreenchClaw paths are excluded by `.gitignore` for build reasons even though
