...
function resolveAuthRole(token: string | null): ActorRole {
  ...
  const maintainerSecret = process.env.GREENCHCLAW_QA_CONVEX_SECRET_MAINTAINER?.trim();
  const ciSecret = process.env.GREENCHCLAW_QA_CONVEX_SECRET_CI?.trim();
...
function assertMaintainerAdminAuth(token: string | null) {
  ...
  if (!maintainerSecret) {
    throw new BrokerHttpError(
      500,
      "SERVER_MISCONFIGURED",
      "Admin endpoints require GREENCHCLAW_QA_CONVEX_SECRET_MAINTAINER on this deployment.",
    );
  }
