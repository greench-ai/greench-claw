...
coverage:
  ...
  secondary:
    - models.openai
objective: Verify a live OpenAI GPT model can use OpenAI native web_search when GreenchClaw web search is enabled in auto mode.
successCriteria:
...
execution:
  kind: flow
  summary: Run with `GREENCHCLAW_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" pnpm GreenchClaw qa suite --provider-mode live-frontier --model openai/gpt-5.5 --alt-model openai/gpt-5.5 --fast --thinking medium --scenario openai-native-web-search-live`.
  config:
...
steps:
  - name: confirms live OpenAI GPT-5.5 web search auto mode
    actions:
      ...
      - assert:
          expr: "env.providerMode !== 'live-frontier' || Boolean(env.gateway.runtimeEnv.OPENAI_API_KEY?.trim() || env.gateway.runtimeEnv.GREENCHCLAW_LIVE_OPENAI_KEY?.trim())"
          message: expected OPENAI_API_KEY or GREENCHCLAW_LIVE_OPENAI_KEY for live OpenAI QA
