...
export async function resolveCodexAppServerProtocolSource(repoRoot: string): Promise<{
  ...
  throw new Error(
    [
      ...
      "Set GREENCHCLAW_CODEX_REPO to a checkout of openai/codex, or keep a sibling `codex` checkout next to the primary GreenchClaw checkout.",
...
async function collectCodexRepoCandidates(repoRoot: string): Promise<string[]> {
  const candidates = [
    process.env.GREENCHCLAW_CODEX_REPO,
