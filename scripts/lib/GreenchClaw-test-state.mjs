...
function usage() {
  return `Usage:
  node scripts/lib/GreenchClaw-test-state.mjs -- create [--label <name>] [--scenario <name>] [--env-file <path>] [--json]
  node scripts/lib/GreenchClaw-test-state.mjs shell [--label <name>] [--scenario <name>]
  node scripts/lib/GreenchClaw-test-state.mjs shell-function
...
function scenarioConfig(scenario, options = {}) {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      skills: {
        allowBundled: ["memory", "GreenchClaw-testing"],
        limits: {
          maxSkillsInPrompt: 8,
  ...
  if (scenario === "gateway-loopback") {
    return {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.token || "GreenchClaw-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function renderConfigWrite(configPathExpression, config) {
  ...
  return [
    `cat > ${configPathExpression} <<'GREENCHCLAW_TEST_STATE_JSON'`,
    json,
    "GREENCHCLAW_TEST_STATE_JSON",
  ].join("\n");
}
...
export async function createState(options = {}) {
  const label = normalizeLabel(options.label);
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `GreenchClaw-${label}-`));
...
export function renderShellSnippet(options = {}) {
  ...
  const homeTemplate = `GreenchClaw-${label}-${scenario}-home.XXXXXX`;
  const lines = [
    'GREENCHCLAW_TEST_STATE_TMP_ROOT="${GREENCHCLAW_TEST_STATE_TMPDIR:-${TMPDIR:-/tmp}}"',
    "export GREENCHCLAW_TEST_STATE_TMP_ROOT",
    'mkdir -p "$GREENCHCLAW_TEST_STATE_TMP_ROOT"',
    `GREENCHCLAW_TEST_STATE_HOME="$(mktemp -d "$GREENCHCLAW_TEST_STATE_TMP_ROOT/${homeTemplate}")"`,
    'export HOME="$GREENCHCLAW_TEST_STATE_HOME"',
    'export USERPROFILE="$GREENCHCLAW_TEST_STATE_HOME"',
    'export GREENCHCLAW_HOME="$GREENCHCLAW_TEST_STATE_HOME"',
    'export GREENCHCLAW_STATE_DIR="$GREENCHCLAW_TEST_STATE_HOME/.GreenchClaw"',
    'export GREENCHCLAW_CONFIG_PATH="$GREENCHCLAW_STATE_DIR/GreenchClaw.json"',
    'export GREENCHCLAW_TEST_WORKSPACE_DIR="$GREENCHCLAW_TEST_STATE_HOME/workspace"',
    'mkdir -p "$GREENCHCLAW_STATE_DIR" "$GREENCHCLAW_TEST_WORKSPACE_DIR"',
  ];
  for (const [key, value] of Object.entries(env)) {
    lines.push(`export ${key}=${shellQuote(value)}`);
  }
  const configWrite = renderConfigWrite('"$GREENCHCLAW_CONFIG_PATH"', config);
  if (configWrite) {
...
export function renderShellFunction() {
  return `GreenchClaw_test_state_create() {
  ...
  case "$scenario" in
    ...
    *)
      echo "unknown GreenchClaw test-state scenario: $scenario" >&2
      return 1
      ;;
  esac
  case "$raw_label" in
    /*)
      GREENCHCLAW_TEST_STATE_HOME="$raw_label"
      mkdir -p "$GREENCHCLAW_TEST_STATE_HOME"
      ;;
    *)
      label="$(printf "%s" "$label" | tr -cs "A-Za-z0-9_.-" "-" | sed -e "s/^-*//" -e "s/-*$//")"
      [ -n "$label" ] || label="state"
      local tmp_root="\${GREENCHCLAW_TEST_STATE_TMPDIR:-\${TMPDIR:-/tmp}}"
      mkdir -p "$tmp_root"
      GREENCHCLAW_TEST_STATE_HOME="$(mktemp -d "$tmp_root/GreenchClaw-$label-$scenario-home.XXXXXX")"
      ;;
  esac
  export HOME="$GREENCHCLAW_TEST_STATE_HOME"
  export USERPROFILE="$GREENCHCLAW_TEST_STATE_HOME"
  ...
  export GREENCHCLAW_CONFIG_PATH="$GREENCHCLAW_STATE_DIR/GreenchClaw.json"
  export GREENCHCLAW_TEST_WORKSPACE_DIR="$GREENCHCLAW_TEST_STATE_HOME/workspace"
  unset GREENCHCLAW_AGENT_DIR
  unset PI_CODING_AGENT_DIR
  unset GREENCHCLAW_SERVICE_REPAIR_POLICY
  mkdir -p "$GREENCHCLAW_STATE_DIR" "$GREENCHCLAW_TEST_WORKSPACE_DIR"
  case "$scenario" in
    minimal)
      cat > "$GREENCHCLAW_CONFIG_PATH" <<'GREENCHCLAW_TEST_STATE_JSON'
{}
GREENCHCLAW_TEST_STATE_JSON
      ;;
    update-stable)
...
GREENCHCLAW_TEST_STATE_JSON
      ;;
    external-service)
      export GREENCHCLAW_SERVICE_REPAIR_POLICY="external"
      cat > "$GREENCHCLAW_CONFIG_PATH" <<'GREENCHCLAW_TEST_STATE_JSON'
{}
GREENCHCLAW_TEST_STATE_JSON
      ;;
