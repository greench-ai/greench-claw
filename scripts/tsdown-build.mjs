...
const logLevel = process.env.GREENCHCLAW_BUILD_VERBOSE ? "info" : "warn";
...
export async function runTsdownBuildInvocation(invocation, params = {}) {
  ...
  const timeoutMs = parsePositiveInteger(env.GREENCHCLAW_TSDOWN_TIMEOUT_MS);
  const heartbeatMs =
    parseNonNegativeInteger(env.GREENCHCLAW_TSDOWN_HEARTBEAT_MS) ?? DEFAULT_HEARTBEAT_MS;
  let timedOut = false;
  let settled = false;
