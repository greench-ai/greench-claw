...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "GreenchClaw.json");
  ...
  const sessionId = process.env.GREENCHCLAW_ZAI_FALLBACK_SESSION_ID ?? randomUUID();
  ...
  const run1 = await runCommand(
    "run1",
    ["GreenchClaw", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["GreenchClaw", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
