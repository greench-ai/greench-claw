#!/usr/bin/env node
// Builds the GreenchClaw package artifact used by Docker E2E.
...
async function newestGreenchClawTarball(outputDir, packOutput) {
  let fromOutput = "";
  ...
  const packed = entries
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed GreenchClaw tarball in ${outputDir}`);
  }
...
async function main() {
  ...
  if (!options.skipBuild) {
    console.error("==> Building GreenchClaw package artifacts");
    await run("pnpm", ["build"], sourceDir);
    console.error("==> Building GreenchClaw Control UI artifacts");
    await run("pnpm", ["ui:build"], sourceDir);
  }

  console.error("==> Writing GreenchClaw package inventory");
  await run(
    "node",
    [
      "--import",
  ...
  console.error("==> Packing GreenchClaw package");
  const packOutput = await runCapture(
    "npm",
    ["pack", "--silent", "--ignore-scripts", "--pack-destination", outputDir],
    sourceDir,
  );
  let tarball = await newestGreenchClawTarball(outputDir, packOutput);
  ...
  console.error("==> Checking GreenchClaw package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-GreenchClaw-package-tarball.mjs"), tarball],
    sourceDir,
    { timeoutMs: 5 * 60 * 1000 },
  );
  console.error(
