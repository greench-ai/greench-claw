...
resolve_default_smoke_platform() {
  local host_os
  local host_arch
  if [[ -n "${GREENCHCLAW_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$GREENCHCLAW_INSTALL_SMOKE_PLATFORM"
    return
  fi
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.GREENCHCLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `GREENCHCLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
SMOKE_IMAGE="${GREENCHCLAW_INSTALL_SMOKE_IMAGE:-GreenchClaw-install-smoke:local}"
NONROOT_IMAGE="${GREENCHCLAW_INSTALL_NONROOT_IMAGE:-GreenchClaw-install-nonroot:local}"
SMOKE_PLATFORM="$(resolve_default_smoke_platform)"
NONROOT_PLATFORM="${GREENCHCLAW_INSTALL_NONROOT_PLATFORM:-$SMOKE_PLATFORM}"
INSTALL_URL="${GREENCHCLAW_INSTALL_URL:-https://GreenchClaw.bot/install.sh}"
CLI_INSTALL_URL="${GREENCHCLAW_INSTALL_CLI_URL:-https://GreenchClaw.bot/install-cli.sh}"
PACKAGE_NAME="${GREENCHCLAW_INSTALL_PACKAGE:-GreenchClaw}"
SKIP_NONROOT="${GREENCHCLAW_INSTALL_SMOKE_SKIP_NONROOT:-0}"
SKIP_SMOKE_IMAGE_BUILD="${GREENCHCLAW_INSTALL_SMOKE_SKIP_IMAGE_BUILD:-0}"
SKIP_NONROOT_IMAGE_BUILD="${GREENCHCLAW_INSTALL_NONROOT_SKIP_IMAGE_BUILD:-0}"
SKIP_UPDATE="${GREENCHCLAW_INSTALL_SMOKE_SKIP_UPDATE:-0}"
SKIP_NPM_GLOBAL="${GREENCHCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"
UPDATE_BASELINE_VERSION="${GREENCHCLAW_INSTALL_SMOKE_UPDATE_BASELINE:-latest}"
UPDATE_PACKAGE_SPEC="${GREENCHCLAW_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_DIST_IMAGE="${GREENCHCLAW_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"
UPDATE_SKIP_LOCAL_BUILD="${GREENCHCLAW_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${GREENCHCLAW_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${GREENCHCLAW_INSTALL_SMOKE_UPDATE_PORT:-}"
UPDATE_EXPECT_VERSION="${GREENCHCLAW_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"
LATEST_DIR="$(mktemp -d)"
...
NPM_CACHE_DIR="${GREENCHCLAW_INSTALL_SMOKE_NPM_CACHE_DIR:-}"
NPM_CACHE_OWNED=0
...
' "$pack_json_file"
  ...
  if [[ -z "$UPDATE_PACKAGE_SPEC" ]]; then
    node scripts/check-GreenchClaw-package-tarball.mjs "${UPDATE_DIR}/${UPDATE_TGZ_FILE}"
  fi
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (GREENCHCLAW_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  prepare_npm_cache
  ...
  docker run --rm -t \
    ...
    -e GREENCHCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e GREENCHCLAW_INSTALL_METHOD=npm \
    -e GREENCHCLAW_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e GREENCHCLAW_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e GREENCHCLAW_INSTALL_LATEST_OUT="/out/latest" \
    -e GREENCHCLAW_NO_ONBOARD=1 \
    -e GREENCHCLAW_NO_PROMPT=1 \
  ...
  docker run --rm -t \
    ...
    "${NPM_CACHE_DOCKER_ARGS[@]}" \
    -e GREENCHCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e GREENCHCLAW_INSTALL_SMOKE_MODE=update \
    -e GREENCHCLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e GREENCHCLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e GREENCHCLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e GREENCHCLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e GREENCHCLAW_NO_ONBOARD=1 \
    -e GREENCHCLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
  ...
  if [[ "$SKIP_NPM_GLOBAL" == "1" ]]; then
    echo "==> Skip direct npm global smoke (GREENCHCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1)"
  else
    ...
    docker run --rm -t \
      ...
      "${NPM_CACHE_DOCKER_ARGS[@]}" \
      -e GREENCHCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
      -e GREENCHCLAW_INSTALL_SMOKE_MODE=npm-global \
      -e GREENCHCLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
      -e GREENCHCLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
      -e GREENCHCLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
      -e GREENCHCLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (GREENCHCLAW_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
  ...
  docker run --rm -t \
    ...
    -e GREENCHCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e GREENCHCLAW_INSTALL_METHOD=npm \
    -e GREENCHCLAW_INSTALL_EXPECT_VERSION="$LATEST_VERSION" \
    -e GREENCHCLAW_NO_ONBOARD=1 \
    -e GREENCHCLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$NONROOT_IMAGE"
...
if [[ "${GREENCHCLAW_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (GREENCHCLAW_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
...
docker run --rm -t \
  ...
  --entrypoint /bin/bash \
  -e GREENCHCLAW_INSTALL_URL="$INSTALL_URL" \
  -e GREENCHCLAW_INSTALL_CLI_URL="$CLI_INSTALL_URL" \
  -e GREENCHCLAW_NO_ONBOARD=1 \
  -e GREENCHCLAW_NO_PROMPT=1 \
  -e DEBIAN_FRONTEND=noninteractive \
