...
- regenerates apps/ios/GreenchClaw.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${GREENCHCLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid GREENCHCLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid GREENCHCLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid GREENCHCLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid GREENCHCLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing GREENCHCLAW_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(GREENCHCLAW_URL_SLASH)$(GREENCHCLAW_URL_SLASH)#g'
)"
...
GREENCHCLAW_CODE_SIGN_STYLE = Automatic
GREENCHCLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
GREENCHCLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
GREENCHCLAW_APP_BUNDLE_ID = ai.GreenchClaw.client
GREENCHCLAW_SHARE_BUNDLE_ID = ai.GreenchClaw.client.share
GREENCHCLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.GreenchClaw.client.activitywidget
GREENCHCLAW_WATCH_APP_BUNDLE_ID = ai.GreenchClaw.client.watchkitapp
GREENCHCLAW_WATCH_EXTENSION_BUNDLE_ID = ai.GreenchClaw.client.watchkitapp.extension
GREENCHCLAW_APP_PROFILE =
GREENCHCLAW_SHARE_PROFILE =
GREENCHCLAW_PUSH_TRANSPORT = relay
GREENCHCLAW_PUSH_DISTRIBUTION = official
GREENCHCLAW_URL_SLASH = /
GREENCHCLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
GREENCHCLAW_PUSH_APNS_ENVIRONMENT = production
EOF
