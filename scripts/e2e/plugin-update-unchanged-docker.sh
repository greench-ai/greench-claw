#!/usr/bin/env bash
# Verifies `GreenchClaw plugins update` is a no-op for an already-current plugin.
...
IMAGE_NAME="$(docker_e2e_resolve_image "GreenchClaw-plugin-update-e2e" GREENCHCLAW_PLUGIN_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${GREENCHCLAW_PLUGIN_UPDATE_E2E_SKIP_BUILD:-0}"
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz plugin-update "${GREENCHCLAW_CURRENT_PACKAGE_TGZ:-}")"
...
GREENCHCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 plugin-update empty)"
