...
IMAGE_NAME="$(docker_e2e_resolve_image "GreenchClaw-live-plugin-tool-e2e" GREENCHCLAW_LIVE_PLUGIN_TOOL_E2E_IMAGE)"
DOCKER_TARGET="${GREENCHCLAW_LIVE_PLUGIN_TOOL_DOCKER_TARGET:-bare}"
HOST_BUILD="${GREENCHCLAW_LIVE_PLUGIN_TOOL_HOST_BUILD:-1}"
PACKAGE_TGZ="${GREENCHCLAW_CURRENT_PACKAGE_TGZ:-}"
AGENT_TURN_TIMEOUT_SECONDS="${GREENCHCLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}"
PROFILE_FILE="${GREENCHCLAW_LIVE_PLUGIN_TOOL_PROFILE_FILE:-${GREENCHCLAW_TESTBOX_PROFILE_FILE:-$HOME/.GreenchClaw-testbox-live.profile}}"
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${GREENCHCLAW_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "GREENCHCLAW_LIVE_PLUGIN_TOOL_HOST_BUILD=0 requires GREENCHCLAW_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
GREENCHCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 live-plugin-tool empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e GREENCHCLAW_LIVE_PLUGIN_TOOL_MODEL="${GREENCHCLAW_LIVE_PLUGIN_TOOL_MODEL:-openai/gpt-5.5}" \
  -e "GREENCHCLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
...
MODEL_REF="${GREENCHCLAW_LIVE_PLUGIN_TOOL_MODEL:?missing GREENCHCLAW_LIVE_PLUGIN_TOOL_MODEL}"
PLUGIN_ID="e2e-slug-tool"
PLUGIN_NAME="@GreenchClaw/e2e-slug-tool"
PLUGIN_VERSION="0.0.0-e2e.1"
TOOL_NAME="e2e_slug_probe"
SEED="GreenchClaw E2E Plugin Tool $(date +%s)-$RANDOM"
...
dump_debug_logs() {
  ...
  GreenchClaw_e2e_dump_logs \
    ...
    /tmp/GreenchClaw-plugin-install.log \
    /tmp/GreenchClaw-plugin-enable.log \
    /tmp/GreenchClaw-plugins-list.json \
    /tmp/GreenchClaw-plugin-inspect.json \
...
fixture_dir="$(mktemp -d /tmp/GreenchClaw-live-plugin-tool.XXXXXX)"
...
GreenchClaw plugins install "npm-pack:$plugin_tgz" --force >/tmp/GreenchClaw-plugin-install.log 2>&1
node scripts/e2e/lib/live-plugin-tool/assertions.mjs configure
GreenchClaw plugins enable "$PLUGIN_ID" >/tmp/GreenchClaw-plugin-enable.log 2>&1
GreenchClaw plugins list --json >/tmp/GreenchClaw-plugins-list.json
GreenchClaw plugins inspect "$PLUGIN_ID" --runtime --json >/tmp/GreenchClaw-plugin-inspect.json
...
GreenchClaw agent --local \
  ...
  --thinking off \
  --timeout "${GREENCHCLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}" \
  --json >/tmp/GreenchClaw-agent.json 2>/tmp/GreenchClaw-agent.err
