    ...
    [
      {
        id: "logging-file",
        intent: "logging",
        argv: ["config", "set", "logging.file", "~/GreenchClaw-upgrade-survivor/gateway.jsonl"],
      },
    ],
  ],
  [
...
function selectedScenario() {
  return process.env.GREENCHCLAW_UPGRADE_SURVIVOR_SCENARIO || "base";
}
...
function runGreenchClaw(step) {
  const result = spawnSync("GreenchClaw", step.argv, {
    encoding: "utf8",
    env: process.env,
  });
  return {
    id: step.id,
    intent: step.intent,
    command: ["GreenchClaw", ...step.argv].join(" "),
...
function applyRecipe() {
  ...
  for (const step of [...recipe.slice(0, -1), ...scenarioSteps, recipe.at(-1)]) {
    ...
    const outcome = runGreenchClaw(adaptedStep);
