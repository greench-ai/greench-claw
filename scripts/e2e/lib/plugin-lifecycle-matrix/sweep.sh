...
GreenchClaw_e2e_install_package /tmp/GreenchClaw-plugin-lifecycle-install.log "mounted GreenchClaw package" /tmp/npm-prefix
...
pack_root="$(mktemp -d "/tmp/GreenchClaw-plugin-lifecycle-pack.XXXXXX")"
registry_root="$(mktemp -d "/tmp/GreenchClaw-plugin-lifecycle-registry.XXXXXX")"
