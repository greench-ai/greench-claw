...
function writeOpenWebUiWorkspace() {
  const workspace =
    process.env.GREENCHCLAW_WORKSPACE_DIR ||
    path.join(process.env.HOME, ".GreenchClaw", "workspace");
  write(
...
function writeAgentsDeleteConfig() {
  const stateDir = requireArg(process.env.GREENCHCLAW_STATE_DIR, "GREENCHCLAW_STATE_DIR");
  ...
  writeJson(path.join(stateDir, "GreenchClaw.json"), {
    agents: {
      list: [
...
function assertAgentsDeleteResult([outputPath]) {
  ...
  const remaining =
    readJson(path.join(process.env.GREENCHCLAW_STATE_DIR, "GreenchClaw.json"))?.agents?.list ?? [];
