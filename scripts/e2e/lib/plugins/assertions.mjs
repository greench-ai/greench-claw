...
function getInstallRecords() {
  ...
  const allowLegacyCompat = process.env.GREENCHCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function readGreenchClawConfig() {
  const configPath = path.join(process.env.HOME, ".GreenchClaw", "GreenchClaw.json");
...
function assertPluginRemoved(params) {
  ...
  const config = readGreenchClawConfig();
...
function recordFixturePluginTrust() {
  ...
  const ledgerPath = path.join(process.env.HOME, ".GreenchClaw", "plugins", "installs.json");
  const ledger = fs.existsSync(ledgerPath)
    ? readJson(ledgerPath)
    : {
        version: 1,
        warning:
          "DO NOT EDIT. This file is generated by GreenchClaw plugin install/update/uninstall commands. Use `GreenchClaw plugins install/update/uninstall` instead.",
        records: {},
      };
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".GreenchClaw", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(GreenchClawPeerPath)) {
    throw new Error(`missing ClawHub GreenchClaw peer symlink: ${GreenchClawPeerPath}`);
  }
  if (!fs.lstatSync(GreenchClawPeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub GreenchClaw peer is not a symlink: ${GreenchClawPeerPath}`);
  }
  ...
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub GreenchClaw peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@GreenchClaw/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@GreenchClaw/demo-plugin-npm") {
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.GREENCHCLAW_CLAWHUB_URL ||
    process.env.CLAWHUB_URL ||
    "https://clawhub.ai"
  ).replace(/\/+$/, "");
  const token =
    process.env.GREENCHCLAW_CLAWHUB_TOKEN ||
...
function assertClawHubInstalled() {
  ...
  const installPath = record.installPath.replace(/^~(?=$|\/)/u, process.env.HOME);
  const extensionsRoot = path.join(process.env.HOME, ".GreenchClaw", "extensions");
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".GreenchClaw", "GreenchClaw.json");
