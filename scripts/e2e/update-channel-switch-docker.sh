...
IMAGE_NAME="$(docker_e2e_resolve_image "GreenchClaw-update-channel-switch-e2e" GREENCHCLAW_UPDATE_CHANNEL_SWITCH_E2E_IMAGE)"
SKIP_BUILD="${GREENCHCLAW_UPDATE_CHANNEL_SWITCH_E2E_SKIP_BUILD:-0}"
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz update-channel-switch "${GREENCHCLAW_CURRENT_PACKAGE_TGZ:-}")"
# Bare lanes mount the package artifact instead of baking app sources into the image.
docker_e2e_package_mount_args "$PACKAGE_TGZ"
GREENCHCLAW_TEST_STATE_SCRIPT_B64="$(
  node "$ROOT_DIR/scripts/lib/GreenchClaw-test-state.mjs" shell \
...
(
  cd "$git_root"
  if ! npm install --omit=optional --no-fund --no-audit >/tmp/GreenchClaw-git-install.log 2>&1; then
    cat /tmp/GreenchClaw-git-install.log >&2 || true
    exit 1
  fi
)
node scripts/e2e/lib/update-channel-switch/assertions.mjs write-control-ui "$git_root"

git config --global user.email "docker-e2e@GreenchClaw.local"
git config --global user.name "GreenchClaw Docker E2E"
...
package_version="$(node -p "JSON.parse(require(\"node:fs\").readFileSync(\"/tmp/npm-prefix/lib/node_modules/GreenchClaw/package.json\", \"utf8\")).version")"
...
export GREENCHCLAW_GIT_DIR="$git_root"
export GREENCHCLAW_UPDATE_DEV_TARGET_REF="$fixture_sha"

echo "==> package -> git dev channel"
set +e
dev_json="$(GreenchClaw update --channel dev --yes --json --no-restart)"
dev_status=$?
set -e
printf "%s\n" "$dev_json"
...
status_json="$(GreenchClaw update status --json)"
printf "%s\n" "$status_json"
...
set +e
stable_json="$(GreenchClaw update --channel stable --tag "$pkg_tgz_path" --yes --json --no-restart)"
stable_status=$?
set -e
printf "%s\n" "$stable_json"
