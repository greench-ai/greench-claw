...
import type { GreenchClawConfig } from "../../dist/config/types.GreenchClaw.js";
...
function makeParams(commandBody: string, cfg: GreenchClawConfig, isGroup = false) {
  return {
    cfg,
    command: {
      surface: "whatsapp",
...
async function main() {
  const stateDir =
    ...
    (await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-crestodian-")));
  const configPath = process.env.GREENCHCLAW_CONFIG_PATH ?? path.join(stateDir, "GreenchClaw.json");
  ...
  const refPlan = await invoke(
    "/crestodian config set-ref gateway.auth.token env GREENCHCLAW_GATEWAY_TOKEN",
    cfg,
  );
  assert(
  ...
  const agentPlan = await invoke(
    "/crestodian create agent work workspace /tmp/GreenchClaw-work",
    cfg,
  );
  assert(
  ...
  const setupPlan = await invoke(
    "/crestodian setup workspace /tmp/GreenchClaw-setup model openai/gpt-5.2",
    cfg,
  );
  ...
  const updatedConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as GreenchClawConfig;
  assert(
  ...
  assert(
    updatedConfig.gateway?.auth?.token &&
      ...
      updatedConfig.gateway.auth.token.id === "GREENCHCLAW_GATEWAY_TOKEN",
    "SecretRef set did not update gateway.auth.token",
  );
  assert(
    updatedConfig.agents?.defaults?.workspace === "/tmp/GreenchClaw-setup",
    "setup did not update default workspace",
  );
  assert(
    updatedConfig.agents?.list?.some(
      (agent) => agent.id === "work" && agent.workspace === "/tmp/GreenchClaw-work",
    ),
