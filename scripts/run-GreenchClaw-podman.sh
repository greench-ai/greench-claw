#!/usr/bin/env bash
# Rootless GreenchClaw in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-GreenchClaw-podman.sh launch        # Start gateway
#   ./scripts/run-GreenchClaw-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   GreenchClaw --container GreenchClaw dashboard --no-open
#   GreenchClaw --container GreenchClaw channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      GREENCHCLAW_GATEWAY_TOKEN|GREENCHCLAW_PODMAN_CONTAINER|GREENCHCLAW_PODMAN_IMAGE|GREENCHCLAW_IMAGE|GREENCHCLAW_PODMAN_PULL|GREENCHCLAW_PODMAN_GATEWAY_HOST_PORT|GREENCHCLAW_GATEWAY_PORT|GREENCHCLAW_PODMAN_BRIDGE_HOST_PORT|GREENCHCLAW_BRIDGE_PORT|GREENCHCLAW_GATEWAY_BIND|GREENCHCLAW_PODMAN_USERNS|GREENCHCLAW_BIND_MOUNT_OPTIONS|GREENCHCLAW_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-GreenchClaw-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${GREENCHCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.GreenchClaw}"
ENV_FILE="${GREENCHCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${GREENCHCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.GreenchClaw}"
ENV_FILE="${GREENCHCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${GREENCHCLAW_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${GREENCHCLAW_PODMAN_CONTAINER:-GreenchClaw}"
GREENCHCLAW_IMAGE="${GREENCHCLAW_PODMAN_IMAGE:-${GREENCHCLAW_IMAGE:-GreenchClaw:local}}"
PODMAN_PULL="${GREENCHCLAW_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${GREENCHCLAW_PODMAN_GATEWAY_HOST_PORT:-${GREENCHCLAW_GATEWAY_PORT:-18420}}"
HOST_BRIDGE_PORT="${GREENCHCLAW_PODMAN_BRIDGE_HOST_PORT:-${GREENCHCLAW_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${GREENCHCLAW_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$GREENCHCLAW_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v GreenchClaw >/dev/null 2>&1; then
    return 0
  fi
  GREENCHCLAW_CONTAINER="" GREENCHCLAW_CONFIG_DIR="$config_dir" \
    GreenchClaw config get gateway.bind 2>/dev/null || true
}
...
# GREENCHCLAW_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${GREENCHCLAW_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'GREENCHCLAW_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v GreenchClaw >/dev/null 2>&1; then
    echo "Warning: GreenchClaw not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    GREENCHCLAW_CONTAINER="" GREENCHCLAW_CONFIG_DIR="$config_dir" \
      GreenchClaw config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    GREENCHCLAW_CONTAINER="" GREENCHCLAW_CONFIG_DIR="$config_dir" \
      GreenchClaw config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  GREENCHCLAW_CONTAINER="" GREENCHCLAW_CONFIG_DIR="$config_dir" \
    GreenchClaw config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${GREENCHCLAW_GATEWAY_TOKEN:-}" ]]; then
  export GREENCHCLAW_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "GREENCHCLAW_GATEWAY_TOKEN" "$GREENCHCLAW_GATEWAY_TOKEN"
  echo "Generated GREENCHCLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/GreenchClaw.json"
...
PODMAN_USERNS="${GREENCHCLAW_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported GREENCHCLAW_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (GREENCHCLAW_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${GREENCHCLAW_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${GREENCHCLAW_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$GREENCHCLAW_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.GreenchClaw/.npm \
    -e GREENCHCLAW_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.GreenchClaw:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.GreenchClaw/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$GREENCHCLAW_IMAGE" \
...
podman run --pull="$PODMAN_PULL" -d --replace \
  ...
  -v "$CONFIG_DIR:/home/node/.GreenchClaw:rw${SELINUX_MOUNT_OPTS}" \
  -v "$WORKSPACE_DIR:/home/node/.GreenchClaw/workspace:rw${SELINUX_MOUNT_OPTS}" \
  -p "${PUBLISH_HOST}:${HOST_GATEWAY_PORT}:18420" \
  -p "${PUBLISH_HOST}:${HOST_BRIDGE_PORT}:18790" \
  "$GREENCHCLAW_IMAGE" \
  node dist/index.js gateway --bind "$GATEWAY_BIND" --port 18420 >/dev/null

echo "Container $CONTAINER_NAME started: http://127.0.0.1:${HOST_GATEWAY_PORT}/"
echo "podman exec -it $CONTAINER_NAME GreenchClaw dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME GreenchClaw devices approve --latest  # if pairing required"
