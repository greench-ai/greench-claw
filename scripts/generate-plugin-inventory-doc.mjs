...
function resolveDescription({ manifest, packageJson }) {
  ...
  if (channels.length > 0) {
    ...
    return `Adds the ${channelLabel}${channelNoun} surface for sending and receiving GreenchClaw messages.`;
  }

  const providers = Array.isArray(manifest.providers) ? manifest.providers : [];
  if (providers.length > 0) {
    return `Adds ${displayList(providers)} model provider support to GreenchClaw.`;
  }
  ...
  return packageDescription ? `${packageDescription}.` : "Provides an GreenchClaw plugin.";
}
...
function resolveDocs({ dirName, manifest, packageJson }) {
  ...
  const channelDoc = normalizeDocPath(packageJson.GreenchClaw?.channel?.docsPath);
  if (channelDoc) {
...
function resolveInstallRoute(packageJson, status) {
  ...
  if (status === "core") {
    return "included in GreenchClaw";
  }
  const install = packageJson.GreenchClaw?.install;
  const release = packageJson.GreenchClaw?.release;
  const clawhubSpec =
...
function resolveStatus({ dirName, packageJson, excludedDirs }) {
  ...
  const hasInstallSpec =
    typeof packageJson.GreenchClaw?.install?.clawhubSpec === "string" ||
...
function renderReferenceIndex(records) {
  return `---
summary: "Generated index of GreenchClaw plugin reference pages"
read_when:
  - You need a reference page for a specific GreenchClaw plugin
...
\`GreenchClaw.plugin.json\`. Regenerate it with:
...
function collectPluginSourceEntries() {
  ...
  for (const dirName of fs
    ...
    const manifestPath = path.join(EXTENSIONS_DIR, dirName, "GreenchClaw.plugin.json");
