...
const RSS_MARKER = "__GREENCHCLAW_MAX_RSS_KB__=";
...
async function main() {
  ...
  const tmpHome = mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-extension-memory-"));
  const hookPath = path.join(tmpHome, "measure-rss.mjs");
  const jsonPath = options.jsonPath ?? path.join(os.tmpdir(), "GreenchClaw-extension-memory.json");
