...
TRUSTED_HARNESS_DIR="${GREENCHCLAW_LIVE_DOCKER_TRUSTED_HARNESS_DIR:-${GREENCHCLAW_LIVE_CODEX_TRUSTED_HARNESS_DIR:-$SCRIPT_ROOT_DIR}}"
...
CODEX_HARNESS_AUTH_MODE="${GREENCHCLAW_LIVE_CODEX_HARNESS_AUTH:-codex-auth}"
TEMP_DIRS=()
DOCKER_USER="${GREENCHCLAW_DOCKER_USER:-node}"
DOCKER_HOME_MOUNT=()
DOCKER_TRUSTED_HARNESS_MOUNT=()
DOCKER_TRUSTED_HARNESS_CONTAINER_DIR=""
DOCKER_CACHE_CONTAINER_DIR="/tmp/GreenchClaw-cache"
DOCKER_CLI_TOOLS_CONTAINER_DIR="/tmp/GreenchClaw-npm-global"
DOCKER_EXTRA_ENV_FILES=()
DOCKER_AUTH_PRESTAGED=0

GreenchClaw_live_codex_harness_is_ci() {
  [[ -n "${CI:-}" && "${CI:-}" != "false" ]] || [[ -n "${GITHUB_ACTIONS:-}" && "${GITHUB_ACTIONS:-}" != "false" ]]
}

GreenchClaw_live_codex_harness_append_build_extension() {
...
case "$CODEX_HARNESS_AUTH_MODE" in
  codex-auth | api-key)
    ;;
  *)
    echo "ERROR: GREENCHCLAW_LIVE_CODEX_HARNESS_AUTH must be one of: codex-auth, api-key." >&2
    exit 1
    ;;
esac
...
if [[ "$CODEX_HARNESS_AUTH_MODE" == "api-key" && -z "${OPENAI_API_KEY:-}" ]]; then
  echo "ERROR: GREENCHCLAW_LIVE_CODEX_HARNESS_AUTH=api-key requires OPENAI_API_KEY." >&2
  exit 1
fi
if [[ "$CODEX_HARNESS_AUTH_MODE" != "api-key" && ! -s "$HOME/.codex/auth.json" ]]; then
  echo "ERROR: GREENCHCLAW_LIVE_CODEX_HARNESS_AUTH=codex-auth requires ~/.codex/auth.json before building the live Docker image." >&2
  if [[ -n "${OPENAI_API_KEY:-}" ]]; then
    echo "If this is a Testbox/API-key run, set GREENCHCLAW_LIVE_CODEX_HARNESS_AUTH=api-key and run through GreenchClaw-testbox-env." >&2
  fi
  exit 1
fi
...
elif GreenchClaw_live_codex_harness_is_ci; then
...
if GreenchClaw_live_codex_harness_is_ci; then
...
if [[ "$CODEX_HARNESS_AUTH_MODE" != "api-key" ]]; then
  ...
  done < <(GreenchClaw_live_collect_auth_files_from_csv "openai-codex")
fi
...
if [[ -n "${DOCKER_HOME_DIR:-}" ]]; then
  GreenchClaw_live_stage_auth_into_home "$DOCKER_HOME_DIR" --files "${AUTH_FILES[@]}"
...
DOCKER_AUTH_ENV=()
if [[ "$CODEX_HARNESS_AUTH_MODE" == "api-key" ]]; then
  docker_env_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/GreenchClaw-codex-harness-env.XXXXXX")"
...
if [ "${GREENCHCLAW_LIVE_CODEX_HARNESS_DEBUG:-}" = "1" ]; then
  id
  mount | grep -E 'GreenchClaw-cache|GreenchClaw-npm|/home/node' || true
...
if [ "${GREENCHCLAW_LIVE_CODEX_HARNESS_AUTH:-codex-auth}" != "api-key" ]; then
...
if [ "${GREENCHCLAW_LIVE_CODEX_HARNESS_AUTH:-codex-auth}" != "api-key" ] && [ ! -s "$HOME/.codex/auth.json" ]; then
...
elif [ -f "$tmp_dir/extensions/codex/GreenchClaw.plugin.json" ]; then
  export GREENCHCLAW_BUNDLED_PLUGINS_DIR="$tmp_dir/extensions"
else
  echo "ERROR: staged Codex plugin not found for live harness." >&2
  exit 1
fi
GreenchClaw_live_stage_state_dir "$tmp_dir/.GreenchClaw-state"
if [ -n "${GREENCHCLAW_LIVE_CODEX_TRUSTED_HARNESS_DIR:-}" ] && [ -d "$GREENCHCLAW_LIVE_CODEX_TRUSTED_HARNESS_DIR" ]; then
  for harness_file in src/gateway/gateway-codex-harness.live-helpers.ts; do
    if [ -f "$GREENCHCLAW_LIVE_CODEX_TRUSTED_HARNESS_DIR/$harness_file" ]; then
      mkdir -p "$(dirname "$tmp_dir/$harness_file")"
      cp "$GREENCHCLAW_LIVE_CODEX_TRUSTED_HARNESS_DIR/$harness_file" "$tmp_dir/$harness_file"
    fi
  done
fi
GreenchClaw_live_prepare_staged_config
cd "$tmp_dir"
if [ "${GREENCHCLAW_LIVE_CODEX_HARNESS_USE_CI_SAFE_CODEX_CONFIG:-1}" = "1" ]; then
...
pnpm test:live ${GREENCHCLAW_LIVE_CODEX_TEST_FILES:-src/gateway/gateway-codex-harness.live.test.ts}
EOF

GreenchClaw_live_codex_harness_append_build_extension codex
...
GREENCHCLAW_SKIP_DOCKER_BUILD=0
export GREENCHCLAW_SKIP_DOCKER_BUILD
...
echo "==> Model: ${GREENCHCLAW_LIVE_CODEX_HARNESS_MODEL:-codex/gpt-5.5}"
echo "==> Image probe: ${GREENCHCLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE:-1}"
echo "==> MCP probe: ${GREENCHCLAW_LIVE_CODEX_HARNESS_MCP_PROBE:-1}"
echo "==> Subagent probe: ${GREENCHCLAW_LIVE_CODEX_HARNESS_SUBAGENT_PROBE:-1}"
echo "==> Subagent-only fast path: ${GREENCHCLAW_LIVE_CODEX_HARNESS_SUBAGENT_ONLY:-auto}"
echo "==> Guardian probe: ${GREENCHCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE:-1}"
echo "==> Auth mode: $CODEX_HARNESS_AUTH_MODE"
echo "==> Profile file: $PROFILE_STATUS"
echo "==> CI-safe Codex config: ${GREENCHCLAW_LIVE_CODEX_HARNESS_USE_CI_SAFE_CODEX_CONFIG:-1}"
echo "==> Test files: ${GREENCHCLAW_LIVE_CODEX_TEST_FILES:-src/gateway/gateway-codex-harness.live.test.ts}"
...
DOCKER_RUN_ARGS=(docker run --rm -t \
  ...
  -e GREENCHCLAW_AGENT_HARNESS_FALLBACK=none \
  -e GREENCHCLAW_DOCKER_AUTH_PRESTAGED="$DOCKER_AUTH_PRESTAGED" \
  -e GREENCHCLAW_CODEX_APP_SERVER_BIN="${GREENCHCLAW_CODEX_APP_SERVER_BIN:-codex}" \
  ...
  -e GREENCHCLAW_LIVE_CODEX_HARNESS_AUTH="$CODEX_HARNESS_AUTH_MODE" \
  -e GREENCHCLAW_LIVE_CODEX_HARNESS=1 \
  -e GREENCHCLAW_LIVE_CODEX_HARNESS_DEBUG="${GREENCHCLAW_LIVE_CODEX_HARNESS_DEBUG:-}" \
  -e GREENCHCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE="${GREENCHCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE:-1}" \
  -e GREENCHCLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE="${GREENCHCLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE:-1}" \
  -e GREENCHCLAW_LIVE_CODEX_HARNESS_MCP_PROBE="${GREENCHCLAW_LIVE_CODEX_HARNESS_MCP_PROBE:-1}" \
  -e GREENCHCLAW_LIVE_CODEX_HARNESS_MODEL="${GREENCHCLAW_LIVE_CODEX_HARNESS_MODEL:-codex/gpt-5.5}" \
  -e GREENCHCLAW_LIVE_CODEX_HARNESS_REQUIRE_GUARDIAN_EVENTS="${GREENCHCLAW_LIVE_CODEX_HARNESS_REQUIRE_GUARDIAN_EVENTS:-1}" \
  -e GREENCHCLAW_LIVE_CODEX_HARNESS_REQUEST_TIMEOUT_MS="${GREENCHCLAW_LIVE_CODEX_HARNESS_REQUEST_TIMEOUT_MS:-}" \
  -e GREENCHCLAW_LIVE_CODEX_HARNESS_SUBAGENT_ONLY="${GREENCHCLAW_LIVE_CODEX_HARNESS_SUBAGENT_ONLY:-}" \
  -e GREENCHCLAW_LIVE_CODEX_HARNESS_SUBAGENT_PROBE="${GREENCHCLAW_LIVE_CODEX_HARNESS_SUBAGENT_PROBE:-1}" \
  -e GREENCHCLAW_LIVE_CODEX_HARNESS_USE_CI_SAFE_CODEX_CONFIG="${GREENCHCLAW_LIVE_CODEX_HARNESS_USE_CI_SAFE_CODEX_CONFIG:-1}" \
  ...
  -e GREENCHCLAW_LIVE_DOCKER_TRUSTED_HARNESS_DIR="$DOCKER_TRUSTED_HARNESS_CONTAINER_DIR" \
  -e GREENCHCLAW_LIVE_CODEX_TRUSTED_HARNESS_DIR="$DOCKER_TRUSTED_HARNESS_CONTAINER_DIR" \
  -e GREENCHCLAW_LIVE_CODEX_BIND="${GREENCHCLAW_LIVE_CODEX_BIND:-}" \
  -e GREENCHCLAW_LIVE_CODEX_BIND_MODEL="${GREENCHCLAW_LIVE_CODEX_BIND_MODEL:-}" \
  -e GREENCHCLAW_LIVE_CODEX_TEST_FILES="${GREENCHCLAW_LIVE_CODEX_TEST_FILES:-}" \
