...
#
# Suggested cron: */30 * * * * /home/admin/GreenchClaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send GreenchClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., GreenchClaw-alerts)
...
STATE_FILE="$HOME/.GreenchClaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via GreenchClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use GreenchClaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via GreenchClaw to $NOTIFY_PHONE..."
            GreenchClaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
    ...
    if [ -n "$NOTIFY_NTFY" ]; then
        ...
        curl -s -o /dev/null \
            -H "Title: GreenchClaw Auth Alert" \
            -H "Priority: $priority" \
            -H "Tags: warning,key" \
            -d "$message" \
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! GreenchClaw is down. Run: ssh l36 '~/GreenchClaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
