#!/usr/bin/env bash
# Rootless NexisClaw in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-NexisClaw-podman.sh launch        # Start gateway
#   ./scripts/run-NexisClaw-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   NexisClaw --container NexisClaw dashboard --no-open
#   NexisClaw --container NexisClaw channels login
#
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-NexisClaw-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${NEXISCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.NexisClaw}"
...
resolve_config_gateway_bind() {
  ...
  NEXISCLAW_CONTAINER="" NEXISCLAW_CONFIG_DIR="$config_dir" \
    NexisClaw config get gateway.bind 2>/dev/null || true
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v NexisClaw >/dev/null 2>&1; then
    echo "Warning: NexisClaw not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    NEXISCLAW_CONTAINER="" NEXISCLAW_CONFIG_DIR="$config_dir" \
      NexisClaw config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    NEXISCLAW_CONTAINER="" NEXISCLAW_CONFIG_DIR="$config_dir" \
      NexisClaw config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  NEXISCLAW_CONTAINER="" NEXISCLAW_CONFIG_DIR="$config_dir" \
    NexisClaw config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
CONFIG_JSON="$CONFIG_DIR/NexisClaw.json"
...
if [[ "$RUN_SETUP" == true ]]; then
  ...
  podman run --pull="$PODMAN_PULL" --rm -it \
    ...
    -e NPM_CONFIG_CACHE=/home/node/.NexisClaw/.npm \
...
echo "podman exec -it $CONTAINER_NAME NexisClaw devices approve --latest  # if pairing required"
