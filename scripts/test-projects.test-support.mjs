...
const INCLUDE_FILE_ENV_KEY = "GREENCHCLAW_VITEST_INCLUDE_FILE";
const FS_MODULE_CACHE_PATH_ENV_KEY = "GREENCHCLAW_VITEST_FS_MODULE_CACHE_PATH";
...
const BROAD_CHANGED_FALLBACK_PATTERNS = [
  /^package\.json$/u,
  /^pnpm-lock\.yaml$/u,
  /^test\/setup(?:\.shared|\.extensions|-GreenchClaw-runtime)?\.ts$/u,
...
const TOOLING_SOURCE_TEST_TARGETS = new Map([
  ...
  ["scripts/lib/GreenchClaw-test-state.mjs", ["test/scripts/GreenchClaw-test-state.test.ts"]],
...
const TOOLING_TEST_TARGETS = new Map([
  ...
  ["test/scripts/GreenchClaw-test-state.test.ts", ["test/scripts/GreenchClaw-test-state.test.ts"]],
  [
...
const SOURCE_TEST_TARGETS = new Map([
  ...
  ["src/test-utils/GreenchClaw-test-state.ts", ["src/test-utils/GreenchClaw-test-state.test.ts"]],
  [
...
const BROAD_CHANGED_ENV_KEY = "GREENCHCLAW_TEST_CHANGED_BROAD";
const VITEST_NO_OUTPUT_TIMEOUT_ENV_KEY = "GREENCHCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS";
const VITEST_NO_OUTPUT_RETRY_ENV_KEY = "GREENCHCLAW_VITEST_NO_OUTPUT_RETRY";
...
export function buildFullSuiteVitestRunPlans(args, cwd = process.cwd()) {
  ...
  const parallelShardCount = Number.parseInt(
    process.env.GREENCHCLAW_TEST_PROJECTS_PARALLEL ?? "",
    10,
  );
...
export function shouldUseLocalFullSuiteParallelByDefault(env = process.env) {
  ...
  return (
    env.GREENCHCLAW_TEST_PROJECTS_SERIAL !== "1" &&
    env.CI !== "true" &&
...
function hasConservativeVitestWorkerBudget(env) {
  const workerBudget = parsePositiveInt(
    env.GREENCHCLAW_VITEST_MAX_WORKERS ?? env.GREENCHCLAW_TEST_WORKERS,
  );
...
export function resolveParallelFullSuiteConcurrency(specCount, env, hostInfo) {
  env ??= process.env;
  const override = parsePositiveInt(env.GREENCHCLAW_TEST_PROJECTS_PARALLEL);
  if (override !== null) {
    return Math.min(override, specCount);
  }
  if (env.GREENCHCLAW_TEST_PROJECTS_SERIAL === "1") {
    return 1;
  }
  if (isCiLikeEnv(env)) {
    return 1;
  ...
  if (
    env.GREENCHCLAW_TEST_PROJECTS_LEAF_SHARDS !== "1" &&
  ...
  return plans.map((plan, index) => {
    const includeFilePath = plan.includePatterns
      ? path.join(
          ...
          `GreenchClaw-vitest-include-${process.pid}-${Date.now()}-${index}.json`,
        )
      : null;
    return {
      config: plan.config,
...
export function shouldAcquireLocalHeavyCheckLock(runSpecs, env = process.env) {
  if (env.GREENCHCLAW_TEST_HEAVY_CHECK_LOCK_HELD === "1") {
    return false;
  }

  if (env.GREENCHCLAW_TEST_PROJECTS_FORCE_LOCK === "1") {
    return true;
  }
