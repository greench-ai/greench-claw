#!/bin/bash
# Setup GreenchClaw Auth Management System
...
echo "=== GreenchClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'GreenchClaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  GreenchClaw message: Send warning via GreenchClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/GreenchClaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/GreenchClaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/GreenchClaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now GreenchClaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status GreenchClaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
