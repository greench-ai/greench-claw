...
function shouldCopyBundledPluginMetadata(id, env, buildablePluginDirs) {
  ...
  return env.GREENCHCLAW_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "GreenchClaw.plugin.json");
    ...
    if (packageJson.GreenchClaw && "extensions" in packageJson.GreenchClaw) {
      packageJson.GreenchClaw = {
        ...packageJson.GreenchClaw,
        extensions: rewritePackageExtensions(packageJson.GreenchClaw.extensions),
        ...(typeof packageJson.GreenchClaw.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.GreenchClaw.setupEntry) }
          : {}),
      };
    }
