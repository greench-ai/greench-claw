...
elif [[ -n "${GREENCHCLAW_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(GreenchClaw_live_collect_auth_dirs_from_csv "${GREENCHCLAW_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(GreenchClaw_live_collect_auth_files_from_csv "${GREENCHCLAW_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
CONTAINER_NODE_OPTIONS="${GREENCHCLAW_DOCKER_NODE_OPTIONS:-${NODE_OPTIONS:-}}"
if [[ -z "$(GreenchClaw_live_trim "$CONTAINER_NODE_OPTIONS")" ]]; then
...
DOCKER_RUN_ARGS=(docker run --rm -t \
  ...
  -e GREENCHCLAW_LIVE_TEST_QUIET="${GREENCHCLAW_LIVE_TEST_QUIET:-}" \
  -e GREENCHCLAW_LIVE_WRAPPER_HEARTBEAT_MS="${GREENCHCLAW_LIVE_WRAPPER_HEARTBEAT_MS:-}" \
  -e GREENCHCLAW_LIVE_REQUIRE_PROFILE_KEYS="${GREENCHCLAW_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e GREENCHCLAW_LIVE_GATEWAY_MODELS="${GREENCHCLAW_LIVE_GATEWAY_MODELS:-modern}" \
  -e GREENCHCLAW_LIVE_GATEWAY_PROVIDERS="${GREENCHCLAW_LIVE_GATEWAY_PROVIDERS:-}" \
  -e GREENCHCLAW_LIVE_GATEWAY_SMOKE="${GREENCHCLAW_LIVE_GATEWAY_SMOKE:-1}" \
  -e GREENCHCLAW_LIVE_GATEWAY_MAX_MODELS="${GREENCHCLAW_LIVE_GATEWAY_MAX_MODELS:-8}" \
  -e GREENCHCLAW_LIVE_GATEWAY_HEARTBEAT_MS="${GREENCHCLAW_LIVE_GATEWAY_HEARTBEAT_MS:-}" \
  -e GREENCHCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS="${GREENCHCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS:-45000}" \
  -e GREENCHCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${GREENCHCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-90000}" \
