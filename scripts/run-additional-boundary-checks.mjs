...
function resolveCliShardSpec(args, env) {
  ...
  return env.GREENCHCLAW_ADDITIONAL_BOUNDARY_SHARD ?? "";
}

if (import.meta.url === `file://${process.argv[1]}`) {
  const concurrency = resolveConcurrency(
    process.env.GREENCHCLAW_ADDITIONAL_BOUNDARY_CONCURRENCY ??
      process.env.GREENCHCLAW_EXTENSION_BOUNDARY_CONCURRENCY,
  );
