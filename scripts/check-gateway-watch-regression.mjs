...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  GREENCHCLAW_SKIP_ACPX_RUNTIME: "1",
  GREENCHCLAW_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$GREENCHCLAW_WATCH_PID_FILE"',
    'mkdir -p "$GREENCHCLAW_HOME/.GreenchClaw"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$GREENCHCLAW_HOME/.GreenchClaw/GreenchClaw.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    GREENCHCLAW_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    GREENCHCLAW_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-gateway-watch-"));
