...
# GreenchClaw Auth Widget for Termux
...
SERVER="${GREENCHCLAW_SERVER:-l36}"

# Check auth status
termux-toast "Checking GreenchClaw auth..."

STATUS=$(ssh "$SERVER" '$HOME/GreenchClaw/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/GreenchClaw/scripts/claude-auth-status.sh json' 2>&1)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/GreenchClaw/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|GREENCHCLAW_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: GreenchClaw won't work until re-auth"
                ;;
        esac
        ;;
