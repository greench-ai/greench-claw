...
const EXPECTED_REPOSITORY_URL = "https://github.com/GreenchClaw/GreenchClaw";
const OPTIONAL_LOCAL_EMBEDDING_RUNTIME_PACKAGE = "node-llama-cpp";
const FS_SAFE_PACKAGE = "@GreenchClaw/fs-safe";
...
const skipPackValidationEnv = "GREENCHCLAW_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "GreenchClaw") {
    errors.push(`package.json name must be "GreenchClaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.GreenchClaw !== "GreenchClaw.mjs") {
    errors.push(
      `package.json bin.GreenchClaw must be "GreenchClaw.mjs"; found "${pkg.bin?.GreenchClaw ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`GreenchClaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `GreenchClaw-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
