...
async function writeFakePlugin(params: {
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify(
      {
        name: "@GreenchClaw/tool-search-e2e-fixture",
        version: "0.0.0",
        type: "module",
        GreenchClaw: {
          extensions: ["./index.js"],
  ...
  try {
    ...
    const response = await fetchJson(`http://127.0.0.1:${gatewayPort}/v1/responses`, {
      ...
      headers: {
        ...
        "x-GreenchClaw-agent": "qa",
      },
      body: JSON.stringify({
        model: "GreenchClaw/qa",
        input: [
          {
            type: "message",
            role: "user",
...
async function main() {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-tool-search-"));
