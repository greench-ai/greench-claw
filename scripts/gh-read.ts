...
const APP_ID_ENV = "GREENCHCLAW_GH_READ_APP_ID";
const KEY_FILE_ENV = "GREENCHCLAW_GH_READ_PRIVATE_KEY_FILE";
const INSTALLATION_ID_ENV = "GREENCHCLAW_GH_READ_INSTALLATION_ID";
const PERMISSIONS_ENV = "GREENCHCLAW_GH_READ_PERMISSIONS";
...
async function githubJson<T>(
  ...
  const response = await fetch(`https://api.github.com${path}`, {
    ...
    headers: {
      ...
      "User-Agent": "GreenchClaw-gh-read",
...
async function main() {
  if (process.argv.length <= 2) {
    fail(
      "usage: scripts/gh-read <gh args...>\nset GREENCHCLAW_GH_READ_APP_ID and GREENCHCLAW_GH_READ_PRIVATE_KEY_FILE first",
    );
  }
