...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.GreenchClaw) ? packageJson.GreenchClaw : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.GreenchClaw?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.GreenchClaw?.channel?.id) || left.name;
    const rightId = trimString(right.GreenchClaw?.channel?.id) || right.name;
