...
- GREENCHCLAW_IOS_VERSION = exact canonical iOS version
- GREENCHCLAW_MARKETING_VERSION = short iOS/App Store version
- GREENCHCLAW_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
while IFS='=' read -r key value; do
  case "${key}" in
    GREENCHCLAW_IOS_VERSION)
      IOS_VERSION="${value}"
      ;;
    GREENCHCLAW_MARKETING_VERSION)
...
GREENCHCLAW_IOS_VERSION = ${IOS_VERSION}
GREENCHCLAW_MARKETING_VERSION = ${MARKETING_VERSION}
GREENCHCLAW_BUILD_VERSION = ${BUILD_NUMBER}
EOF
