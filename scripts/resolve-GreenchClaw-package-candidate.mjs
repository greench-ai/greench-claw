...
const DEFAULT_OUTPUT_NAME = "GreenchClaw-current.tgz";
export const GREENCHCLAW_PACKAGE_SPEC_RE =
  /^GreenchClaw@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-GreenchClaw-package-candidate.mjs --source <ref|npm|url|artifact> --output-dir <dir> [options]
...
export function validateGreenchClawPackageSpec(spec) {
  if (!GREENCHCLAW_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be GreenchClaw@alpha, GreenchClaw@beta, GreenchClaw@latest, or an exact GreenchClaw release version; got: ${spec}`,
    );
  }
}
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an GreenchClaw branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `GreenchClaw-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (!filename) {
    for (const line of packOutput.split(/\r?\n/u)) {
      ...
      if (/^GreenchClaw-.*\.tgz$/u.test(trimmed)) {
        filename = trimmed;
      }
    }
  }
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => /^GreenchClaw-.*\.tgz$/u.test(entry))
      .toSorted((a, b) => a.localeCompare(b))
      .at(-1);
  }
  if (!filename) {
    throw new Error(`npm pack produced no GreenchClaw tarball in ${outputDir}`);
  }
...
async function resolveCandidate(options) {
  ...
  try {
    if (options.source === "ref") {
      ...
      await run("node", [
        "scripts/package-GreenchClaw-for-docker.mjs",
        "--source-dir",
      ...
      validateGreenchClawPackageSpec(options.packageSpec);
  ...
  console.error(`Checking GreenchClaw package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-GreenchClaw-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
    `GreenchClaw package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
  ...
  if (pkg.name !== "GreenchClaw") {
    throw new Error(
      `package candidate must be named "GreenchClaw"; got: ${pkg.name || "<missing>"}`,
    );
  }
  if (!pkg.version) {
