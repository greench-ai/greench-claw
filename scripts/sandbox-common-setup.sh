...
BASE_IMAGE="${BASE_IMAGE:-GreenchClaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-GreenchClaw-sandbox-common:bookworm-slim}"
...
GREENCHCLAW_DOCKER_BUILD_USE_BUILDX="${GREENCHCLAW_DOCKER_BUILD_USE_BUILDX:-0}"
GREENCHCLAW_DOCKER_BUILD_CACHE_FROM="${GREENCHCLAW_DOCKER_BUILD_CACHE_FROM:-}"
GREENCHCLAW_DOCKER_BUILD_CACHE_TO="${GREENCHCLAW_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=GreenchClaw.sandbox=1)
NOTE
