...
const PUBLISHED_INSTALLER_BASE_URL = "https://GreenchClaw.ai";
...
export const CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS = parsePositiveIntegerEnv(
  "GREENCHCLAW_CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS",
  600,
);
const CROSS_OS_AGENT_TURN_OPTIONAL = parseBooleanEnv(
  "GREENCHCLAW_CROSS_OS_AGENT_TURN_OPTIONAL",
  true,
);
...
export function resolveProviderConfig(provider, env = process.env) {
  ...
  const providerEnvKey = `GREENCHCLAW_CROSS_OS_${provider.toUpperCase().replace(/[^A-Z0-9]+/gu, "_")}_MODEL`;
  const model =
    env[providerEnvKey]?.trim() || env.GREENCHCLAW_CROSS_OS_MODEL?.trim() || config.model;
  return { ...config, model };
}
...
const INSTALL_STAGE_DEBRIS_DIR_PATTERN = /^\.GreenchClaw-install-stage(?:-[^/]+)?$/iu;
...
export const CROSS_OS_COMMAND_HEARTBEAT_SECONDS = parsePositiveIntegerEnv(
  "GREENCHCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS",
  60,
);
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.GREENCHCLAW_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.GREENCHCLAW_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.GREENCHCLAW_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `GreenchClaw@${previousVersion}` : "GreenchClaw@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.GREENCHCLAW_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.GREENCHCLAW_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
...
async function runUpgradeLane(params) {
  ...
  try {
    ...
    await runTimedLanePhase(lane, "update", async () => {
      try {
        updateResult = await runGreenchClaw({
          lane,
          env: updateEnv,
          args: updateArgs,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        GREENCHCLAW_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `GreenchClaw-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".GreenchClaw");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    GREENCHCLAW_HOME: lane.homeDir,
    GREENCHCLAW_STATE_DIR: lane.stateDir,
    GREENCHCLAW_CONFIG_PATH: join(lane.stateDir, "GreenchClaw.json"),
...
export function buildRealUpdateEnv(env) {
  ...
  delete updateEnv.GREENCHCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function isRecoverableWindowsPackagedUpgradeTimeoutError(
  ...
  return (
    ...
    /[/\\]GreenchClaw\.mjs update --tag http:\/\/127\.0\.0\.1:\d+\/GreenchClaw[^/\s]*\.tgz --yes --json(?: --no-restart)? --timeout \d+/u.test(
      message,
    )
  );
}
...
export function resolveExplicitBaselineVersion(baselineSpec) {
  ...
  if (!trimmed || trimmed === "GreenchClaw@latest") {
    return "";
  }
  if (trimmed.startsWith("GreenchClaw@")) {
    return trimmed.slice("GreenchClaw@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "GreenchClaw@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view GreenchClaw@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'GreenchClaw.cmd'),
      (Join-Path $npmPrefix 'GreenchClaw.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command GreenchClaw -ErrorAction Stop
...
Write-Output "__GREENCHCLAW_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__GREENCHCLAW_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed GreenchClaw path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v GreenchClaw >/dev/null 2>&1",
    'printf "__GREENCHCLAW_PATH__=%s\\n" "$(command -v GreenchClaw)"',
    "GreenchClaw --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__GREENCHCLAW_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed GreenchClaw path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `GreenchClaw update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.GreenchClaw/logs/${params.lane.name}-discord-gateway.log`),
    });
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.GREENCHCLAW_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.GREENCHCLAW_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.GREENCHCLAW_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.GREENCHCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function buildInstalledBrowserOverrideImportProbeScript(
  runtimeModuleSpecifier = "GreenchClaw/plugin-sdk/plugin-runtime",
) {
  return `
import { existsSync } from "node:fs";
import { startLazyPluginServiceModule } from ${JSON.stringify(runtimeModuleSpecifier)};

const startedPath = process.env.GREENCHCLAW_BROWSER_OVERRIDE_STARTED_PATH;
const stoppedPath = process.env.GREENCHCLAW_BROWSER_OVERRIDE_STOPPED_PATH;

if (!process.env.GREENCHCLAW_BROWSER_CONTROL_MODULE) {
  throw new Error("Missing GREENCHCLAW_BROWSER_CONTROL_MODULE.");
}
...
export async function startBrowserControlService() {
  writeFileSync(process.env.GREENCHCLAW_BROWSER_OVERRIDE_STARTED_PATH, "started\\n", "utf8");
}

export async function stopBrowserControlService() {
  writeFileSync(process.env.GREENCHCLAW_BROWSER_OVERRIDE_STOPPED_PATH, "stopped\\n", "utf8");
}
`.trim();
}
...
async function runInstalledBrowserOverrideImportSmoke(params) {
  ...
  await runCommand(process.execPath, [probePath], {
    ...
    env: {
      ...
      GREENCHCLAW_BROWSER_OVERRIDE_STARTED_PATH: startedPath,
      GREENCHCLAW_BROWSER_OVERRIDE_STOPPED_PATH: stoppedPath,
    },
    logPath: params.logPath,
    timeoutMs: 60_000,
  });
...
async function resolveGatewayStatusArgs(lane, env, logPath) {
  const help = await runGreenchClaw({
    lane,
    env,
    args: ["gateway", "status", "--help"],
    logPath,
...
async function runAgentTurn(params) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    const sessionId = `cross-os-release-check-${params.label}-${Date.now()}-${attempt}`;
    try {
      const result = await runGreenchClaw({
        lane: params.lane,
        env: params.env,
        args: buildReleaseAgentTurnArgs(sessionId),
        logPath: params.logPath,
  ...
  try {
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (
          response.ok &&
          html.includes("<title>GreenchClaw Control</title>") &&
          html.includes("<GreenchClaw-app></GreenchClaw-app>")
        ) {
          logStream.write(
        ...
        logStream.write(
          `${new Date().toISOString()} dashboard-not-ready status=${response.status} title=${html.includes("<title>GreenchClaw Control</title>")} app=${html.includes("<GreenchClaw-app></GreenchClaw-app>")}\n`,
        );
      } catch (error) {
        logStream.write(
...
async function runGreenchClaw(params) {
...
function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "GreenchClaw.mjs");
}
