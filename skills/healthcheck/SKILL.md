---
name: healthcheck
description: Audit and harden hosts running GreenchClaw for SSH, firewall, updates, exposure, cron checks, and risk posture.
---

# GreenchClaw Host Hardening

## Overview

Assess and harden the host running GreenchClaw, then align it to a user-defined risk tolerance without breaking access. Use GreenchClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim GreenchClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. GreenchClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if GreenchClaw can access sensitive data.
...
### 2) Run GreenchClaw security audits (read-only)

As part of the default read-only checks, run `GreenchClaw security audit --deep`. Only offer alternatives if the user requests them:

1. `GreenchClaw security audit` (faster, non-probing)
2. `GreenchClaw security audit --json` (structured output)

Offer to apply GreenchClaw safe defaults (numbered):

1. `GreenchClaw security audit --fix`

Be explicit that `--fix` only tightens GreenchClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check GreenchClaw version/update status (read-only)

As part of the default read-only checks, run `GreenchClaw update status`.
...
- Credential hygiene notes (location of GreenchClaw creds, prefer disk encryption)
...
- GreenchClaw security audit (re-run)
...
After GreenchClaw install or first hardening pass, run at least one baseline audit and version check:

- `GreenchClaw security audit`
- `GreenchClaw security audit --deep`
- `GreenchClaw update status`

Ongoing monitoring is recommended. Use the GreenchClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `GreenchClaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `GreenchClaw update status`
...
Before creating, `GreenchClaw cron list` and match on exact `name`. If found, `GreenchClaw cron edit <id> ...`.
If not found, `GreenchClaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `GreenchClaw update status` (preferred for source checkouts and channels)
2. `npm view GreenchClaw version` (published npm version)

## GreenchClaw command accuracy

Use only supported commands and flags:

- `GreenchClaw security audit [--deep] [--fix] [--json]`
- `GreenchClaw status` / `GreenchClaw status --deep`
- `GreenchClaw health --json`
- `GreenchClaw update status`
- `GreenchClaw cron add|list|runs|run`

Do not invent CLI flags or imply GreenchClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by GreenchClaw compaction:
