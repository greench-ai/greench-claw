...
type GreenchClawTestStateLayout = "home" | "state-only" | "split";

type GreenchClawTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
export type GreenchClawTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: GreenchClawTestStateLayout;
  scenario?: GreenchClawTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
export type GreenchClawTestState = {
  root: string;
  home: string;
  stateDir: string;
  configPath: string;
...
const DEFAULT_PREFIX = "GreenchClaw-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
  ...
  "PI_CODING_AGENT_DIR",
  "GREENCHCLAW_SERVICE_REPAIR_POLICY",
] as const;
...
function resolveLayout(
  ...
  if (layout === "home") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(stateDir, "GreenchClaw.json"),
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "GreenchClaw.json"),
...
function scenarioConfig(options: GreenchClawTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "GreenchClaw-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: GreenchClawTestStateOptions): Record<string, string | undefined> {
...
function buildEnvVars(params: {
  ...
  const agentDirEnv =
    params.agentEnv === "main"
      ...
      : {
          GREENCHCLAW_AGENT_DIR: undefined,
...
export async function createGreenchClawTestState(
  options: GreenchClawTestStateOptions = {},
): Promise<GreenchClawTestState> {
  ...
  const state: GreenchClawTestState = {
    root,
    ...paths,
    env,
    envVars,
...
export async function withGreenchClawTestState<T>(
  options: GreenchClawTestStateOptions,
  fn: (state: GreenchClawTestState) => Promise<T>,
): Promise<T> {
  const state = await createGreenchClawTestState(options);
  try {
