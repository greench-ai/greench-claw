...
import { resolveStateDir, type GreenchClawConfig } from "../config/config.js";
...
export async function runSecretsAudit(
  ...
  const config = snapshot.valid ? snapshot.config : ({} as GreenchClawConfig);
  let resolution = {
    refsChecked: 0,
    skippedExecRefs: 0,
