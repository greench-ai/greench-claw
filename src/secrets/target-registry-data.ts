...
function createPluginGreenchClawConfigSecretTargetEntry(
  pluginId: string,
  configPath: string,
  ...
  for (const record of bundledPlugins) {
    for (const config of WEB_PROVIDER_SECRET_CONFIGS) {
      if (
        ...
        entries.push(createPluginGreenchClawConfigSecretTargetEntry(record.id, config.configPath));
      }
    }
  }
...
function listBundledPluginConfigSecretTargetRegistryEntries(
  ...
  for (const record of bundledPlugins) {
    ...
    for (const secretInput of secretInputs) {
      const entry = createPluginGreenchClawConfigSecretTargetEntry(record.id, secretInput.path);
...
export function getSourceSecretTargetRegistry(): SecretTargetRegistryEntry[] {
  return loadSecretTargetRegistryFromPluginMetadata({
    env: {
      ...process.env,
      GREENCHCLAW_BUNDLED_PLUGINS_DIR: process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR ?? "extensions",
    },
