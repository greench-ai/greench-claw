...
import {
  ...
  GREENCHCLAW_NODE_EXTRA_CA_CERTS_READY,
  GREENCHCLAW_NODE_OPTIONS_READY,
...
describe("buildCliRespawnPlan", () => {
  ...
  it("adds NODE_EXTRA_CA_CERTS and warning suppression in one respawn", () => {
    ...
    expect(respawnPlan.env[GREENCHCLAW_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
    expect(respawnPlan.env[GREENCHCLAW_NODE_OPTIONS_READY]).toBe("1");
  });
  ...
  it.each(["tui", "terminal", "chat"] as const)(
    ...
    (command) => {
      ...
      const respawnPlan = expectCliRespawnPlan(plan);
      expect(respawnPlan.argv).toEqual(["GreenchClaw", command]);
      expect(respawnPlan.env.NODE_EXTRA_CA_CERTS).toBe("/etc/ssl/certs/ca-certificates.crt");
      expect(respawnPlan.env[GREENCHCLAW_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
      expect(respawnPlan.env[GREENCHCLAW_NODE_OPTIONS_READY]).toBeUndefined();
    },
  );
  ...
  it("does not respawn interactive commands for warning suppression only", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "GreenchClaw", "tui"],
        env: { [GREENCHCLAW_NODE_EXTRA_CA_CERTS_READY]: "1" },
        execArgv: [],
  ...
  it("returns null when both respawn guards are already satisfied", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "GreenchClaw", "status"],
        env: {
          [GREENCHCLAW_NODE_EXTRA_CA_CERTS_READY]: "1",
          [GREENCHCLAW_NODE_OPTIONS_READY]: "1",
        },
  ...
  it("respawns Volta shims through node so the shim is not called directly", () => {
    const plan = buildCliRespawnPlan({
      argv: ["/home/alice/.volta/bin/volta-shim", "/usr/local/bin/GreenchClaw", "status"],
    ...
    runCliRespawnPlan(
      {
        ...
        argv: ["/repo/GreenchClaw/dist/entry.js", "status"],
        env: { GREENCHCLAW_NODE_OPTIONS_READY: "1" },
      },
      {
  ...
  it("force-kills a signaled respawn child that does not exit", () => {
    ...
    try {
      runCliRespawnPlan(
        {
          command: "/usr/bin/node",
          argv: ["/repo/GreenchClaw/dist/entry.js", "tui"],
          env: {},
        },
        {
          spawn: spawn as unknown as typeof import("node:child_process").spawn,
