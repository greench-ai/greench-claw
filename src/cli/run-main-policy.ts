...
export function shouldUseRootHelpFastPath(
  ...
  const invocation = resolveCliArgvInvocation(argv);
  return (
    env.GREENCHCLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH !== "1" &&
...
export function shouldUseBrowserHelpFastPath(
  ...
  if (env.GREENCHCLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1") {
    return false;
  }
  const invocation = resolveCliArgvInvocation(argv);
  return (
  ...
  if (parentPluginId) {
    if (allow.length > 0 && !allow.includes(parentPluginId)) {
      if (parentPluginId === normalizedPluginId) {
        return (
          `The \`GreenchClaw ${normalizedPluginId}\` command is unavailable because ` +
    ...
    if (
      ...
      return (
        `The \`GreenchClaw ${normalizedPluginId}\` command is provided by the ` +
        `"${parentPluginId}" plugin, but that bundled plugin is disabled by default. Run ` +
        `\`GreenchClaw plugins enable ${parentPluginId}\` to enable that CLI surface.`
      );
    }
    if (commandAlias.kind === "runtime-slash") {
      const cliHint = commandAlias.cliCommand
        ? `Use \`GreenchClaw ${commandAlias.cliCommand}\` for related CLI operations, or `
        : "Use ";
      return (
    ...
    if (ownerEnabled) {
      ...
      if (toolOwner.availability === "manifest-only") {
        return (
          ...
          "Run `GreenchClaw --help` to see available CLI subcommands."
        );
      }
      return (
