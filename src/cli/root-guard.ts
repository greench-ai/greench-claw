 ...
 * - Separate state dir (/root/.GreenchClaw/ vs /home/<user>/.GreenchClaw/)
...
export function assertNotRoot(env: NodeJS.ProcessEnv = process.env): void {
  ...
  if (
    env.GREENCHCLAW_ALLOW_ROOT === "1" ||
    (env.GREENCHCLAW_CLI_CONTAINER_BYPASS === "1" && env.GREENCHCLAW_CONTAINER_HINT)
  ) {
    return;
  }
  process.stderr.write(
    "[GreenchClaw] Refusing to run as root.\n" +
      "\n" +
      "Why this is blocked:\n" +
      "  - A separate state directory under /root/.GreenchClaw/ instead of the service user's\n" +
      "  - Conflicting systemd user services that race on port 18789\n" +
      "  - Root-owned files in the service user's state dir (EACCES errors)\n" +
      "\n" +
      "What to do:\n" +
      "  - Re-run as the service user: sudo -u <service-user> -H GreenchClaw ...\n" +
      "  - Or switch shells first: su - <service-user>\n" +
      "\n" +
      "Intentional container/CI run only:\n" +
      "  GREENCHCLAW_ALLOW_ROOT=1 GreenchClaw ...\n",
  );
  process.exit(1);
}
