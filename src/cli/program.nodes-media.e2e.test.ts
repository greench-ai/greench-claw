...
describe("cli program (nodes media)", () => {
  ...
  it("runs nodes camera snap and prints two MEDIA paths", async () => {
    ...
    expect(mediaPaths[0]).toContain("GreenchClaw-camera-snap-");
    expect(mediaPaths[1]).toContain("GreenchClaw-camera-snap-");
  ...
  it("runs nodes camera clip and prints one MEDIA path", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /GreenchClaw-camera-clip-front-.*\.mp4$/,
    });
  });
    ...
    it.each([
      {
        ...
        expectedPathPattern: /GreenchClaw-camera-snap-front-.*\.jpg$/,
      },
      {
