...
describe("completion-cli write-state", () => {
  ...
  it("keeps completion cache generation alive when a subcli fails to register", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-completion-state-"));
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-completion-home-"));
