...
const STEP_LABELS: Record<string, string> = {
  ...
  "GreenchClaw doctor entry": "Checking doctor entrypoint",
  "GreenchClaw doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g GreenchClaw@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g GreenchClaw@latest --omit=optional");
  }
