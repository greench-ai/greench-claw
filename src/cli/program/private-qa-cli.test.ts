...
describe("private-qa-cli", () => {
  ...
  afterEach(() => {
    ...
    if (originalPrivateQaCli === undefined) {
      ...
      process.env.GREENCHCLAW_ENABLE_PRIVATE_QA_CLI = originalPrivateQaCli;
    }
  });

  it("loads the private QA CLI from a source checkout path", async () => {
    process.env.GREENCHCLAW_ENABLE_PRIVATE_QA_CLI = "1";
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-private-qa-source-"));
  ...
  it("rejects non-source package roots even when private QA is enabled", () => {
    process.env.GREENCHCLAW_ENABLE_PRIVATE_QA_CLI = "1";
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-private-qa-"));
    tempDirs.push(root);
    fs.writeFileSync(
    ...
    ).toThrow("Private QA CLI is only available from an GreenchClaw source checkout.");
  ...
  it("rejects when the private QA env flag is disabled", () => {
    ...
    expect(() => loadPrivateQaCliModule({ importModule })).toThrow(
      "Private QA CLI is only available from an GreenchClaw source checkout.",
    );
    expect(importModule).not.toHaveBeenCalled();
  });
});
