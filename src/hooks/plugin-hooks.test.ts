...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.GREENCHCLAW_BUNDLED_HOOKS_DIR;
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.GREENCHCLAW_BUNDLED_HOOKS_DIR;
    } else {
      process.env.GREENCHCLAW_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".GreenchClaw", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): GreenchClawConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entry.hook.source).toBe("GreenchClaw-plugin");
  ...
  it("does not treat Claude hooks.json bundles as GreenchClaw hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".GreenchClaw", "extensions", "claude-bundle");
