...
const SUPPORT_EXPORT_PREFIX = "GreenchClaw-diagnostics-";
...
function readStabilityBundle(
  ...
  if (target === false) {
    return { status: "missing", dir: "$GREENCHCLAW_STATE_DIR/logs/stability" };
  }
...
function renderSummary(params: {
  ...
  return [
    "# GreenchClaw Diagnostics Export",
    "",
    ...
    "- `logs/GreenchClaw-sanitized.jsonl`: sanitized log summaries and metadata",
