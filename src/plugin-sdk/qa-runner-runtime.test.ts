...
describe("plugin-sdk qa-runner-runtime", () => {
  ...
  it("loads bundled plugin test APIs with the private QA source tree override", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "GreenchClaw-qa-test-api-root-");
    resolveGreenchClawPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(testApiCall?.env?.GREENCHCLAW_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(testApiCall?.env?.GREENCHCLAW_BUNDLED_PLUGINS_DIR).toBe(
  ...
  it("prefers the source bundled tree for private qa discovery in repo checkouts", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "GreenchClaw-qa-runner-root-");
    resolveGreenchClawPackageRootSync.mockReturnValue(sourceRoot);
    ...
    const manifestCall = loadPluginManifestRegistry.mock.calls.at(0)?.[0] as
      ...
      | undefined;
    expect(manifestCall?.env?.GREENCHCLAW_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(manifestCall?.env?.GREENCHCLAW_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
    ...
    expect(publicSurfaceCall?.env?.GREENCHCLAW_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(publicSurfaceCall?.env?.GREENCHCLAW_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
  });
  ...
  it("fails when runtime registrations include an undeclared command", async () => {
    ...
    expect(() => module.listQaRunnerCliContributions()).toThrow(
      'QA runner plugin "qa-matrix" exported "extra" from runtime-api.js but did not declare it in GreenchClaw.plugin.json',
    );
  });
});
