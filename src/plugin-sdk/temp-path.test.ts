...
function expectPathInsideTmpRoot(resultPath: string) {
  const tmpRoot = fsSync.realpathSync(resolvePreferredGreenchClawTmpDir());
...
describe("withTempDownloadPath", () => {
  it.each([
    ...
    if (expectedBasename) {
      ...
      expect(capturedPath).toContain(path.join(resolvePreferredGreenchClawTmpDir(), "line-media-"));
    }
    if (expectCleanup) {
