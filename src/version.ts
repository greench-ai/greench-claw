...
declare const __GREENCHCLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "GreenchClaw";
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["GREENCHCLAW_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["GREENCHCLAW_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["GREENCHCLAW_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.GREENCHCLAW_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current GreenchClaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion:
    typeof __GREENCHCLAW_VERSION__ === "string" ? __GREENCHCLAW_VERSION__ : undefined,
  bundledVersion: process.env.GREENCHCLAW_BUNDLED_VERSION,
});
