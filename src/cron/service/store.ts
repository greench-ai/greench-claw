  ...
  for (const [index, job] of jobs.entries()) {
    ...
    try {
      ...
      state.deps.log.warn(
        ...
        "cron: job has invalid persisted sessionTarget; run GreenchClaw doctor --fix to repair",
      );
    }
    const hydrated =
    ...
    if (legacyJobIdIssue) {
      ...
      state.deps.log.warn(
        ...
        "cron: job used legacy jobId field; normalized id in memory (run GreenchClaw doctor --fix to persist canonical shape)",
      );
    }
