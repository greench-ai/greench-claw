...
function makeStateDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-plugin-index-records-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("plugin index install records store", () => {
    ...
    await writePersistedInstalledPluginIndexInstallRecords(
      {
        twitch: {
          source: "npm",
          spec: "@GreenchClaw/plugin-twitch@1.0.0",
          installPath: "plugins/npm/@GreenchClaw/plugin-twitch",
        },
      },
      {
        stateDir,
  ...
  it("recovers managed npm plugin records when the persisted ledger is empty", async () => {
    ...
    expectRecordFields(loaded.codex, {
      source: "npm",
      spec: "@GreenchClaw/codex@2026.5.2",
    ...
    expectRecordFields(loaded.discord, {
      ...
      version: "2026.5.2",
      resolvedName: "@GreenchClaw/discord",
      resolvedVersion: "2026.5.2",
