...
describe("startLazyPluginServiceModule", () => {
  afterEach(() => {
    delete process.env.GREENCHCLAW_LAZY_SERVICE_SKIP;
    delete process.env.GREENCHCLAW_LAZY_SERVICE_OVERRIDE;
  });
  ...
  it("uses the override module when configured", async () => {
    process.env.GREENCHCLAW_LAZY_SERVICE_OVERRIDE = "virtual:service";
    const start = createAsyncHookMock();
    const loadOverrideModule = vi.fn(async () => ({ startOverride: start }));

    await expectLifecycleStarted({
      overrideEnvVar: "GREENCHCLAW_LAZY_SERVICE_OVERRIDE",
  ...
  it("leaves caller-supplied override loaders responsible for their own specifiers", async () => {
    process.env.GREENCHCLAW_LAZY_SERVICE_OVERRIDE = "C:\\Users\\alice\\browser-service.mjs";
    const platformSpy = vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    const start = createAsyncHookMock();
    const loadOverrideModule = vi.fn(async () => ({ startOverride: start }));
  ...
  it("surfaces override validation failures", async () => {
    process.env.GREENCHCLAW_LAZY_SERVICE_OVERRIDE = "data:text/javascript,boom";
