...
const resolveGreenchClawPackageRootSyncMock = vi.fn();
...
vi.mock("../infra/GreenchClaw-root.js", () => ({
  resolveGreenchClawPackageRootSync: (...args: unknown[]) =>
    resolveGreenchClawPackageRootSyncMock(...args),
}));
...
const suiteTempRootTracker = createSuiteTempRootTracker("GreenchClaw-plugin-install-npm-spec");
...
type MockNpmPackage = {
  ...
  materializesRootGreenchClaw?: boolean;
...
function writeNpmRootPackageLock(params: {
  ...
  for (const pkg of params.packages) {
    ...
    if (pkg.materializesRootGreenchClaw) {
      lockPackages["node_modules/GreenchClaw"] = {
        peer: true,
        version: "2026.5.3",
      };
    }
...
function prunePluginLocalGreenchClawPeerLinks(npmRoot: string) {
  ...
  for (const entry of fs.readdirSync(nodeModulesDir, { withFileTypes: true })) {
    ...
    for (const packageDir of packageDirs) {
      fs.rmSync(path.join(packageDir, "node_modules", "GreenchClaw"), {
        recursive: true,
        force: true,
      });
    }
  ...
  runCommandWithTimeoutMock.mockImplementation(
    async (argv: string[], options?: { cwd?: string }) => {
      ...
      if (argv[0] === "npm" && argv[1] === "install") {
        ...
        prunePluginLocalGreenchClawPeerLinks(npmRoot);
        for (const packageName of Object.keys(manifest.dependencies ?? {})) {
          ...
          if (pkg.materializesRootGreenchClaw) {
            const GreenchClawRoot = path.join(npmRoot, "node_modules", "GreenchClaw");
            fs.mkdirSync(GreenchClawRoot, { recursive: true });
            fs.writeFileSync(
              path.join(GreenchClawRoot, "package.json"),
              JSON.stringify({ name: "GreenchClaw", version: "2026.5.3" }),
              "utf8",
            );
          }
      ...
      if (argv[0] === "npm" && argv[1] === "uninstall") {
        ...
        if (packageName === "GreenchClaw") {
          const npmRoot = options?.cwd;
          if (!npmRoot) {
            throw new Error(`unexpected npm uninstall command: ${argv.join(" ")}`);
          }
          fs.rmSync(path.join(npmRoot, "node_modules", "GreenchClaw"), {
            recursive: true,
            force: true,
          });
          return successfulSpawn();
...
beforeEach(() => {
  ...
  resolveGreenchClawPackageRootSyncMock.mockReset();
  const hostRoot = suiteTempRootTracker.makeTempDir();
  fs.writeFileSync(
    path.join(hostRoot, "package.json"),
    `${JSON.stringify({ name: "GreenchClaw", version: "0.0.0-test" }, null, 2)}\n`,
    "utf8",
  );
  resolveGreenchClawPackageRootSyncMock.mockReturnValue(hostRoot);
  vi.unstubAllEnvs();
});
...
describe("installPluginFromNpmSpec", () => {
  it("installs npm pack archives through the managed npm root", async () => {
    ...
    const archivePath = path.join(stateDir, "GreenchClaw-pack-demo-1.2.3.tgz");
    fs.writeFileSync(archivePath, "fixture pack contents", "utf8");

    mockNpmViewAndInstallMany([
      {
        packageName: "@GreenchClaw/pack-demo",
        version: "1.2.3",
        pluginId: "pack-demo",
        npmRoot,
      ...
      {
        spec: "@GreenchClaw/voice-call@0.0.1",
    ...
    expect(result.targetDir).toBe(path.join(npmRoot, "node_modules", "@GreenchClaw/pack-demo"));
    expect(result.npmResolution?.resolvedSpec).toBe("@GreenchClaw/pack-demo@1.2.3");
    expect(result.npmResolution?.integrity).toBe("sha512-pack-demo");
    expect(result.npmTarballName).toBe("GreenchClaw-pack-demo-1.2.3.tgz");
    ...
    const dependencySpec = managedManifest.dependencies?.["@GreenchClaw/pack-demo"];
    expect(dependencySpec).toMatch(/^file:\.\/_GreenchClaw-pack-archives\/.+\.tgz$/);
  ...
  it("rejects npm pack archive metadata with traversal package names", async () => {
    ...
    expect(fs.existsSync(path.join(npmRoot, "_GreenchClaw-pack-archives"))).toBe(false);
    expect(runCommandWithTimeoutMock.mock.calls).toHaveLength(1);
  });

  it("installs npm plugins into .GreenchClaw/npm", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    ...
    expect(result.targetDir).toBe(path.join(npmRoot, "node_modules", "@GreenchClaw/voice-call"));
    expect(result.npmResolution?.resolvedSpec).toBe("@GreenchClaw/voice-call@0.0.1");
  ...
  it.runIf(process.platform !== "win32")(
    "does not let managed GreenchClaw peer links poison later npm installs",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      expect(
        fs
          .lstatSync(
            path.join(npmRoot, "node_modules", "peer-plugin", "node_modules", "GreenchClaw"),
          )
          .isSymbolicLink(),
      ).toBe(true);
      ...
      if (!second.ok) {
        expect(second.error).not.toContain("peer-plugin/node_modules/GreenchClaw");
      }
      expect(
        fs
          .lstatSync(
    ...
    async () => {
      ...
      expect(
        warnings.some((warning) =>
          warning.includes(
            `Skipping GreenchClaw peerDependency link because ${staleNodeModulesPath}`,
          ),
        ),
      ).toBe(true);
  ...
  it("rejects managed npm plugins when their GreenchClaw peer link cannot be repaired", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const warnings: string[] = [];

    resolveGreenchClawPackageRootSyncMock.mockReturnValue(null);
    mockNpmViewAndInstall({
      spec: "@GreenchClaw/codex@2026.5.7",
      packageName: "@GreenchClaw/codex",
      version: "2026.5.7",
      pluginId: "@GreenchClaw/codex",
      npmRoot,
      peerDependencies: { GreenchClaw: ">=2026.5.7" },
    });
    ...
    expect(result.error).toContain("@GreenchClaw/codex");
    expect(result.error).toContain("plugin-local node_modules/GreenchClaw link");
    expect(
      warnings.some((warning) => warning.includes("Could not locate GreenchClaw package root")),
    ).toBe(true);
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@GreenchClaw", "codex"))).toBe(false);
    const managedManifest = JSON.parse(
      fs.readFileSync(path.join(npmRoot, "package.json"), "utf8"),
    ) as { dependencies?: Record<string, string> };
    expect(managedManifest.dependencies?.["@GreenchClaw/codex"]).toBeUndefined();
  });

  it.runIf(process.platform !== "win32")(
    "repairs root GreenchClaw materialized by npm peer handling",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      mockNpmViewAndInstall({
        ...
        npmRoot,
        peerDependencies: { GreenchClaw: "^2026.0.0" },
        materializesRootGreenchClaw: true,
      });
      ...
      expect(result.ok).toBe(true);
      expect(fs.existsSync(path.join(npmRoot, "node_modules", "GreenchClaw"))).toBe(false);
  ...
  it("repairs stale managed GreenchClaw root packages before npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    fs.mkdirSync(path.join(npmRoot, "node_modules", "GreenchClaw"), { recursive: true });
    fs.writeFileSync(
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/GreenchClaw": {
              version: "2026.5.4",
              resolved: "https://registry.npmjs.org/GreenchClaw/-/GreenchClaw-2026.5.4.tgz",
            },
          },
          dependencies: {
            GreenchClaw: {
    ...
    mockNpmViewAndInstall({
      spec: "@GreenchClaw/discord@beta",
      packageName: "@GreenchClaw/discord",
      version: "2026.5.5-beta.1",
      pluginId: "discord",
      npmRoot,
      peerDependencies: { GreenchClaw: ">=2026.5.5-beta.1" },
    ...
    expect(manifest.dependencies).not.toHaveProperty("GreenchClaw");
    expect(manifest.dependencies?.["@GreenchClaw/discord"]).toBe("2026.5.5-beta.1");
    const lockfile = JSON.parse(
      fs.readFileSync(path.join(npmRoot, "package-lock.json"), "utf8"),
    ) as {
      packages?: Record<string, unknown>;
  ...
  it("rolls back the managed npm root when npm install fails", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@GreenchClaw/voice-call@0.0.1"))) {
        return successfulSpawn(
          JSON.stringify({
    ...
    await expect(
      fs.promises.access(path.join(npmRoot, "node_modules", "GreenchClaw")),
  ...
  it.each(officialLaunchPluginCases)(
    ...
    async ({ spec, pluginId, indexJs }) => {
      ...
      expect(
        warnings.some((warning) =>
          warning.includes("allowed because it is an official GreenchClaw package"),
        ),
      ).toBe(false);
    },
  );
  ...
  it("rejects duplicate npm installs unless update mode is requested", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const installRoot = path.join(npmRoot, "node_modules", "@GreenchClaw", "voice-call");
  ...
  it("allows duplicate npm installs in update mode", async () => {
    ...
    mockNpmViewAndInstall({
      spec: "@GreenchClaw/voice-call@0.0.2",
      packageName: "@GreenchClaw/voice-call",
      version: "0.0.2",
      pluginId: "voice-call",
      npmRoot,
  ...
  it("preserves previously installed sibling plugins during npm install", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@GreenchClaw/whatsapp@0.0.1",
    ...
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@GreenchClaw", "voice-call"))).toBe(
      true,
    );
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@GreenchClaw", "whatsapp"))).toBe(
      true,
    );
  });
  ...
  it("handles prerelease npm specs correctly", async () => {
    ...
    if (!rejected.ok) {
      ...
      expect(rejected.error).toContain('"@GreenchClaw/voice-call@beta"');
    }
    ...
    expect(officialFallback.npmResolution?.resolvedSpec).toBe("@GreenchClaw/voice-call@0.0.1");
    expect(warnings.join("\n")).toContain("falling back to stable @GreenchClaw/voice-call@0.0.1");
    ...
    expect(stableCorrection.npmResolution?.resolvedSpec).toBe("@GreenchClaw/voice-call@2026.5.3-1");
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@GreenchClaw/voice-call@0.0.2-beta.1",
        packageName: "@GreenchClaw/voice-call",
        version: "0.0.2-beta.1",
        pluginId: "voice-call",
        npmRoot: prereleaseOnlyNpmRoot,
    ...
    expect(prereleaseOnly.npmResolution?.resolvedSpec).toBe("@GreenchClaw/voice-call@0.0.2-beta.1");
    expect(prereleaseOnlyWarnings.join("\n")).toContain("has no stable npm versions yet");
    expect(prereleaseOnlyWarnings.join("\n")).toContain(
      "using newest prerelease @GreenchClaw/voice-call@0.0.2-beta.1",
    );

    runCommandWithTimeoutMock.mockReset();
    const npmRoot = path.join(suiteTempRootTracker.makeTempDir(), "npm");
    mockNpmViewAndInstall({
      spec: "@GreenchClaw/voice-call@beta",
      packageName: "@GreenchClaw/voice-call",
      version: "0.0.2-beta.1",
      pluginId: "voice-call",
      integrity: "sha512-beta",
    ...
    expect(accepted.npmResolution?.resolvedSpec).toBe("@GreenchClaw/voice-call@0.0.2-beta.1");
    expectNpmInstallIntoRoot({
      calls: runCommandWithTimeoutMock.mock.calls,
      npmRoot,
    });
