...
vi.mock("./loader.js", () => ({
  loadGreenchClawPlugins: (params: unknown) => loadGreenchClawPluginsMock(params),
...
function setRegistry(entries: MockRegistryToolEntry[]) {
  ...
  loadGreenchClawPluginsMock.mockReturnValue(registry);
...
function expectLoaderSelectedOnlyPluginIds(expectedPluginIds: readonly string[]) {
  const selectedPluginIds = loadGreenchClawPluginsMock.mock.calls.map(
...
describe("resolvePluginTools optional tools", () => {
  ...
  beforeEach(() => {
    ...
    resolveRuntimePluginRegistryMock.mockImplementation((params) =>
      loadGreenchClawPluginsMock(params),
    );
  ...
  it("does not load plugin-owned tools whose manifest metadata has no available signal", () => {
    ...
    loadGreenchClawPluginsMock.mockImplementation((params) =>
  ...
  it("does not reuse a partial active registry for wildcard-selected plugin tools", () => {
    ...
    loadGreenchClawPluginsMock.mockReturnValue(fullRegistry);
    ...
    const loaderParams = mockCallParams(loadGreenchClawPluginsMock) as {
      activate?: unknown;
      cache?: unknown;
  ...
  it("uses the fresh cold-loaded registry for diagnostics when partial active registries remain incomplete", () => {
    ...
    loadGreenchClawPluginsMock.mockReturnValue(freshRegistry);
  ...
  it("does not reuse a pinned gateway registry for manifest-unavailable tools", () => {
    ...
    loadGreenchClawPluginsMock.mockReturnValue({ tools: [], diagnostics: [] });
    ...
    const config = {
      ...
      secrets: {
        providers: {
          vault: {
            source: "file",
            path: "/tmp/GreenchClaw-secrets.json",
            mode: "json",
          },
        },
      },
  ...
  it("cold-loads default plugin tools when alsoAllow opts into optional tools", () => {
    ...
    loadGreenchClawPluginsMock.mockReturnValue(
  ...
  it("does not cold-load unrelated manifest-optional plugins when alsoAllow opts into one optional tool", () => {
    ...
    loadGreenchClawPluginsMock.mockReturnValue(createToolRegistry([explicitOptionalEntry]));
    installToolManifestSnapshots({
      config,
      plugins: [
        {
  ...
  it.each([
    {
      ...
      params: {
        env: { GREENCHCLAW_HOME: "/srv/GreenchClaw-home" } as NodeJS.ProcessEnv,
        toolAllowlist: ["optional_tool"],
      },
      expectedLoaderCall: {
        env: { GREENCHCLAW_HOME: "/srv/GreenchClaw-home" },
      },
    },
    {
  ...
  it("loads a standalone registry when cached runtime registries lack matching tool entries", () => {
    ...
    resolveRuntimePluginRegistryMock.mockReturnValue(undefined);
    loadGreenchClawPluginsMock.mockReturnValue(loadedRegistry);
  ...
  it("adds enabled non-startup tool plugins to the active tool runtime scope", () => {
    ...
    setActivePluginRegistry(activeRegistry as never, "gateway-startup", "gateway-bindable", "/tmp");
    resolveRuntimePluginRegistryMock.mockReturnValue(activeRegistry);
    loadGreenchClawPluginsMock.mockReturnValue(createToolRegistry([]));
