...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @GreenchClaw/voice-call -> voice-call).
...
function addCandidate(params: {
  ...
  params.candidates.push({
    ...
    rootDir: resolvedRoot,
    origin: params.origin,
    format: params.format ?? "GreenchClaw",
...
export function discoverGreenchClawPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ...
  const scopedResult = tracePluginLifecyclePhase(
    ...
    () => {
      ...
      for (const extraPath of extra) {
        ...
        if (bundledAlias) {
          result.diagnostics.push({
            level: "warn",
            source: trimmed,
            message: `ignored plugins.load.paths entry that points at GreenchClaw's ${bundledAlias.kind} bundled plugin directory; remove this redundant path or run GreenchClaw doctor --fix`,
          });
          continue;
        }
        discoverFromPath({
      ...
      if (roots.workspace && workspaceRoot && !workspaceMatchesBundledRoot) {
        // Keep workspace auto-discovery constrained to the GreenchClaw extensions root.
