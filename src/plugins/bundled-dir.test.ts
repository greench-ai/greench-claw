...
const originalBundledDir = process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR;
...
function createGreenchClawRoot(params: {
  prefix: string;
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: "GreenchClaw" }, null, 2)}\n`,
    "utf8",
  );
  return repoRoot;
}
...
function seedBundledPluginTree(rootDir: string, relativeDir: string, pluginId = "discord") {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: `@GreenchClaw/${pluginId}` }, null, 2)}\n`,
    "utf8",
  );
  fs.writeFileSync(
...
function expectResolvedBundledDir(params: {
  ...
  if (params.bundledDirOverride === undefined) {
    ...
    process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR = params.bundledDirOverride;
  }
  if (params.disableBundledPlugins === undefined) {
    delete process.env.GREENCHCLAW_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.GREENCHCLAW_DISABLE_BUNDLED_PLUGINS = params.disableBundledPlugins;
  }
...
function expectResolvedBundledDirFromRoot(params: {
  ...
  expectResolvedBundledDir({
    ...
    argv1: params.argv1 ?? path.join(params.repoRoot, "GreenchClaw.mjs"),
...
afterEach(() => {
  ...
  if (originalBundledDir === undefined) {
    ...
    process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR = originalBundledDir;
  }
...
describe("resolveBundledPluginsDir", () => {
  it.each([
    [
      ...
      {
        prefix: "GreenchClaw-bundled-dir-runtime-",
    ...
    [
      ...
      {
        prefix: "GreenchClaw-bundled-dir-dist-",
        hasDistExtensions: true,
      },
      {
        expectedRelativeDir: path.join("dist", "extensions"),
    ...
    [
      ...
      {
        prefix: "GreenchClaw-bundled-dir-git-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    [
      ...
      {
        prefix: "GreenchClaw-bundled-dir-vitest-",
        hasExtensions: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
      },
    ...
    [
      ...
      {
        prefix: "GreenchClaw-bundled-dir-tsx-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    [
      ...
      {
        prefix: "GreenchClaw-bundled-dir-git-",
        hasExtensions: true,
        hasSrc: true,
        hasGitCheckout: true,
        hasPnpmWorkspace: true,
    ...
    const repoRoot = createGreenchClawRoot(layout);
  ...
  it("falls back to source extensions when dist trees exist but do not contain real plugin manifests", () => {
    const repoRoot = createGreenchClawRoot({
      prefix: "GreenchClaw-bundled-dir-incomplete-built-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("keeps built bundled plugins for git-looking trees without pnpm workspace metadata", () => {
    const repoRoot = createGreenchClawRoot({
      prefix: "GreenchClaw-bundled-dir-git-no-pnpm-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("reports missing pnpm workspace deps for source checkouts", () => {
    const repoRoot = createGreenchClawRoot({
      prefix: "GreenchClaw-bundled-dir-source-deps-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
      hasPnpmWorkspace: true,
    ...
    process.argv[1] = path.join(repoRoot, "GreenchClaw.mjs");
  ...
  it("returns a stable empty bundled plugin directory when bundled plugins are disabled", () => {
    const repoRoot = createGreenchClawRoot({
      prefix: "GreenchClaw-bundled-dir-disabled-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it("separates tilde override cache entries by GREENCHCLAW_HOME", () => {
    const homeA = makeRepoRoot("GreenchClaw-bundled-dir-home-a-");
    const homeB = makeRepoRoot("GreenchClaw-bundled-dir-home-b-");
    seedBundledPluginTree(homeA, "bundled", "memory-core");
    seedBundledPluginTree(homeB, "bundled", "discord");
    const envBase = {
      GREENCHCLAW_BUNDLED_PLUGINS_DIR: "~/bundled",
      GREENCHCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR: "1",
      VITEST: "true",
    } satisfies NodeJS.ProcessEnv;

    const bundledA = resolveBundledPluginsDir({ ...envBase, GREENCHCLAW_HOME: homeA });
    const bundledB = resolveBundledPluginsDir({ ...envBase, GREENCHCLAW_HOME: homeB });
  ...
  it("ignores an existing override under an argv1-derived fake package root", () => {
    const installedRoot = createGreenchClawRoot({
      prefix: "GreenchClaw-bundled-dir-argv-override-reject-",
      hasDistExtensions: true,
    });
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));

    vi.spyOn(process, "cwd").mockReturnValue(installedRoot);
    process.argv[1] = path.join(installedRoot, "GreenchClaw.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR = path.join(installedRoot, "dist", "extensions");
    delete process.env.GREENCHCLAW_DISABLE_BUNDLED_PLUGINS;
  ...
  it("does not let VITEST relax existing override trust checks", () => {
    const overrideRoot = makeRepoRoot("GreenchClaw-bundled-dir-vitest-override-reject-");
    ...
    process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
    delete process.env.GREENCHCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR;
    delete process.env.GREENCHCLAW_DISABLE_BUNDLED_PLUGINS;
  ...
  it("does not let VITEST add cwd to bundled plugin resolution candidates", () => {
    const cwdRepoRoot = createGreenchClawRoot({
      prefix: "GreenchClaw-bundled-dir-vitest-cwd-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it("falls back from a missing override instead of returning an untrusted future path", () => {
    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("GreenchClaw-bundled-dir-missing-cwd-"));
    process.argv[1] = "/usr/bin/env";
    process.execArgv.length = 0;
    delete process.env.VITEST;
    const missingOverride = path.join(
      makeRepoRoot("GreenchClaw-bundled-dir-missing-override-"),
      "extensions",
    );
    process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR = missingOverride;
    delete process.env.GREENCHCLAW_DISABLE_BUNDLED_PLUGINS;
  ...
  it("falls back to argv root when an existing rejected override is unrelated", () => {
    const installedRoot = createGreenchClawRoot({
      prefix: "GreenchClaw-bundled-dir-rejected-override-argv-",
      hasDistExtensions: true,
    });
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
    const overrideRoot = makeRepoRoot("GreenchClaw-bundled-dir-rejected-override-");
    seedBundledPluginTree(overrideRoot, "extensions", "memory-core");

    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("GreenchClaw-bundled-dir-rejected-cwd-"));
    process.argv[1] = path.join(installedRoot, "GreenchClaw.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
  ...
  it("does not resolve bundled plugins from cwd when argv1 is not a package root", () => {
    const cwdRepoRoot = createGreenchClawRoot({
      prefix: "GreenchClaw-bundled-dir-untrusted-cwd-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it.each([
    {
      ...
      createScenario: () => {
        const installedRoot = createGreenchClawRoot({
          prefix: "GreenchClaw-bundled-dir-installed-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        const cwdRepoRoot = createGreenchClawRoot({
          prefix: "GreenchClaw-bundled-dir-cwd-",
          hasExtensions: true,
          hasSrc: true,
          hasGitCheckout: true,
        });
        return {
          installedRoot,
          cwd: cwdRepoRoot,
          argv1: path.join(installedRoot, "GreenchClaw.mjs"),
        };
      },
    },
    {
      ...
      createScenario: () => {
        const installedRoot = createGreenchClawRoot({
          prefix: "GreenchClaw-bundled-dir-override-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        return {
