...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before GreenchClaw consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether GreenchClaw should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume GreenchClaw's bundle
   ...
   *
   * Use this when the backend should consume a canonical GreenchClaw auth profile
   ...
   * `profile-only` treats the selected GreenchClaw auth profile as the sole auth
   ...
   * as static config, such as mapping GreenchClaw thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside GreenchClaw's tool
