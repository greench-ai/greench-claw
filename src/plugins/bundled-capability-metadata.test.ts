  ...
  return fs
    ...
    .filter((pluginDir) => {
      ...
      const packageJson = JSON.parse(fs.readFileSync(packagePath, "utf-8")) as {
        GreenchClaw?: GreenchClawPackageManifest;
      };
      return normalizeBundledPluginStringList(packageJson.GreenchClaw?.extensions).length > 0;
    })
    .map(
      (pluginDir) =>
        JSON.parse(
          fs.readFileSync(path.join(pluginDir, "GreenchClaw.plugin.json"), "utf-8"),
        ) as PluginManifest,
    )
