...
import type { ConfigFileSnapshot, GreenchClawConfig } from "./types.GreenchClaw.js";
...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "GreenchClaw-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  const createFastConfigIO = (home: string) =>
    createConfigIO({
      env: { GREENCHCLAW_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      homedir: () => home,
      logger: silentLogger,
    });
  ...
  it("logs health-state write failures through public config reads", async () => {
    await withSuiteHome(async (home) => {
      ...
      const healthPath = path.join(home, ".GreenchClaw", "logs", "config-health.json");
  ...
  it("loads shipped plugin install config records without mutating config or plugin index", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".GreenchClaw", "GreenchClaw.json");
      const pluginDir = path.join(home, ".GreenchClaw", "plugins", "demo");
      ...
      try {
        ...
        await expect(
          readPersistedInstalledPluginIndex({
            stateDir: path.join(home, ".GreenchClaw"),
          }),
        ).resolves.toBeNull();
        await expect(fs.readFile(configPath, "utf-8")).resolves.toBe(initialRaw);
      } finally {
  ...
  it("migrates shipped plugin install config records during explicit writes even when the manifest is missing", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".GreenchClaw", "GreenchClaw.json");
      const pluginDir = path.join(home, ".GreenchClaw", "plugins", "missing");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
  ...
  it("keeps shipped plugin install config records when index migration fails", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".GreenchClaw", "GreenchClaw.json");
      const unwritableStatePath = path.join(home, ".GreenchClaw");
  ...
  it("keeps writes inside an GREENCHCLAW_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".GreenchClaw", "GreenchClaw.json");
      ...
      const env = { GREENCHCLAW_STATE_DIR: overrideDir } as NodeJS.ProcessEnv;
      const io = createConfigIO({
        env,
        homedir: () => home,
        logger: silentLogger,
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "GreenchClaw.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "GreenchClaw.json"), "utf-8"),
      ) as {
  ...
  it("suppresses overwrite audit output when skipOutputLogs is set", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          VITEST: "true",
          GREENCHCLAW_TEST_CONFIG_OVERWRITE_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("rejects destructive internal writes before replacing the config", async () => {
    await withSuiteHome(async (home) => {
      ...
      const original = {
        ...
        agents: { list: [{ id: "main", default: true, workspace: "/tmp/GreenchClaw-main" }] },
  ...
  it("keeps authored agent provider params during narrowed internal agent writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as GreenchClawConfig;
  ...
  it("rejects root-include partial writes instead of flattening the root config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".GreenchClaw", "GreenchClaw.json");
      const includePath = path.join(home, ".GreenchClaw", "extra.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ $schema: "https://GreenchClaw.ai/config-from-include.json" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/GreenchClaw-test-required-plugin",
          source: "/tmp/GreenchClaw-test-required-plugin/index.ts",
          manifestPath: "/tmp/GreenchClaw-test-required-plugin/GreenchClaw.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
    ...
    await withSuiteHome(async (home) => {
      ...
      try {
        ...
        if (previousConfigPath === undefined) {
          ...
          process.env.GREENCHCLAW_CONFIG_PATH = previousConfigPath;
        }
      }
    });
  });
  ...
  it("notifies in-process reloaders with resolved source config when persisted env refs are restored", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".GreenchClaw", "GreenchClaw.json");
      const previousConfigPath = process.env.GREENCHCLAW_CONFIG_PATH;
      const previousGatewayToken = process.env.GREENCHCLAW_GATEWAY_TOKEN;
      process.env.GREENCHCLAW_CONFIG_PATH = configPath;
      process.env.GREENCHCLAW_GATEWAY_TOKEN = "gateway-token-runtime";
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
          {
            gateway: {
              mode: "local",
              auth: { mode: "token", token: "${GREENCHCLAW_GATEWAY_TOKEN}" },
            },
      ...
      try {
        ...
        expect(persisted.gateway?.auth?.token).toBe("${GREENCHCLAW_GATEWAY_TOKEN}");
        ...
        if (previousGatewayToken === undefined) {
          ...
          process.env.GREENCHCLAW_GATEWAY_TOKEN = previousGatewayToken;
        }
      }
    });
  });
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/GreenchClaw-test-demo",
          source: "/tmp/GreenchClaw-test-demo/index.ts",
          manifestPath: "/tmp/GreenchClaw-test-demo/GreenchClaw.plugin.json",
          configSchema: {
            type: "object",
            properties: {
    ...
    await withSuiteHome(async (home) => {
      ...
      try {
        ...
        const postWriteSnapshot = await createConfigIO({
          env: { GREENCHCLAW_CONFIG_PATH: configPath, VITEST: "true" } as NodeJS.ProcessEnv,
          homedir: () => home,
          logger: silentLogger,
  ...
  it("skipPluginValidation bypasses plugin schema rejection on writeConfigFile (#76800)", async () => {
    await withSuiteHome(async (home) => {
        ...
        plugins: [
          {
            ...
            skills: [],
            hooks: [],
            rootDir: "/tmp/GreenchClaw-test-strict-plugin",
            source: "/tmp/GreenchClaw-test-strict-plugin/index.ts",
            manifestPath: "/tmp/GreenchClaw-test-strict-plugin/GreenchClaw.plugin.json",
            configSchema: {
              type: "object",
      ...
      try {
        ...
        await expect(
          writeConfigFile({ agents: { list: "not-array" } } as unknown as GreenchClawConfig, {
  ...
  it("preserves authored tilde paths when runtime-shaped writes hand back absolute paths", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            logging: { file: "~/GreenchClaw-upgrade-survivor/gateway.jsonl" },
          },
          null,
          2,
        )}\n`,
      ...
      await io.writeConfigFile(
        {
          logging: {
            file: path.join(home, "GreenchClaw-upgrade-survivor", "gateway.jsonl"),
            level: "debug",
          },
        },
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as GreenchClawConfig;
      expect(persisted.logging?.file).toBe("~/GreenchClaw-upgrade-survivor/gateway.jsonl");
