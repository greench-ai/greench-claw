...
import type { GreenchClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
export type ConfigWriteOptions = {
   ...
   */
  explicitSetValueSource?: GreenchClawConfig;
  /**
...
function coerceConfig(value: unknown): GreenchClawConfig {
...
function stampConfigVersion(cfg: GreenchClawConfig): GreenchClawConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    ...
    logger.warn(
      [
        `Your GreenchClaw config was written by version ${touched}, but this command is running ${VERSION}.`,
        "Check: `GreenchClaw --version`, `which GreenchClaw`, and `GreenchClaw gateway status --deep`.",
        "If unexpected, update PATH so `GreenchClaw` points to the version you want, or reinstall the Gateway service from that same GreenchClaw install.",
      ].join("\n"),
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as GreenchClawConfig, env);
  }
...
export function createConfigIO(
  ...
  function finalizeLoadedRuntimeConfig(cfg: GreenchClawConfig): GreenchClawConfig {
  ...
  function ensureShippedPluginInstallConfigRecordsMigratedForWrite(
    ...
    try {
      ...
      throw new Error(
        `Config write blocked: shipped plugins.installs records in ${configPath} could not be migrated into the plugin index. Fix state directory permissions or run GreenchClaw plugins registry --refresh, then retry. ${formatErrorMessage(
          err,
        )}`,
        { cause: err },
      );
  ...
  function loadConfig(): GreenchClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(
        validationConfigRaw as GreenchClawConfig,
        {
          env: deps.env,
          homedir: deps.homedir,
        },
      ...
      const loadValidationPluginMetadataSnapshot = (config: GreenchClawConfig) => {
  ...
  async function readConfigFileSnapshotInternal(): Promise<ReadConfigFileSnapshotInternalResult> {
    ...
    let fallbackSourceConfig: GreenchClawConfig = {};
  ...
  async function readBestEffortConfig(): Promise<GreenchClawConfig> {
    const result = await readConfigFileSnapshotInternal();
  ...
  async function readSourceConfigBestEffort(): Promise<GreenchClawConfig> {
    maybeLoadDotEnvForConfig(deps.env);
    const exists = deps.fs.existsSync(configPath);
    if (!exists) {
      return {};
  ...
  async function writeConfigFile(
  ...
  ): Promise<{ persistedHash: string; persistedConfig: GreenchClawConfig }> {
    ...
    persistCandidate = applyUnsetPathsForWrite(persistCandidate as GreenchClawConfig, unsetPaths);
    ...
    let cfgToWrite = persistCandidate as GreenchClawConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as GreenchClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.GREENCHCLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.GREENCHCLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
...
// module scope. `GREENCHCLAW_CONFIG_PATH` (and friends) are expected to work even
...
function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: GreenchClawConfig;
...
export function loadConfig(): GreenchClawConfig {
  ...
  // reparsing GreenchClaw.json on hot code paths.
  return loadPinnedRuntimeConfig(() => createConfigIO().loadConfig());
}

export function getRuntimeConfig(): GreenchClawConfig {
  return loadConfig();
}

export async function readBestEffortConfig(): Promise<GreenchClawConfig> {
  return await createConfigIO().readBestEffortConfig();
}

export async function readSourceConfigBestEffort(): Promise<GreenchClawConfig> {
...
export async function writeConfigFile(
  ...
  let canonicalSourceConfig: GreenchClawConfig = nextCfg;
  try {
