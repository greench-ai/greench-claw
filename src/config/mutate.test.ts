...
function createSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/GreenchClaw.json",
    exists: true,
    raw: "{}",
...
describe("config mutate helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "GreenchClaw-config-mutate-" });
  const originalNixMode = process.env.GREENCHCLAW_NIX_MODE;
  ...
  afterAll(async () => {
    if (originalNixMode === undefined) {
      ...
      process.env.GREENCHCLAW_NIX_MODE = originalNixMode;
    }
  ...
  it("refuses mutate writes in Nix mode before touching disk", async () => {
    ...
    ).rejects.toThrow("GreenchClaw Nix overview: https://docs.GreenchClaw.ai/install/nix");
  ...
  it("writes through a single-file top-level plugins include", async () => {
    ...
    const pluginsPath = path.join(home, ".GreenchClaw", "config", "plugins.json5");
