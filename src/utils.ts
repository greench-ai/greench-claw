...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".GreenchClaw");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.GREENCHCLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$GREENCHCLAW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via GREENCHCLAW_STATE_DIR.
