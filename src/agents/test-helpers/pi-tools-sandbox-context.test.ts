...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("GreenchClaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("GreenchClaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("GreenchClaw-sbx-");
