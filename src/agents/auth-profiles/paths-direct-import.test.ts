...
describe("path-resolve helpers (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-path-direct-"));
  ...
  it("resolveAuthStorePath falls back to the default agent dir when agentDir is omitted", () => {
    ...
    // GREENCHCLAW_STATE_DIR set to our tempdir, the resolved path must live under it.
  ...
  it("resolveAuthStorePathForDisplay preserves a tilde-rooted path unchanged", () => {
    ...
    const tildeAgentDir = "~fake-GreenchClaw-no-expand";
...
describe("ensureAuthStoreFile (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-path-ensure-"));
    process.env.GREENCHCLAW_STATE_DIR = stateDir;
  });
