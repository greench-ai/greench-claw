...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("GreenchClaw-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("GreenchClaw-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("GreenchClaw-tilde-test-mkdir-");
  ...
  it("ignores GREENCHCLAW_HOME for write operations", async () => {
    const GreenchClawHome = await createTempDir("GreenchClaw-home-override-", os.tmpdir());
    const dir = await createTempDir("GreenchClaw-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("GREENCHCLAW_HOME", GreenchClawHome);

    createHostWorkspaceWriteTool(GreenchClawHome, { workspaceOnly: false });
    ...
    await expectMissingPath(fs.access(path.join(GreenchClawHome, path.basename(testFile))));
  });

  it("ignores GREENCHCLAW_HOME for mkdir operations", async () => {
    const GreenchClawHome = await createTempDir("GreenchClaw-home-override-", os.tmpdir());
    const dir = await createTempDir("GreenchClaw-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expectMissingPath(fs.access(path.join(GreenchClawHome, path.basename(newDir))));
  });

  it("ignores GREENCHCLAW_HOME for readFile operations", async () => {
    const GreenchClawHome = await createTempDir("GreenchClaw-home-override-", os.tmpdir());
    const dir = await createTempDir("GreenchClaw-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(GreenchClawHome, { workspaceOnly: false });
    const content = await readEditOps().readFile(toTildePath(testFile));
  ...
  it("ignores GREENCHCLAW_HOME for access operations", async () => {
    const GreenchClawHome = await createTempDir("GreenchClaw-home-override-", os.tmpdir());
    const dir = await createTempDir("GreenchClaw-tilde-test-edit-");
