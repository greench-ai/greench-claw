...
type AttemptSpawnWorkspaceHoisted = {
  ...
  createGreenchClawCodingToolsMock: UnknownMock;
...
const hoisted = vi.hoisted((): AttemptSpawnWorkspaceHoisted => {
  ...
  const createGreenchClawCodingToolsMock = vi.fn(() => []);
  ...
  return {
    ...
    createGreenchClawCodingToolsMock,
...
vi.mock("../../docs-path.js", () => ({
  resolveGreenchClawReferencePaths: async () => ({ docsPath: undefined, sourcePath: undefined }),
}));
...
vi.mock("../../pi-tools.js", () => ({
  createGreenchClawCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) =>
    hoisted.createGreenchClawCodingToolsMock(options),
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("GreenchClaw.cache-ttl", data),
  ...
  readLastCacheTtlTimestamp: (
    ...
    for (let index = calls.length - 1; index >= 0; index -= 1) {
      ...
      if (customType !== "GreenchClaw.cache-ttl") {
        continue;
      }
      const entry = data as
        | {
...
export function resetEmbeddedAttemptHarness(
  ...
  hoisted.createGreenchClawCodingToolsMock.mockReset().mockImplementation((...args: unknown[]) => {
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "GreenchClaw-ctx-engine-workspace-"),
  );
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-ctx-engine-agent-"));
  ...
  const previousTrajectoryEnv = process.env.GREENCHCLAW_TRAJECTORY;
  if (params.trajectory !== true) {
    process.env.GREENCHCLAW_TRAJECTORY = "0";
  }
  try {
    return await (
    ...
    if (previousTrajectoryEnv === undefined) {
      delete process.env.GREENCHCLAW_TRAJECTORY;
    } else {
      process.env.GREENCHCLAW_TRAJECTORY = previousTrajectoryEnv;
    }
  }
}
