...
const LIVE = isLiveTestEnabled(["GREENCHCLAW_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "GreenchClaw-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("GreenchClaw_live_probe", "GreenchClaw Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an GreenchClaw-configured streamable-http server", async () => {
    const geminiCommand = process.env.GREENCHCLAW_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("GreenchClawLiveProbe");
