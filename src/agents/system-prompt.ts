...
function buildWebchatCanvasSection(params: { isMinimal: boolean; runtimeChannel?: string }) {
  ...
  return [
    ...
    '- You may also use an explicit hosted URL: `[embed url="/__GreenchClaw__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    '- Never use local filesystem paths or `file://...` URLs in `[embed ...]`. Hosted embeds must point at `/__GreenchClaw__/canvas/...` URLs or use `ref="..."`.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__GreenchClaw__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; GreenchClaw handles all routing internally.",
...
function buildDocsSection(params: {
  ...
  const lines = [
    "## Documentation",
    docsPath ? `Docs: ${docsPath}` : "Docs: https://docs.GreenchClaw.ai",
    docsPath ? "Mirror: https://docs.GreenchClaw.ai" : undefined,
    sourcePath ? `Source: ${sourcePath}` : "Source: https://github.com/GreenchClaw/GreenchClaw",
    docsPath
      ? "GreenchClaw behavior/config/architecture: read local docs first."
      : "GreenchClaw behavior/config/architecture: read docs mirror first.",
    ...
    "Diagnosing issues: run `GreenchClaw status` when possible; ask user only if blocked.",
    "",
  ];
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running GreenchClaw process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List GreenchClaw agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List GreenchClaw agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return ["You are a personal assistant running inside GreenchClaw.", modelIdentityLine]
      .filter(Boolean)
      .join("\n");
  }
  ...
  const stablePrefix = cacheStablePromptPrefix(stablePrefixCacheKey, () => {
    const lines = [
      "You are a personal assistant running inside GreenchClaw.",
      "",
      "## Tooling",
      ...
      toolLines.length > 0
        ...
        : [
            ...
            "- browser: control GreenchClaw's dedicated browser",
            "- canvas: present/eval/snapshot the Canvas",
            "- nodes: list/describe/notify/camera/screen on paired nodes",
            "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
      ...
      "CLI lifecycle only on explicit user request: `GreenchClaw gateway status|restart|start|stop`.",
      "`restart`, not stop+start.",
      "",
      ...skillsSection,
      ...memorySection,
      hasGateway && !isMinimal ? "## GreenchClaw Self-Update" : "",
      hasGateway && !isMinimal
        ? [
            ...
            "After restart, GreenchClaw pings the last active session automatically.",
          ].join("\n")
        : "",
      hasGateway && !isMinimal ? "" : "",
      "",
      ...
      "These user-editable files are loaded by GreenchClaw and included below in Project Context.",
      "",
