...
function hasExplicitImageModelConfig(config: GreenchClawConfig | undefined): boolean {
  return hasExplicitToolModelConfig(config?.agents?.defaults?.imageModel);
}

function hasExplicitPdfModelConfig(config: GreenchClawConfig | undefined): boolean {
  return (
