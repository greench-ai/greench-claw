...
describe("edit tool recovery hardening", () => {
  ...
  it("adds current file contents to exact-match mismatch errors", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-edit-recovery-"));
  ...
  it("recovers tilde paths against the OS home even when GREENCHCLAW_HOME differs", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-edit-recovery-"));
    const osHome = path.join(tmpDir, "home");
    const GreenchClawHome = path.join(tmpDir, "GreenchClaw-home");
    await fs.mkdir(osHome, { recursive: true });
    await fs.mkdir(GreenchClawHome, { recursive: true });

    const previousHome = process.env.HOME;
    const previousUserProfile = process.env.USERPROFILE;
    const previousOpenclawHome = process.env.GREENCHCLAW_HOME;
    process.env.HOME = osHome;
    process.env.USERPROFILE = osHome;
    process.env.GREENCHCLAW_HOME = GreenchClawHome;
    ...
    try {
      ...
      await expectPathMissing(path.join(GreenchClawHome, "demo.txt"));
    } finally {
      ...
      if (previousOpenclawHome === undefined) {
        ...
        process.env.GREENCHCLAW_HOME = previousOpenclawHome;
      }
    }
  });
