...
} from "./GreenchClaw-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./GreenchClaw-tools.nodes-workspace-guard.js";
import {
  collectPresentGreenchClawTools,
  isUpdatePlanToolEnabledForGreenchClawTools,
} from "./GreenchClaw-tools.registration.js";
import {
  type HookContext,
...
type GreenchClawToolsDeps = {
  callGateway: typeof callGateway;
  config?: GreenchClawConfig;
};

const defaultGreenchClawToolsDeps: GreenchClawToolsDeps = {
  callGateway,
};

let openClawToolsDeps: GreenchClawToolsDeps = defaultGreenchClawToolsDeps;

export function createGreenchClawTools(
  options?: {
  ...
  options?.recordToolPrepStage?.("GreenchClaw-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("GreenchClaw-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("GreenchClaw-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("GreenchClaw-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("GreenchClaw-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("GreenchClaw-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("GreenchClaw-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("GreenchClaw-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("GreenchClaw-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("GreenchClaw-tools:nodes-tool");
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentGreenchClawTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentGreenchClawTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
    ...
    ...collectPresentGreenchClawTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("GreenchClaw-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    options?.recordToolPrepStage?.("GreenchClaw-tools:plugin-tools");
  }
  ...
  options?.recordToolPrepStage?.("GreenchClaw-tools:tool-hooks");
...
export const __testing = {
  resolveOptionalMediaToolFactoryPlan,
  setDepsForTest(overrides?: Partial<GreenchClawToolsDeps>) {
    openClawToolsDeps = overrides
      ? {
          ...defaultGreenchClawToolsDeps,
          ...overrides,
        }
      : defaultGreenchClawToolsDeps;
  },
