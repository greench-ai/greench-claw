...
let configOverride: GreenchClawConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: GreenchClawConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/GreenchClaw/GreenchClaw/issues/31298
    previousFastTestEnv = process.env.GREENCHCLAW_TEST_FAST;
  ...
  beforeEach(() => {
    vi.useRealTimers();
    resetAnnounceQueuesForTests();
    // GREENCHCLAW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("GreenchClaw runtime context (internal):");
