...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses GREENCHCLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      GREENCHCLAW_HOME: "/srv/GreenchClaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/GreenchClaw-home"), ".GreenchClaw", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".GreenchClaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("GreenchClaw-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-workspace-hardlink-"));
    try {
