...
const ensureGreenchClawModelsJsonMock = vi.fn(async () => ({ wrote: false }));
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureGreenchClawModelsJson: (...args: Parameters<typeof ensureGreenchClawModelsJsonMock>) =>
        ensureGreenchClawModelsJsonMock(...args),
    };
  });
};
...
beforeAll(async () => {
  ...
  e2eWorkspace = await createEmbeddedPiRunnerTestWorkspace("GreenchClaw-embedded-agent-");
...
beforeEach(() => {
  ...
  ensureGreenchClawModelsJsonMock.mockReset();
  ensureGreenchClawModelsJsonMock.mockResolvedValue({ wrote: false });
