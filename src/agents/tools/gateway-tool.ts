...
import { isGreenchClawOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `GreenchClaw security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as GreenchClawConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as GreenchClawConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isGreenchClawOwnerOnlyCoreToolName("gateway"),
    description:
