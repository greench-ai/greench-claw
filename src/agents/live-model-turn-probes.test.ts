...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "GREENCHCLAW_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { GREENCHCLAW_LIVE_MODEL_IMAGE_PROBE: "false" },
        "GREENCHCLAW_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { GREENCHCLAW_LIVE_MODEL_IMAGE_PROBE: "1" },
        "GREENCHCLAW_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
