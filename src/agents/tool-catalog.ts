...
type CoreToolDefinition = {
  ...
  includeInGreenchClawGroup?: boolean;
};
...
const CORE_TOOL_DEFINITIONS: CoreToolDefinition[] = [
  ...
  },
  {
    ...
    sectionId: "runtime",
    profiles: ["coding"],
    includeInGreenchClawGroup: true,
  },
  {
    id: "web_search",
    label: "web_search",
...
function buildCoreToolGroupMap() {
  ...
  const GreenchClawTools = CORE_TOOL_DEFINITIONS.filter(
    (tool) => tool.includeInGreenchClawGroup,
  ).map((tool) => tool.id);
  return {
    "group:GreenchClaw": GreenchClawTools,
