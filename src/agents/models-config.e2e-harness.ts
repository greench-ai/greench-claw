...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousGreenchClawAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousGreenchClawAgentDir = process.env.GREENCHCLAW_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousGreenchClawAgentDir === undefined) {
      delete process.env.GREENCHCLAW_AGENT_DIR;
    } else {
      process.env.GREENCHCLAW_AGENT_DIR = previousGreenchClawAgentDir;
    }
...
export const MODELS_CONFIG_IMPLICIT_ENV_VARS = [
  "GREENCHCLAW_TEST_ONLY_PROVIDER_PLUGIN_IDS",
  "VITEST",
  "NODE_ENV",
  "AI_GATEWAY_API_KEY",
...
export const CUSTOM_PROXY_MODELS_CONFIG: GreenchClawConfig = {
  models: {
    providers: {
      "custom-proxy": {
