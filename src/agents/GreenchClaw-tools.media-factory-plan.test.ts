...
import { resolveOptionalMediaToolFactoryPlan } from "./GreenchClaw-tools.media-factory-plan.js";
...
type CreateGreenchClawToolsOptions = Parameters<
  typeof import("./GreenchClaw-tools.js").createGreenchClawTools
>[0];

async function createGreenchClawToolsForTest(options?: CreateGreenchClawToolsOptions) {
  const { createGreenchClawTools } = await import("./GreenchClaw-tools.js");
  return createGreenchClawTools(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/GreenchClaw.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): GreenchClawConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  ...
  it("preserves implicit allow-all from alsoAllow-only policies for built-in media factories", async () => {
    ...
    const toolNames = (
      await createGreenchClawToolsForTest({
        config,
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: GreenchClawConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createGreenchClawToolsForTest({
      config,
      agentDir: "/tmp/GreenchClaw-agent-main",
