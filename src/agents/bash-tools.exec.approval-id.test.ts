...
async function writeExecApprovalsConfig(config: Record<string, unknown>) {
  const approvalsPath = path.join(process.env.HOME ?? "", ".GreenchClaw", "exec-approvals.json");
...
describe("exec approvals", () => {
  ...
  beforeEach(async () => {
    ...
    previousBundledPluginsDir = process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR;
    previousDisableBundledPlugins = process.env.GREENCHCLAW_DISABLE_BUNDLED_PLUGINS;
  ...
  afterEach(() => {
    ...
    if (previousDisableBundledPlugins === undefined) {
      ...
      process.env.GREENCHCLAW_DISABLE_BUNDLED_PLUGINS = previousDisableBundledPlugins;
    }
  });
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-test-bin-"));
  ...
  it("runs a skill wrapper chain without prompting when the wrapper is allowlisted", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-skill-wrapper-"));
    try {
      const skillDir = path.join(tempDir, ".GreenchClaw", "skills", "gog");
