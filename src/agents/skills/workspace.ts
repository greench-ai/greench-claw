...
import { resolveGreenchClawMetadata, resolveSkillInvocationPolicy } from "./frontmatter.js";
...
function resolveSkillsLimits(config?: GreenchClawConfig, agentId?: string): ResolvedSkillsLimits {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "GreenchClaw-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "GreenchClaw-bundled") {
    return {
...
function resolveAllowedSymlinkTargetRealPaths(config?: GreenchClawConfig): string[] {
...
function loadSkillEntries(
  ...
  const loadSkills = (params: { dir: string; source: string }): LoadedSkillRecord[] => {
    ...
    // skill directories (e.g. ~/.GreenchClaw/skills/coze/koze-retrieval/SKILL.md).
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`GreenchClaw skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`GreenchClaw skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
