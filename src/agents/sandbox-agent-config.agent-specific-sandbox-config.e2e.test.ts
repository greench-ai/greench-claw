...
async function resolveContext(config: GreenchClawConfig, sessionKey: string, workspaceDir: string) {
...
function createWorkSetupCommandConfig(scope: "agent" | "shared"): GreenchClawConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
    ...
    const cfg: GreenchClawConfig = {
      agents: {
        ...
        list: [
          {
            id: "isolated",
            workspace: "~/GreenchClaw-isolated",
            sandbox: {
              mode: "all",
              scope: "agent",
