...
// https://github.com/GreenchClaw/GreenchClaw/issues/33785
...
describe("failover-error", () => {
  ...
  it("does not classify session lock wait errors as model timeout failover", () => {
    const sessionLockError = new SessionWriteLockTimeoutError({
      timeoutMs: 10_000,
      owner: "pid=37121",
      lockPath: "/tmp/GreenchClaw/session.jsonl.lock",
    });
  ...
  it("403 OpenRouter 'Key limit exceeded' returns billing (model fallback trigger)", () => {
    // GitHub: GreenchClaw/GreenchClaw#53849 — OpenRouter returns 403 with "Key limit exceeded"
