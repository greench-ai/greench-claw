...
type BaseStreamOptions = {
  ...
  GreenchClawCodeModeToolSurface?: boolean;
};
  ...
  return (model, context, options) => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (
          (options as { GreenchClawCodeModeToolSurface?: unknown } | undefined)
            ?.GreenchClawCodeModeToolSurface === true
        ) {
...
function getCompletionsContentDeltas(content: unknown): CompletionsReasoningDelta[] {
  ...
  // Preserve provider reasoning as GreenchClaw thinking blocks so channel/UI
