...
const GOOGLE_LIVE_SINGLE_KEY = "GREENCHCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "GREENCHCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "GREENCHCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "GREENCHCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `GREENCHCLAW_LIVE_${base}_KEY`;
