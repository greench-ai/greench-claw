...
const acpMocks = vi.hoisted(() => ({
  ...
  readAcpSessionEntry: vi.fn<(params: { sessionKey: string; cfg?: GreenchClawConfig }) => unknown>(
    () => null,
  ),
...
vi.mock("../../tts/tts.js", () => ({
  ...
  resolveTtsConfig: (cfg: GreenchClawConfig) => ttsMocks.resolveTtsConfig(cfg),
}));
...
vi.mock("../../tts/tts-config.js", () => ({
  normalizeTtsAutoMode: (value: unknown) => ttsMocks.normalizeTtsAutoMode(value),
  resolveConfiguredTtsMode: (cfg: GreenchClawConfig) => ttsMocks.resolveTtsConfig(cfg).mode,
  shouldCleanTtsDirectiveText: () => true,
  shouldAttemptTtsPayload: () => true,
}));

const noAbortResult = { handled: false, aborted: false } as const;
const emptyConfig = {} as GreenchClawConfig;
...
} as const satisfies GreenchClawConfig;
...
describe("dispatchReplyFromConfig", () => {
  ...
  it("normalizes accumulated block TTS-only media before final delivery", async () => {
    ...
    replyMediaPathMocks.createReplyMediaPathNormalizer.mockReturnValue(
      async (payload: ReplyPayload) => ({
        ...payload,
        mediaUrl: "/tmp/GreenchClaw-media/normalized-tts.ogg",
        mediaUrls: ["/tmp/GreenchClaw-media/normalized-tts.ogg"],
      }),
    );
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
    ...
    const finalPayload = firstFinalReplyPayload(dispatcher);
    expect(finalPayload?.mediaUrl).toBe("/tmp/GreenchClaw-media/normalized-tts.ogg");
    expect(finalPayload?.mediaUrls).toStrictEqual(["/tmp/GreenchClaw-media/normalized-tts.ogg"]);
  ...
  it("does not mark duplicate inbound returns as tool-only when message is unavailable", async () => {
    setNoAbort();
    const cfg = { tools: { allow: ["read"] } } as GreenchClawConfig;
    const ctx = buildTestCtx({
      Provider: "telegram",
      Surface: "telegram",
      ChatType: "channel",
  ...
  it("emits diagnostics when enabled", async () => {
    setNoAbort();
    const cfg = { diagnostics: { enabled: true } } as GreenchClawConfig;
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
      Provider: "slack",
      Surface: "slack",
  ...
  it("routes plugin-owned bindings to the owning plugin before generic inbound claim broadcast", async () => {
    ...
    hookMocks.registry.plugins = [{ id: "GreenchClaw-codex-app-server", status: "loaded" }];
    hookMocks.runner.runInboundClaimForPluginOutcome.mockResolvedValue({
      status: "handled",
      result: { handled: true },
    });
    sessionBindingMocks.resolveByConversation.mockReturnValue({
      ...
      metadata: {
        ...
        pluginRoot: "/Users/huntharo/github/GreenchClaw-app-server",
        data: {
          kind: "codex-app-server-session",
          version: 1,
          sessionFile: "/tmp/session.jsonl",
          workspaceDir: "/workspace/GreenchClaw",
        },
      },
    } satisfies SessionBindingRecord);
    const cfg = emptyConfig;
    ...
    const inboundClaimCall = hookMocks.runner.runInboundClaimForPluginOutcome.mock
      ...
      | undefined;
    expect(inboundClaimCall?.[0]).toBe("GreenchClaw-codex-app-server");
  ...
  it("falls back to GreenchClaw once per startup when a bound plugin is missing", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
    ...
    const replyResolver = vi.fn(
      async () => ({ text: "GreenchClaw fallback" }) satisfies ReplyPayload,
    );
  ...
  it("falls back to GreenchClaw when the bound plugin is loaded but has no inbound_claim handler", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
  ...
  it("passes the loaded config plus configOverride patch to replyResolver when provided", async () => {
    ...
    let receivedCfg: GreenchClawConfig | undefined;
    const replyResolver = async (
      _ctx: MsgContext,
      _opts?: GetReplyOptions,
      cfgArg?: GreenchClawConfig,
    ) => {
      receivedCfg = cfgArg;
      return { text: "hi" } satisfies ReplyPayload;
    };
  ...
  it("passes the already loaded config to replyResolver when configOverride is not provided", async () => {
    setNoAbort();
    const cfg = { agents: { defaults: { userTimezone: "UTC" } } } as GreenchClawConfig;
    const dispatcher = createDispatcher();
...
describe("sendPolicy deny — suppress delivery, not processing (#53328)", () => {
  ...
  it("suppresses tool result delivery when sendPolicy is deny", async () => {
    ...
    const replyResolver = vi.fn(
      async (_ctx: MsgContext, opts?: GetReplyOptions, _cfg?: GreenchClawConfig) => {
  ...
  it("falls back to automatic group/channel delivery when the message tool is unavailable", async () => {
    ...
    const result = await dispatchReplyFromConfig({
      ...
      cfg: { tools: { allow: ["read"] } } as GreenchClawConfig,
      dispatcher,
      replyResolver,
    });
