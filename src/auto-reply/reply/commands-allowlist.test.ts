...
async function withTempConfigPath<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-allowlist-config-"));
  const configPath = path.join(dir, "GreenchClaw.json");
  const previous = process.env.GREENCHCLAW_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.GREENCHCLAW_CONFIG_PATH = previous;
    }
        ...
        run: async () => {
          await withTempConfigPath(
            ...
            async (configPath) => {
              ...
              const written = await readJsonFile<GreenchClawConfig>(configPath);
