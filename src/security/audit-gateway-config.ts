...
type CollectDangerousConfigFlags = (cfg: GreenchClawConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.GREENCHCLAW_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.GREENCHCLAW_GATEWAY_PASSWORD);
