...
export async function finalizeSetupWizard(
  ...
  if (!opts.skipHealth) {
    ...
    if (gatewayProbe.ok) {
      try {
        const healthConfig: GreenchClawConfig =
        ...
        await prompter.note(
          [
            "Docs:",
            "https://docs.GreenchClaw.ai/gateway/health",
            "https://docs.GreenchClaw.ai/gateway/troubleshooting",
          ].join("\n"),
          "Health check help",
        );
      }
      ...
      await prompter.note(
        [
          ...
          `Start now: ${formatCliCommand("GreenchClaw gateway run")}`,
          `Or rerun with: ${formatCliCommand("GreenchClaw onboard --install-daemon")}`,
          `Or skip this probe next time: ${formatCliCommand("GreenchClaw onboard --skip-health")}`,
        ].join("\n"),
        "Gateway",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.GreenchClaw.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
    ...
    if (gatewayProbe.ok) {
      await prompter.note(
        [
          ...
          "Stored in: $GREENCHCLAW_CONFIG_PATH (default: ~/.GreenchClaw/GreenchClaw.json) under gateway.auth.token, or in GREENCHCLAW_GATEWAY_TOKEN.",
          `View token: ${formatCliCommand("GreenchClaw config get gateway.auth.token")}`,
          `Generate token: ${formatCliCommand("GreenchClaw doctor --generate-gateway-token")}`,
          "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
          `Open the dashboard anytime: ${formatCliCommand("GreenchClaw dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control GreenchClaw."
            : "Copy/paste this URL in a browser on this machine to control GreenchClaw.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("GreenchClaw dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.GreenchClaw.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.GreenchClaw.ai/security",
    "Security",
  );
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("GreenchClaw configure --section web")}`,
          "",
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("GreenchClaw configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.GreenchClaw.ai/tools/web"}`,
          "Docs: https://docs.GreenchClaw.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("GreenchClaw configure --section web")}`,
          "",
          "Docs: https://docs.GreenchClaw.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://GreenchClaw.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control GreenchClaw."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control GreenchClaw.",
  );
