...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.GREENCHCLAW_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.GREENCHCLAW_GATEWAY_PASSWORD ?? configuredPassword;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from GreenchClaw.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<GreenchClawConfig["tools"]>["web"]>["search"];
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "GreenchClaw update wizard" : "GreenchClaw configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    let currentBaseHash = snapshot.hash;
    const baseConfig: GreenchClawConfig = snapshot.valid
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("GreenchClaw doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbePromise = (async () => {
      ...
      return probeGatewayReachable({
        url: localUrl,
        token: process.env.GREENCHCLAW_GATEWAY_TOKEN ?? baseLocalProbeToken,
        password: process.env.GREENCHCLAW_GATEWAY_PASSWORD ?? baseLocalProbePassword,
