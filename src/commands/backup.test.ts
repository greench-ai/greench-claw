...
describe("backup commands", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("GreenchClaw-backup-test-");
  });
  ...
  async function withInvalidWorkspaceBackupConfig<T>(fn: (runtime: RuntimeEnv) => Promise<T>) {
    const stateDir = path.join(tempHome.home, ".GreenchClaw");
    const configPath = path.join(tempHome.home, "custom-config.json");
    process.env.GREENCHCLAW_CONFIG_PATH = configPath;
    await fs.writeFile(path.join(stateDir, "GreenchClaw.json"), JSON.stringify({}), "utf8");
  ...
  it("orders coverage checks by canonical path so symlinked workspaces do not duplicate state", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".GreenchClaw");
    const workspaceDir = path.join(stateDir, "workspace");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-workspace-link-"));
  ...
  it("creates an archive with a manifest and external workspace payload", async () => {
    const stateDir = path.join(tempHome.home, ".GreenchClaw");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-workspace-"));
    const configPath = path.join(tempHome.home, "custom-config.json");
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-backups-"));
  ...
  it("keeps volatile-skip notices out of json output", async () => {
    const stateDir = path.join(tempHome.home, ".GreenchClaw");
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-backups-json-"));
    try {
      const runtime = createBackupTestRuntime();
  ...
  it("rejects symlinked output paths even when intermediate directories do not exist yet", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".GreenchClaw");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-backup-link-"));
  ...
  it("falls back to the home directory when cwd is inside a backed-up source tree", async () => {
    ...
    if (process.platform !== "win32") {
      const linkParent = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-backup-cwd-link-"));
