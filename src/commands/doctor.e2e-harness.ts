...
export const resolveGreenchClawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadGreenchClawPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("../config/config.js", async () => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/GreenchClaw.json",
    createConfigIO,
...
vi.mock("GreenchClaw/plugin-sdk/provider-auth", () => ({
  isNonSecretApiKeyMarker: () => false,
}));

vi.mock("GreenchClaw/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("GreenchClaw/plugin-sdk/provider-stream-shared", () => ({
...
beforeEach(() => {
  ...
  resolveGreenchClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.GREENCHCLAW_STATE_DIR;
  originalUpdateInProgress = process.env.GREENCHCLAW_UPDATE_IN_PROGRESS;
  process.env.GREENCHCLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-doctor-state-"));
