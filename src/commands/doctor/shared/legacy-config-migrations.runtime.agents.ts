...
const MEMORY_SEARCH_RULE: LegacyConfigRule = {
  path: ["memorySearch"],
  message:
    'top-level memorySearch was moved; use agents.defaults.memorySearch instead. Run "GreenchClaw doctor --fix".',
};
...
const LEGACY_SANDBOX_SCOPE_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.sandbox.perSession is legacy; use agents.defaults.sandbox.scope instead. Run "GreenchClaw doctor --fix".',
    match: (value) => hasLegacySandboxPerSession(value),
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].sandbox.perSession is legacy; use agents.list[].sandbox.scope instead. Run "GreenchClaw doctor --fix".',
...
const LEGACY_AGENT_RUNTIME_POLICY_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.agentRuntime is ignored; set models.providers.<provider>.agentRuntime or a model-scoped agentRuntime instead. Run "GreenchClaw doctor --fix".',
  },
  {
    path: ["agents", "defaults", "embeddedHarness"],
    message:
      'agents.defaults.embeddedHarness is legacy and ignored; set provider/model runtime policy instead. Run "GreenchClaw doctor --fix".',
  ...
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].agentRuntime is ignored; set provider/model runtime policy instead. Run "GreenchClaw doctor --fix".',
    match: (value) => hasAgentListRuntimePolicy(value),
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].embeddedHarness is legacy and ignored; set provider/model runtime policy instead. Run "GreenchClaw doctor --fix".',
...
const LEGACY_AGENT_LLM_TIMEOUT_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.llm is legacy; use models.providers.<id>.timeoutSeconds for slow model/provider timeouts. Run "GreenchClaw doctor --fix".',
    match: (value) => getRecord(value) !== null,
  },
];
