...
test("sessions.compact without maxLines runs embedded manual compaction for checkpoint-capable flows", async () => {
  ...
  expect(compactionCall.workspaceDir).toBe(path.join(os.tmpdir(), "GreenchClaw-gateway-test"));
  ...
  expect(callConfig.agents?.defaults?.workspace).toBe(
    path.join(os.tmpdir(), "GreenchClaw-gateway-test"),
  );
...
test("sessions.patch preserves nested model ids under provider overrides", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-gw-sessions-nested-"));
  ...
  await withEnvAsync({ GREENCHCLAW_CONFIG_PATH: undefined }, async () => {
    ...
    await withEnvAsync({ GREENCHCLAW_CONFIG_PATH: configPath }, async () => {
