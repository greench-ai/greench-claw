...
import { clearActivatedPluginRuntimeState, loadGreenchClawPlugins } from "../plugins/loader.js";
...
const FALLBACK_GATEWAY_CONTEXT_STATE_KEY: unique symbol = Symbol.for(
  "GreenchClaw.fallbackGatewayContextState",
);
...
const PLUGIN_SUBAGENT_POLICY_STATE_KEY: unique symbol = Symbol.for(
  "GreenchClaw.pluginSubagentOverridePolicyState",
);
...
export function setPluginSubagentOverridePolicies(cfg: GreenchClawConfig): void {
  ...
  if (!policy?.allowModelOverride) {
    return {
      ...
      reason:
        ...
        "See https://docs.GreenchClaw.ai/tools/plugin#runtime-helpers and search for: " +
...
export function loadGatewayPlugins(params: {
  ...
  const pluginRegistry = loadGreenchClawPlugins({
    config: resolvedConfig,
