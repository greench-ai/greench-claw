...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-GreenchClaw-message-channel") ?? "",
  );
  const accountId = normalizeOptionalString(getHeader(req, "x-GreenchClaw-account-id"));
  const agentTo = normalizeOptionalString(getHeader(req, "x-GreenchClaw-message-to"));
  const agentThreadId = normalizeOptionalString(getHeader(req, "x-GreenchClaw-thread-id"));
