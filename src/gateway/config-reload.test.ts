...
function createReloaderHarness(
  ...
  const onHotReload = vi.fn(async (_plan: GatewayReloadPlan, _nextConfig: GreenchClawConfig) => {});
  const onRestart = vi.fn((_plan: GatewayReloadPlan, _nextConfig: GreenchClawConfig) => {});
  ...
  const reloader = startGatewayConfigReloader({
    ...
    onRestart,
    log,
    watchPath: "/tmp/GreenchClaw.json",
  });
  return {
    watcher,
    onHotReload,
...
function getOnlyRestartCall(harness: ReloaderHarness): [GatewayReloadPlan, GreenchClawConfig] {
...
function getOnlyHotReloadCall(harness: ReloaderHarness): [GatewayReloadPlan, GreenchClawConfig] {
...
describe("startGatewayConfigReloader", () => {
  ...
  it("skips plugin-local invalid reloads without degraded mode", async () => {
    const activeConfig: GreenchClawConfig = {
      gateway: { reload: { debounceMs: 0 } },
    ...
    const promoteSnapshot = vi.fn(async (_snapshot: ConfigFileSnapshot, _reason: string) => true);
    const previousConfig: GreenchClawConfig = {
      ...activeConfig,
      plugins: {
        entries: {
          "lossless-claw": {
  ...
  it("queues restart when an external plugin source write only changes the managed index", async () => {
    ...
    const readPluginInstallRecords = vi.fn().mockResolvedValueOnce({
      "lossless-claw": {
        source: "npm",
        spec: "@martian-engineering/lossless-claw",
        installPath: "/tmp/GreenchClaw/plugins/lossless-claw",
        installedAt: "2026-04-22T00:00:00.000Z",
      },
  ...
  it("keeps external plugin policy-only writes on the hot reload path", async () => {
    ...
    const installRecords = {
      telegram: {
        source: "npm",
        spec: "@GreenchClaw/telegram",
        installPath: "/tmp/GreenchClaw/plugins/telegram",
      },
    } satisfies Record<string, PluginInstallRecord>;
    const readSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>().mockResolvedValueOnce(
      makeSnapshot({
