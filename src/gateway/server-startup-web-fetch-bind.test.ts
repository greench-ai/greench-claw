...
async function writeConfig(config: GreenchClawConfig): Promise<void> {
...
describe("gateway startup web fetch config", () => {
  it("binds HTTP with credential-free tools.web.fetch config without fetch provider discovery", async () => {
    const previousMinimal = process.env.GREENCHCLAW_TEST_MINIMAL_GATEWAY;
    process.env.GREENCHCLAW_TEST_MINIMAL_GATEWAY = "0";
    ...
    try {
      ...
      if (previousMinimal === undefined) {
        ...
        process.env.GREENCHCLAW_TEST_MINIMAL_GATEWAY = previousMinimal;
      }
    }
  });
});
