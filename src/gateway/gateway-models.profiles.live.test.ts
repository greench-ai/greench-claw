...
import { ensureGreenchClawModelsJson } from "../agents/models-config.js";
...
import type { ModelsConfig, ModelProviderConfig, GreenchClawConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.GREENCHCLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.GREENCHCLAW_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.GREENCHCLAW_LIVE_GATEWAY_SMOKE);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.GREENCHCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_SETUP_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.GREENCHCLAW_LIVE_GATEWAY_SETUP_TIMEOUT_MS, 60_000),
);
const GATEWAY_LIVE_MODEL_TIMEOUT_MS = resolveGatewayLiveModelTimeoutMs();
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.GREENCHCLAW_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.GREENCHCLAW_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["GREENCHCLAW_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.GREENCHCLAW_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.GREENCHCLAW_LIVE_GATEWAY_MODELS?.trim();
  const useExplicitModels = Boolean(rawModels) && rawModels !== "modern" && rawModels !== "all";
  return resolveHighSignalLiveModelLimit({
    rawMaxModels: process.env.GREENCHCLAW_LIVE_MAX_MODELS,
    useExplicitModels,
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.GREENCHCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.GREENCHCLAW_LIVE_MODEL_TIMEOUT_MS,
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.GREENCHCLAW_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.GREENCHCLAW_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.GREENCHCLAW_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("GREENCHCLAW_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("GREENCHCLAW_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("GREENCHCLAW_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.GREENCHCLAW_LIVE_GATEWAY_MODELS;
    delete process.env.GREENCHCLAW_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.GREENCHCLAW_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.GREENCHCLAW_LIVE_GATEWAY_MODELS = "openai/gpt-5.5,anthropic/claude-opus-4-6";
    delete process.env.GREENCHCLAW_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.GREENCHCLAW_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.GREENCHCLAW_LIVE_GATEWAY_MAX_MODELS = "2";
...
async function sanitizeAuthConfig(params: {
  cfg: GreenchClawConfig;
  agentDir: string;
}): Promise<GreenchClawConfig["auth"] | undefined> {
  ...
  let profiles: NonNullable<GreenchClawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    ...
    disableBonjour: process.env.GREENCHCLAW_DISABLE_BONJOUR,
    logLevel: process.env.GREENCHCLAW_LOG_LEVEL,
  ...
  if (QUIET_LIVE_LOGS) {
    process.env.GREENCHCLAW_DISABLE_BONJOUR = "1";
    process.env.GREENCHCLAW_LOG_LEVEL = "silent";
  }
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-live-state-"));
  ...
  await fs.mkdir(path.join(workspaceDir, ".GreenchClaw"), { recursive: true });
  await fs.writeFile(
    path.join(workspaceDir, ".GreenchClaw", "workspace-state.json"),
    `${JSON.stringify(
      {
        version: 1,
  ...
  const toolProbePath = path.join(workspaceDir, `.GreenchClaw-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveDefaultAgentDir(params.cfg);
  const sanitizedCfg: GreenchClawConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "GreenchClaw-live-"));
  const tempConfigPath = path.join(tempDir, "GreenchClaw.json");
  ...
  try {
            ...
            (async () => {
              ...
              for (
                ...
                toolText = await requestGatewayAgentText({
                  ...
                  modelKey,
                  message: strictReply
                    ? "GreenchClaw live tool probe (local, safe): " +
    ...
    process.env.GREENCHCLAW_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.GREENCHCLAW_LOG_LEVEL = previous.logLevel;
    process.env.GREENCHCLAW_AGENT_DIR = previous.agentDir;
...
describeLive("gateway live (dev agent, profile keys)", () => {
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        await withGatewayLiveSetupTimeout(
          ensureGreenchClawModelsJson(cfg, undefined, {
            workspaceDir,
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via GREENCHCLAW_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    await ensureGreenchClawModelsJson(cfg);
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.GreenchClaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
