...
function createScopedCliClient(
  ...
  const id = client.id ?? "GreenchClaw-cli";
  return {
    connect: {
      scopes,
      client: {
...
describe("chat directive tag stripping for non-streaming final payloads", () => {
  ...
  it("registers tool-event recipients for clients advertising tool-events capability", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-tool-events-");
  ...
  it("does not register tool-event recipients without tool-events capability", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-tool-events-off-");
    mockState.finalText = "ok";
    mockState.triggerAgentRunStart = true;
  ...
  it("persists agent-run audio replies emitted as media-bearing block payloads", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-agent-audio-");
  ...
  it("persists auto-TTS final media as audio-only so webchat does not duplicate assistant text", async () => {
    const transcriptDir = createTranscriptFixture("GreenchClaw-chat-send-agent-tts-final-");
  ...
  it("does not mirror agent-run stale media final text from live delivery", async () => {
    const transcriptDir = createTranscriptFixture("GreenchClaw-chat-send-agent-stale-tts-");
  ...
  it("does not mirror normal agent-run final text from live delivery", async () => {
    const transcriptDir = createTranscriptFixture("GreenchClaw-chat-send-agent-text-only-");
    mockState.config = {
      agents: {
        defaults: {
          workspace: transcriptDir,
  ...
  it("keeps visible text on non-agent TTS final media because no model transcript exists", async () => {
    const transcriptDir = createTranscriptFixture("GreenchClaw-chat-send-command-tts-final-");
    const audioPath = path.join(transcriptDir, "tts.mp3");
    fs.writeFileSync(audioPath, Buffer.from([0xff, 0xfb, 0x90, 0x00]));
    mockState.config = {
      agents: {
  ...
  it("renders image reply payloads as assistant image content instead of MEDIA text", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-agent-image-");
    mockState.finalPayload = {
      text: "Scan this QR code with the GreenchClaw iOS app:",
  ...
  it("suppresses reasoning payloads from webchat transcript replies", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-reasoning-hidden-");
    mockState.dispatchedReplies = [
      {
        kind: "final",
  ...
  it("chat.inject keeps message defined when directive tag is the only content", async () => {
    createTranscriptFixture("GreenchClaw-chat-inject-directive-only-");
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("chat.send non-streaming final keeps message defined for directive-only assistant text", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-directive-only-");
    mockState.finalText = "[[reply_to_current]]";
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("rejects oversized chat.send session keys before dispatch", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-session-key-too-long-");
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("chat.inject strips external untrusted wrapper metadata from final payload text", async () => {
    createTranscriptFixture("GreenchClaw-chat-inject-untrusted-meta-");
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("chat.inject broadcasts and routes on the canonical session key", async () => {
    createTranscriptFixture("GreenchClaw-chat-inject-canonical-key-");
    mockState.sessionEntry = {
  ...
  it("chat.send non-streaming final strips external untrusted wrapper metadata from final payload text", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-untrusted-meta-");
  ...
  it("chat.send non-streaming final broadcasts and routes on the canonical session key", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-canonical-key-");
    mockState.sessionEntry = {
      canonicalKey: "agent:main:canon",
    };
    mockState.finalText = "hello";
  ...
  it("chat.send broadcasts final replies for telegram-shaped session keys", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-telegram-final-");
  ...
  it("chat.send keeps explicit delivery routes for channel-scoped sessions", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-origin-routing-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      deliveryContext: {
        channel: "telegram",
  ...
  it("chat.send marks user slash commands as text command sources", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-text-command-source-");
    mockState.finalText = "ok";
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("chat.send keeps explicit delivery routes for Feishu channel-scoped sessions", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-feishu-origin-routing-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      deliveryContext: {
        channel: "feishu",
  ...
  it("chat.send keeps explicit delivery routes for per-account channel-peer sessions", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-per-account-channel-peer-routing-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      deliveryContext: {
        channel: "telegram",
  ...
  it("chat.send keeps explicit delivery routes for legacy channel-peer sessions", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-legacy-channel-peer-routing-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      deliveryContext: {
        channel: "telegram",
  ...
  it("chat.send keeps explicit delivery routes for legacy thread sessions", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-legacy-thread-channel-peer-routing-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      deliveryContext: {
        channel: "telegram",
  ...
  it("chat.send does not inherit external delivery context for shared main sessions", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-main-no-cross-route-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      deliveryContext: {
        channel: "discord",
  ...
  it("chat.send does not inherit external delivery context for UI clients on main sessions", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-main-ui-routes-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      deliveryContext: {
        channel: "whatsapp",
  ...
  it("chat.send does not inherit external delivery context for UI clients on main sessions when deliver is enabled", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-main-ui-deliver-no-route-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      deliveryContext: {
        channel: "telegram",
  ...
  it("chat.send inherits external delivery context for CLI clients on configured main sessions", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-config-main-cli-routes-");
  ...
  it("chat.send falls back to origin provider metadata for configured main CLI delivery inheritance", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-config-main-origin-provider-routes-");
    mockState.mainSessionKey = "work";
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      origin: {
  ...
  it("chat.send falls back to origin thread metadata for configured main CLI delivery inheritance", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-config-main-origin-thread-routes-");
    mockState.mainSessionKey = "work";
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      origin: {
  ...
  it("chat.send keeps configured main delivery inheritance when connect metadata omits client details", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-config-main-connect-no-client-");
    mockState.mainSessionKey = "work";
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      deliveryContext: {
  ...
  it("chat.send does not inherit external delivery context for non-channel custom sessions", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-custom-no-cross-route-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      deliveryContext: {
        channel: "discord",
  ...
  it("chat.send keeps replies on the internal surface when deliver is not enabled", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-no-deliver-internal-surface-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      deliveryContext: {
        channel: "discord",
  ...
  it("chat.send does not inherit external routes for webchat clients on channel-scoped sessions", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-webchat-channel-scoped-no-inherit-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      deliveryContext: {
        channel: "imessage",
  ...
  it("chat.send still inherits external routes for UI clients on channel-scoped sessions", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-ui-channel-scoped-inherit-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      deliveryContext: {
        channel: "imessage",
  ...
  it("chat.send accepts admin-scoped synthetic originating routes without external delivery", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-synthetic-origin-admin-");
    mockState.finalText = "ok";
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("rejects synthetic originating routes when the caller lacks admin scope", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-synthetic-origin-reject-");
    mockState.finalText = "ok";
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("rejects reserved system provenance fields for non-ACP clients", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-system-provenance-reject-");
    mockState.finalText = "ok";
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("rejects forged ACP metadata when the caller lacks admin scope", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-system-provenance-spoof-reject-");
    mockState.finalText = "ok";
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("allows admin-scoped clients to inject system provenance without ACP metadata", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-system-provenance-admin-");
    mockState.finalText = "ok";
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("forwards gateway caller scopes into the dispatch context", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-gateway-client-scopes-");
    mockState.finalText = "ok";
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("normalizes missing gateway caller scopes to an empty array before dispatch", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-missing-gateway-client-scopes-");
    mockState.finalText = "ok";
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("injects ACP system provenance into the agent-visible body", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-system-provenance-acp-");
    mockState.finalText = "ok";
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("emits a user transcript update when chat.send starts an agent run", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-user-transcript-agent-run-");
    mockState.finalText = "ok";
    mockState.triggerAgentRunStart = true;
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("does not emit pre-gate user transcript content when before_agent_run hooks are registered", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-user-transcript-before-run-gate-");
    mockState.finalText = "ok";
    mockState.triggerAgentRunStart = true;
  ...
  it("does not emit raw user transcript content when before_agent_run blocks without a persisted marker", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-user-transcript-blocked-live-signal-");
  ...
  it("does not emit live user transcript content when before_agent_run hooks are present and the agent fails", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-user-transcript-gate-pass-error-");
    mockState.triggerAgentRunStart = true;
    mockState.hasBeforeAgentRunHooks = true;
  ...
  it("adds persisted media paths to the user transcript update", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-user-transcript-images-");
    mockState.finalText = "ok";
    mockState.triggerAgentRunStart = true;
    mockState.savedMediaResults = [
  ...
  it("persists non-image chat.send attachments as media refs without dispatch images", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-user-transcript-file-");
    mockState.finalText = "ok";
    mockState.triggerAgentRunStart = true;
    mockState.savedMediaResults = [
  ...
  it("preserves offloaded attachment media paths in transcript order", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-user-transcript-offloaded-");
    mockState.finalText = "ok";
    mockState.triggerAgentRunStart = true;
    mockState.sessionEntry = {
  ...
  it("skips transcript media notes for ACP bridge clients", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-user-transcript-acp-images-");
    mockState.finalText = "ok";
    mockState.triggerAgentRunStart = true;
    mockState.savedMediaResults = [
  ...
  it("waits for the user transcript update before final broadcast on non-agent attachment sends", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-no-agent-images-order-");
    mockState.finalText = "ok";
    mockState.savedMediaResults = [
      { path: "/tmp/chat-send-image-a.png", contentType: "image/png" },
    ];
  ...
  it("preserves media-only final replies in the final broadcast message", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-media-only-final-");
  ...
  it("strips NO_REPLY from transcript text when final replies only carry media", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-media-only-silent-final-");
    mockState.finalPayload = {
      text: "NO_REPLY",
      mediaUrl: "data:image/png;base64,cG5n",
    };
  ...
  it("preserves reply tags in transcript updates for media replies while stripping them from the broadcast", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-media-reply-tags-");
    mockState.finalPayload = {
      replyToCurrent: true,
      mediaUrl: "data:image/png;base64,cG5n",
    };
  ...
  it("does not persist sensitive image media into transcript updates", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-sensitive-media-final-");
    mockState.finalPayload = {
      text: "Scan this QR code with the GreenchClaw iOS app:",
      mediaUrl: "data:image/png;base64,cG5n",
      sensitiveMedia: true,
  ...
  it("sanitizes replyToId before emitting inline reply directives", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-sanitized-reply-id-");
    mockState.finalPayload = {
      text: "hello",
      replyToId: "abc]]\n[[audio_as_voice]]",
    };
  ...
  it("routes text-only image offloads into media-understanding fields", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-text-only-attachments-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      modelProvider: "test-provider",
      model: "text-only",
  ...
  it("keeps image attachments inline for configured custom vision models", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-configured-custom-vision-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
  ...
  it("keeps image attachments for text-only sessions bound to ACP", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-text-only-acp-bound-attachments-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      modelProvider: "test-provider",
      model: "text-only",
  ...
  it("resolves attachment image support from the session agent model", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-agent-scoped-text-only-attachments-");
    mockState.finalText = "ok";
    mockState.config = {
      agents: {
        list: [
  ...
  it("routes non-image offloaded refs into ctx.MediaPaths + MediaTypes for chat.send", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-non-image-ctx-media-paths-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      modelProvider: "test-provider",
      model: "vision-model",
    ...
    mockState.savedMediaResults = [
      { path: "/home/user/.GreenchClaw/media/inbound/report.pdf", contentType: "application/pdf" },
    ];
    const respond = vi.fn();
    const context = createChatContext();
    ...
    expect(mockState.lastDispatchCtx?.MediaPaths).toEqual([
      "/home/user/.GreenchClaw/media/inbound/report.pdf",
    ]);
  ...
  it("preserves sandbox-relative MediaPaths and stores workspace context for media-understanding", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-non-image-absolutize-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      modelProvider: "test-provider",
      model: "vision-model",
  ...
  it("wraps stageSandboxMedia infrastructure errors as 5xx UNAVAILABLE and cleans up media-store files", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-stage-unavailable-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      modelProvider: "test-provider",
      model: "vision-model",
  ...
  it("logs chat.send attachment parse failures with stack details", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-attachment-parse-stack-");
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("surfaces partial non-image staging failures as 5xx UNAVAILABLE", async () => {
    ...
    createTranscriptFixture("GreenchClaw-chat-send-partial-stage-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      modelProvider: "test-provider",
      model: "vision-model",
    ...
    mockState.savedMediaResults = [
      { path: "/home/user/.GreenchClaw/media/inbound/report.pdf", contentType: "application/pdf" },
      {
        path: "/home/user/.GreenchClaw/media/inbound/oversize.pdf",
        contentType: "application/pdf",
      },
    ];
    mockState.sandboxWorkspace = { workspaceDir: "/sandbox/workspace" };
    mockState.stagedRelativePaths = ["media/inbound/report.pdf", "media/inbound/oversize.pdf"];
    mockState.unstagedSources = ["/home/user/.GreenchClaw/media/inbound/oversize.pdf"];
    const respond = vi.fn();
    const context = createChatContext();
    const pdf = Buffer.from("%PDF-1.4\n").toString("base64");
  ...
  it("rejects sandbox-oversized non-image attachments as 4xx before staging", async () => {
    ...
    createTranscriptFixture("GreenchClaw-chat-send-sandbox-oversize-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      modelProvider: "test-provider",
      model: "vision-model",
    ...
    mockState.savedMediaResults = [
      { path: "/home/user/.GreenchClaw/media/inbound/huge.pdf", contentType: "application/pdf" },
    ];
    mockState.sandboxWorkspace = { workspaceDir: "/sandbox/workspace" };
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("passes imageOrder for mixed inline and offloaded chat.send attachments", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-image-order-");
    mockState.finalText = "ok";
    mockState.sessionEntry = {
      modelProvider: "test-provider",
      model: "vision-model",
  ...
  it("maps media offload failures to UNAVAILABLE in chat.send", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-media-offload-error-");
    mockState.sessionEntry = {
      modelProvider: "test-provider",
      model: "vision-model",
    };
  ...
  it("persists chat.send attachments one at a time", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-image-serial-save-");
    mockState.finalText = "ok";
    mockState.savedMediaResults = [
      { path: "/tmp/chat-send-image-a.png", contentType: "image/png" },
      { path: "/tmp/chat-send-image-b.jpg", contentType: "image/jpeg" },
  ...
  it("does not parse or offload attachments for stop commands", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-stop-command-attachments-");
  ...
  it("emits a user transcript update when chat.send completes without an agent run", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-user-transcript-no-run-");
    mockState.finalText = "ok";
    const respond = vi.fn();
    const context = createChatContext();
  ...
  it("emits a user transcript update when chat.send fails before an agent run starts", async () => {
    createTranscriptFixture("GreenchClaw-chat-send-user-transcript-error-no-run-");
