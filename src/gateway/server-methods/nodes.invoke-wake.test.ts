...
describe("node plugin surface refresh", () => {
  it("refreshes generic plugin surface capability urls", async () => {
    ...
    expect(parsedCanvasUrl.pathname.startsWith("/__GreenchClaw__/cap/")).toBe(true);
    const capabilityToken = parsedCanvasUrl.pathname.slice("/__GreenchClaw__/cap/".length);
