...
function resolveDiscoveryAdvertiseTimeoutMs(env: NodeJS.ProcessEnv): number {
  const raw = env.GREENCHCLAW_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS?.trim();
  if (!raw) {
...
export async function startGatewayDiscovery(params: {
  ...
  const localDiscoveryEnabled =
    mdnsMode !== "off" &&
    !isTruthyEnvValue(process.env.GREENCHCLAW_DISABLE_BONJOUR) &&
  ...
  const tailnetDns = needsTailnetDns
    ...
    : undefined;
  const sshPortEnv = mdnsMinimal ? undefined : process.env.GREENCHCLAW_SSH_PORT?.trim();
