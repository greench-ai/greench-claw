        ...
        plugins: {
          ...
          installs: {
            ...
            [entry.pluginId]: {
              ...
              installPath: `/tmp/GreenchClaw-plugins/${entry.pluginId}`,
            },
          },
        },
      },
...
describe("runSearchSetupFlow", () => {
  ...
  it("installs an external catalog search provider before enabling it", async () => {
    ...
    expect(installRequest.entry?.install?.npmSpec).toBe("@GreenchClaw/brave-plugin");
    ...
    expect(next.plugins?.installs?.brave?.spec).toBe("@GreenchClaw/brave-plugin");
  });
