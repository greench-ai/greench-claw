...
function collectGatewayServiceStartRepairIssues(
  ...
  const serviceVersion = command.environment?.GREENCHCLAW_SERVICE_VERSION?.trim();
  if (serviceVersion && serviceVersion !== VERSION) {
    issues.push({
      code: "version-mismatch",
      message: `service was installed by GreenchClaw ${serviceVersion}, current CLI is ${VERSION}`,
    });
  }
