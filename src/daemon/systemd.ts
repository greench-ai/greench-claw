...
function resolveSystemdServiceName(env: GatewayServiceEnv): string {
  const override = env.GREENCHCLAW_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override.slice(0, -".service".length) : override;
  }
  return resolveGatewaySystemdServiceName(env.GREENCHCLAW_PROFILE);
}
...
async function writeSystemdGatewayEnvironmentFile(params: {
  ...
  /** GreenchClaw-managed keys that must not be preserved from an old env file; stale file values
  ...
  // GreenchClaw-managed keys (identified by inlineManagedKeys) are excluded: a stale
