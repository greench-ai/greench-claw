...
export const GREENCHCLAW_WRAPPER_ENV_KEY = "GREENCHCLAW_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/GreenchClaw -> .pnpm/GreenchClaw@X.Y.Z/...
...
export async function resolveGreenchClawWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${GREENCHCLAW_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveGreenchClawWrapperPath(params.wrapperPath);
  if (wrapperPath) {
