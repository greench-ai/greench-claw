...
describe("bundled root-aware plugin lookups", () => {
  it("reads bundled channel ids from the active bundled root without re-importing", async () => {
    const rootA = makeBundledRoot("GreenchClaw-bundled-ids-a-");
    const rootB = makeBundledRoot("GreenchClaw-bundled-ids-b-");

    vi.doMock("../../plugins/channel-catalog-registry.js", () => ({
      listChannelCatalogEntries: (params?: { env?: NodeJS.ProcessEnv }) => {
        const activeRoot = params?.env?.GREENCHCLAW_BUNDLED_PLUGINS_DIR;
    ...
    process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR = rootA.pluginsDir;
    expect(bundledIds.listBundledChannelPluginIds()).toEqual(["alpha"]);
    expect(bundledIds.listBundledChannelIds()).toEqual(["alpha-chat"]);

    process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR = rootB.pluginsDir;
  ...
  it("reads bootstrap plugins from the active bundled root without re-importing", async () => {
    const rootA = makeBundledRoot("GreenchClaw-bootstrap-a-");
    const rootB = makeBundledRoot("GreenchClaw-bootstrap-b-");

    vi.doMock("./bundled-ids.js", () => ({
      listBundledChannelPluginIdsForRoot: () => {
        if (process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR === rootA.pluginsDir) {
          return ["alpha"];
        }
        if (process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR === rootB.pluginsDir) {
          return ["beta"];
        }
        return [];
      },
    ...
    vi.doMock("./bundled.js", () => ({
      ...
      getBundledChannelSetupPlugin: (id: string) => {
        const suffix = resolveMockRootSuffix({
          activeRoot: process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR,
  ...
  it("retries bootstrap plugin loading after an error", async () => {
    const root = makeBundledRoot("GreenchClaw-bootstrap-plugin-throw-");

    vi.doMock("./bundled-ids.js", () => ({
      listBundledChannelPluginIdsForRoot: () =>
        process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR === root.pluginsDir ? ["alpha"] : [],
    }));
    ...
    process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR = root.pluginsDir;
    expect(bootstrapRegistry.listBootstrapChannelPluginIds()).toEqual(["alpha"]);
  ...
  it("keeps plugin loading independent from bootstrap secrets loading errors", async () => {
    const root = makeBundledRoot("GreenchClaw-bootstrap-secrets-throw-");
