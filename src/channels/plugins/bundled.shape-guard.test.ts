...
function restoreBundledPluginsDir(previousBundledPluginsDir: string | undefined) {
  if (previousBundledPluginsDir === undefined) {
    ...
    process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
  }
}
  ...
  return fs
    ...
    .filter(
      (entryPath) =>
        ...
        fs.existsSync(path.join(entryPath, "GreenchClaw.plugin.json")),
    );
}

afterEach(() => {
  delete (globalThis as { __GreenchClawBundledChannelReenter?: () => void })
    .__GreenchClawBundledChannelReenter;
  vi.resetModules();
...
describe("bundled channel entry shape guards", () => {
  ...
  it("fills sparse bundled channel plugin metadata from package metadata", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-bundled-metadata-"));
    const previousBundledPluginsDir = process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR = path.join(tempRoot, "dist", "extensions");
  ...
  it("uses the active bundled plugin root override for channel entry loading", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-bundled-override-"));
    const previousBundledPluginsDir = process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(tempRoot, "dist", "extensions", "alpha");
    fs.mkdirSync(pluginDir, { recursive: true });
    fs.writeFileSync(
  ...
  it("treats direct bundled plugin-tree overrides as scan roots", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-bundled-direct-override-"));
    const previousBundledPluginsDir = process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR = pluginsRoot;
  ...
  it("partitions bundled channel lazy caches by active bundled root without re-importing", async () => {
    const rootA = fs.mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-bundled-root-a-"));
    const rootB = fs.mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-bundled-root-b-"));
    const previousBundledPluginsDir = process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR;
    const testGlobal = globalThis as typeof globalThis & {
    ...
    try {
      ...
      process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR = path.join(rootA, "dist", "extensions");
      ...
      process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR = path.join(rootB, "dist", "extensions");
  ...
  it("loads setup-entry feature plugins without loading the main channel entry", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-bundled-setup-only-"));
    const previousBundledPluginsDir = process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR = path.join(root, "dist", "extensions");
  ...
  it("swallows and caches bundled plugin and setup load failures", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-bundled-load-failure-"));
    const previousBundledPluginsDir = process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(root, "dist", "extensions", "alpha");
    const testGlobal = globalThis as typeof globalThis & {
  ...
  it("caches undefined bundled plugin loads as unavailable", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-bundled-null-load-"));
    const previousBundledPluginsDir = process.env.GREENCHCLAW_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(root, "dist", "extensions", "alpha");
    const testGlobal = globalThis as typeof globalThis & {
  ...
  it("keeps channel entrypoints on the dedicated entry-contract SDK surface", () => {
    const offenders = collectBundledChannelEntrypointOffenders(
      bundledPluginRoots,
      (source) =>
        !source.includes('from "GreenchClaw/plugin-sdk/channel-entry-contract"') ||
        source.includes('from "GreenchClaw/plugin-sdk/core"') ||
        source.includes('from "GreenchClaw/plugin-sdk/channel-core"'),
    );
  ...
  it("keeps setup-entry legacy feature hints mirrored in package metadata", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      ...
      for (const feature of ["legacyStateMigrations", "legacySessionSurfaces"]) {
        ...
        const hasHint = packageJson.GreenchClaw?.setupFeatures?.[feature] === true;
  ...
  it("keeps channel implementations off the broad core SDK surface", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      for (const relativePath of ["src/channel.ts", "src/plugin.ts"]) {
        ...
        if (source.includes('from "GreenchClaw/plugin-sdk/core"')) {
          offenders.push(path.relative(process.cwd(), filePath));
        }
      }
    }
  ...
  it("keeps bundled hot runtime barrels off the broad core SDK surface", () => {
    const offenders = [
      ...
      fs.readFileSync(path.resolve(filePath), "utf8").includes("GreenchClaw/plugin-sdk/core"),
    );
  ...
  it("keeps bundled doctor surfaces off the broad runtime barrel", () => {
    const offenders = [
      ...
      fs
        ...
        .includes('from "GreenchClaw/plugin-sdk/runtime"'),
    );
  ...
  it("breaks reentrant bundled channel discovery cycles with an empty fallback", async () => {
    const pluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "GreenchClaw-bundled-reentrant-"));
    const modulePath = path.join(pluginDir, "index.cjs");
    fs.writeFileSync(
      modulePath,
      `
const reenter = globalThis.__GreenchClawBundledChannelReenter;
...
`,
    ...
    let reentered = false;
    (
      globalThis as { __GreenchClawBundledChannelReenter?: () => void }
    ).__GreenchClawBundledChannelReenter = () => {
      if (!reentered) {
        reentered = true;
        expect(bundled.listBundledChannelPlugins()).toStrictEqual([]);
