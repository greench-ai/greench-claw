...
describe("renderQrPngBase64", () => {
  const tmpRoot = path.join(os.tmpdir(), "GreenchClaw-qr-image-tests");
  ...
  it("delegates PNG rendering to qrcode", async () => {
    await expect(renderQrPngBase64("GreenchClaw", { scale: 8, marginModules: 2 })).resolves.toBe(
      MOCK_PNG_BASE64,
    );
    expect(toDataURL).toHaveBeenCalledWith("GreenchClaw", {
      margin: 2,
      scale: 8,
      type: "image/png",
    });
  ...
  it("uses the default PNG rendering options", async () => {
    await renderQrPngBase64("GreenchClaw");
    expect(toDataURL).toHaveBeenCalledWith("GreenchClaw", {
      margin: 4,
      scale: 6,
      type: "image/png",
  ...
  it("floors finite PNG rendering options before delegating", async () => {
    await renderQrPngBase64("GreenchClaw", { scale: 8.9, marginModules: 2.9 });
    expect(toDataURL).toHaveBeenCalledWith("GreenchClaw", {
      margin: 2,
      scale: 8,
      type: "image/png",
  ...
  it.each([
    ...
    await expect(renderQrPngBase64("GreenchClaw", { scale, marginModules })).rejects.toThrow(
      message,
    );
  ...
  it("rejects non-PNG qrcode data URLs", async () => {
    ...
    await expect(renderQrPngBase64("GreenchClaw")).rejects.toThrow(
  ...
  it("formats QR PNG data URLs", async () => {
    ...
    await expect(renderQrPngDataUrl("GreenchClaw")).resolves.toBe(
  ...
  it("writes QR PNGs to a scoped temp file", async () => {
    ...
    const result = await writeQrPngTempFile("GreenchClaw", {
      tmpRoot,
      dirPrefix: "pair-",
  ...
  it.each([
    ...
    await expect(
      writeQrPngTempFile("GreenchClaw", {
        tmpRoot,
