...
vi.mock("./GreenchClaw-root.js", () => ({
  resolveGreenchClawPackageRoot: vi.fn(async () => null),
  resolveGreenchClawPackageRootSync: vi.fn(() => null),
}));
...
let GreenchClawRoot: typeof import("./GreenchClaw-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    GreenchClawRoot = await import("./GreenchClaw-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/GreenchClaw");
    const wrapperArgv1 = abs("fixtures/bin/GreenchClaw");
  ...
  it("uses resolveGreenchClawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/GreenchClaw");
    (
      GreenchClawRoot.resolveGreenchClawPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/GreenchClaw"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "GreenchClaw" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "GreenchClaw.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-GreenchClaw");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", () => {
    const pkgRoot = abs("fixtures/GreenchClaw-bundle");
    (
      GreenchClawRoot.resolveGreenchClawPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/GreenchClaw.app/Contents/MacOS/GreenchClaw");
    const bundledUiDir = abs("fixtures/GreenchClaw.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/GreenchClaw-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
