...
import { resolveGreenchClawPackageRoot } from "./GreenchClaw-root.js";
...
function resolveAutoUpdatePolicy(cfg: GreenchClawConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: GreenchClawConfig): number {
...
async function runAutoUpdateCommand(params: {
  ...
  if (argv.length === 0) {
    argv.push("GreenchClaw", ...baseArgs);
  }
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.GREENCHCLAW_NO_AUTO_UPDATE);
  ...
  const root = await resolveGreenchClawPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("GreenchClaw update")}`,
      );
    ...
    if (auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by GREENCHCLAW_NO_AUTO_UPDATE", {
