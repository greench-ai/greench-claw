 ...
 * returning control to the caller (typically `triggerGreenchClawRestart` →
...
/**
 * Windows: find listening PIDs on the port, then verify each is an GreenchClaw
 ...
 * Returns only PIDs that belong to GreenchClaw gateway processes (not the current process).
 */
export function findGatewayPidsOnPortSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only GreenchClaw gateway processes.
