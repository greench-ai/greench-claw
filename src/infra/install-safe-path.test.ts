...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@GreenchClaw/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@GreenchClaw/matrix", id: "matrix", expected: true },
    { packageName: "@GreenchClaw/matrix", id: "@GreenchClaw/matrix", expected: true },
    { packageName: "@GreenchClaw/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@GreenchClaw/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      resolveSafeInstallDir({
        baseDir: "/tmp/plugins",
        id: "@GreenchClaw/matrix",
        invalidNameMessage: "invalid plugin name",
      }),
    ).toEqual({
      ok: true,
      path: path.join("/tmp/plugins", "@GreenchClaw__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "GreenchClaw-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "GreenchClaw-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "GreenchClaw-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "GreenchClaw-install-safe-" }, async (parentDir) => {
