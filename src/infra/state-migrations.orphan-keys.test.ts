...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".GreenchClaw");
...
function sharedMainOpsConfig(sharedStorePath: string): GreenchClawConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: GreenchClawConfig = OPS_WORK_CONFIG) {
