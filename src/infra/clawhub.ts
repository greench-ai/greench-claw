...
// Keep aligned with @GreenchClaw/plugin-package-contract ExternalPluginCompatibility.
...
export type ClawHubPackageReadinessPhase =
  ...
  | "metadata-ready"
  | "blocked"
  | "ready-for-GreenchClaw"
  | (string & {});
export type ClawHubPackageReadiness = {
  ready?: boolean | null;
  readyForGreenchClaw?: boolean | null;
...
function normalizeBaseUrl(baseUrl?: string): string {
  const envValue =
    normalizeOptionalString(process.env.GREENCHCLAW_CLAWHUB_URL) ||
...
function resolveClawHubConfigPaths(): string[] {
  const explicit =
    normalizeOptionalString(process.env.GREENCHCLAW_CLAWHUB_CONFIG_PATH) ||
...
export async function resolveClawHubAuthToken(): Promise<string | undefined> {
  const envToken =
    normalizeOptionalString(process.env.GREENCHCLAW_CLAWHUB_TOKEN) ||
...
const GREENCHCLAW_CALVER_STABLE_CORRECTION_PATTERN = /^[vV]?(\d{4}\.\d{1,2}\.\d{1,2})-\d+$/;

function normalizeCalVerCorrectionForPluginApi(pluginApiVersion: string): string {
  const match = GREENCHCLAW_CALVER_STABLE_CORRECTION_PATTERN.exec(pluginApiVersion.trim());
...
export async function downloadClawHubPackageArchive(params: {
  ...
  if (params.artifact === "clawpack") {
    ...
    const target = await createTempDownloadTarget({
      prefix: "GreenchClaw-clawhub-clawpack",
  ...
  const sha256Hex = formatSha256Hex(bytes);
  const target = await createTempDownloadTarget({
    prefix: "GreenchClaw-clawhub-package",
...
export async function downloadClawHubSkillArchive(params: {
  ...
  const sha256Hex = formatSha256Hex(bytes);
  const target = await createTempDownloadTarget({
    prefix: "GreenchClaw-clawhub-skill",
