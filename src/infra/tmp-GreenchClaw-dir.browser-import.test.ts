...
describe("tmp-GreenchClaw-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__GreenchClawTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { POSIX_GREENCHCLAW_TMP_DIR, resolvePreferredGreenchClawTmpDir } from "./src/infra/tmp-GreenchClaw-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: POSIX_GREENCHCLAW_TMP_DIR,
            resolverType: typeof resolvePreferredGreenchClawTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-GreenchClaw-dir-browser-entry.ts",
      },
      write: false,
    });
