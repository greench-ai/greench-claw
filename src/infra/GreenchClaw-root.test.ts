...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__GreenchClaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "GreenchClaw-root");
...
function setPackageRoot(root: string, name = "GreenchClaw") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./GreenchClaw-root.js").resolveGreenchClawPackageRootSync,
  asyncResolver: typeof import("./GreenchClaw-root.js").resolveGreenchClawPackageRoot,
  opts: Parameters<typeof import("./GreenchClaw-root.js").resolveGreenchClawPackageRootSync>[0],
...
vi.mock("./GreenchClaw-root.fs.runtime.js", () => ({
  openClawRootFsSync: mockFsModule(),
  openClawRootFs: mockFsPromisesModule(),
}));

describe("resolveGreenchClawPackageRoot", () => {
  let resolveGreenchClawPackageRoot: typeof import("./GreenchClaw-root.js").resolveGreenchClawPackageRoot;
  let resolveGreenchClawPackageRootSync: typeof import("./GreenchClaw-root.js").resolveGreenchClawPackageRootSync;
  let clearGreenchClawPackageRootCaches: typeof import("./GreenchClaw-root.js").__testing.clearGreenchClawPackageRootCaches;

  beforeAll(async () => {
    ({
      resolveGreenchClawPackageRoot,
      resolveGreenchClawPackageRootSync,
      __testing: { clearGreenchClawPackageRootCaches },
    } = await import("./GreenchClaw-root.js"));
  });

  beforeEach(() => {
    clearGreenchClawPackageRootCaches();
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "GreenchClaw");
        const pkgRoot = path.join(project, "node_modules", "GreenchClaw");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "GreenchClaw");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "GreenchClaw.mjs")));
    ...
    {
      name: "falls through from a non-GreenchClaw moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-GreenchClaw");
    ...
    {
      name: "returns null for non-GreenchClaw package roots",
      setup: () => {
        const pkgRoot = fx("not-GreenchClaw");
        setPackageRoot(pkgRoot, "not-GreenchClaw");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "GreenchClaw.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "GreenchClaw");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
