...
const EXTENSION_MANIFEST = bundledPluginFile("demo", "GreenchClaw.plugin.json");
...
const DIST_RUNTIME_EXTENSION_MANIFEST = "dist-runtime/extensions/demo/GreenchClaw.plugin.json";
...
const DIST_GREENCHCLAW_ALIAS_PACKAGE = "dist/extensions/node_modules/GreenchClaw/package.json";
const DIST_GREENCHCLAW_ALIAS_PLUGIN_SDK_INDEX =
  "dist/extensions/node_modules/GreenchClaw/plugin-sdk/index.js";
...
const DIST_EXTENSION_MANIFEST = bundledDistPluginFile("demo", "GreenchClaw.plugin.json");
...
const BASE_PROJECT_FILES = {
  [ROOT_TSCONFIG]: "{}\n",
  [ROOT_PACKAGE]: '{"name":"GreenchClaw-test"}\n',
...
async function writeRuntimePostBuildScaffold(tmp: string): Promise<void> {
  ...
  await writeProjectFiles(tmp, {
    ...
    [DIST_GREENCHCLAW_ALIAS_PACKAGE]:
      '{"name":"GreenchClaw","type":"module","exports":{"./plugin-sdk":"./plugin-sdk/index.js"}}\n',
  });
...
function statusCommandSpawn() {
  return [process.execPath, "GreenchClaw.mjs", "status"];
}

function gatewayCallStatusCommandSpawn() {
  return [process.execPath, "GreenchClaw.mjs", "gateway", "call", "status", "--json"];
}
...
describe("run-node script", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
        ...
        expect(nodeCalls).toEqual([
          ...
          [process.execPath, "GreenchClaw.mjs", "--version"],
        ]);
      });
    },
  );
    ...
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[], options?: unknown) => {
        ...
        return createPipedExitedProcess({
          stdout: args[0] === "GreenchClaw.mjs" ? "child stdout\n" : "",
          stderr: args[0] === "GreenchClaw.mjs" ? "child stderr\n" : "",
        });
      };
      const mutedStream = {
        write: () => true,
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          GREENCHCLAW_FORCE_BUILD: "1",
          GREENCHCLAW_RUNNER_LOG: "1",
          GREENCHCLAW_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr: mutedStream,
        stdout: mutedStream,
      ...
      await expect(fs.readFile(outputPath, "utf-8")).resolves.toContain("[GreenchClaw]");
      expect(spawnCalls.at(-1)?.args).toEqual(["GreenchClaw.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.GREENCHCLAW_RUN_NODE_OUTPUT_LOG).toBe(outputPath);
    ...
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[]) =>
        createPipedExitedProcess({
          stderr: args[0] === "GreenchClaw.mjs" ? childStderr : "",
        });
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          GREENCHCLAW_RUNNER_LOG: "0",
          GREENCHCLAW_RUN_NODE_FILTER_SYNC_IO_STDERR: "1",
          GREENCHCLAW_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr,
  ...
  it("adds Node CPU profiling flags to the launched GreenchClaw child when requested", async () => {
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          GREENCHCLAW_RUNNER_LOG: "0",
          GREENCHCLAW_RUN_NODE_CPU_PROF_DIR: ".artifacts/profiles",
        },
        spawn,
        spawnSync,
        execPath: process.execPath,
      ...
      expect(childArgs[2]).toMatch(
        /^--cpu-prof-name=GreenchClaw-status-4242-\d{4}-\d{2}-\d{2}T.*\.cpuprofile$/,
      );
      expect(childArgs.slice(3)).toEqual(["GreenchClaw.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.GREENCHCLAW_RUN_NODE_CPU_PROF_DIR).toBe(profileDir);
      expect(fsSync.existsSync(profileDir)).toBe(true);
    });
  });

  it("adds Node sync I/O tracing flag to the launched GreenchClaw child when requested", async () => {
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
  ...
  it("does not mutate Matrix QA args when no generic output log is requested", async () => {
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      ...
      expect(exitCode).toBe(0);
      const childArgs = spawnCalls.at(-1)?.args ?? [];
      expect(childArgs).toEqual(["GreenchClaw.mjs", "qa", "matrix"]);
      expect(spawnCalls.at(-1)?.env.GREENCHCLAW_RUN_NODE_OUTPUT_LOG).toBeUndefined();
    });
  });
  ...
  it("passes the synthesized private QA env into runtime postbuild staging", async () => {
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      ...
      expect(postBuildParams?.env?.GREENCHCLAW_BUILD_PRIVATE_QA).toBe("1");
      expect(postBuildParams?.env?.GREENCHCLAW_ENABLE_PRIVATE_QA_CLI).toBe("1");
    });
  });
  ...
  it("reruns runtime postbuild in watch mode when required outputs are missing with no runtime stamp", async () => {
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      ...
      await fs.rm(resolvePath(tmp, DIST_GREENCHCLAW_ALIAS_PACKAGE));
  ...
  it("reruns runtime postbuild for dirty extension package metadata in watch mode", async () => {
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          [EXTENSION_PACKAGE]: '{"GreenchClaw":{"extensions":["./index.ts"]}}\n',
    ...
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      ...
      const runs = Promise.all([
        runStatusCommand({
          ...
          spawnSync,
          env: {
            GREENCHCLAW_RUN_NODE_BUILD_LOCK_POLL_MS: "1",
          },
          runRuntimePostBuild,
        }),
        runStatusCommand({
  ...
  it("forwards wrapper SIGTERM to the active GreenchClaw child and returns 143", async () => {
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
      ...
      expect(spawnCall?.[1]).toEqual(["GreenchClaw.mjs", "status"]);
    ...
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [EXTENSION_INDEX]: "export default {};\n",
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"name":"demo","GreenchClaw":{"extensions":["./index.ts"]}}\n',
          [ROOT_TSDOWN]: "export default {};\n",
          [DIST_EXTENSION_INDEX]: "export default {};\n",
          [DIST_EXTENSION_PACKAGE]: '{"name":"demo","GreenchClaw":{"extensions":["./stale.js"]}}\n',
        },
    ...
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]:
            '{"GreenchClaw":{"extensions":["./src/index.ts","./src/extra.ts"]}}\n',
    ...
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [EXTENSION_SRC]: "export default {};\n",
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"GreenchClaw":{"extensions":["./src/index.ts"]}}\n',
        },
        buildPaths: [
          ROOT_SRC,
          EXTENSION_SRC,
    ...
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [DIST_EXTENSION_PACKAGE]: '{"GreenchClaw":{"extensions":["./src/index.js"]}}\n',
          [DIST_EXTENSION_RUNTIME_SRC]: "export default {};\n",
          [DIST_RUNTIME_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_RUNTIME_EXTENSION_PACKAGE]: '{"GreenchClaw":{"extensions":["./src/index.js"]}}\n',
          [RUNTIME_POSTBUILD_STAMP]: '{"head":"abc123"}\n',
        },
        buildPaths: [
          ROOT_SRC,
    ...
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [DIST_EXTENSION_INDEX]: "export default {};\n",
          [DIST_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_EXTENSION_PACKAGE]: '{"GreenchClaw":{"extensions":["./index.js"]}}\n',
          [DIST_RUNTIME_EXTENSION_INDEX]: "export default {};\n",
          [DIST_RUNTIME_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_RUNTIME_EXTENSION_PACKAGE]: '{"GreenchClaw":{"extensions":["./index.js"]}}\n',
          [RUNTIME_POSTBUILD_STAMP]: '{"head":"abc123"}\n',
        },
        buildPaths: [
          ROOT_SRC,
  ...
  it("does not require GreenchClaw SDK alias outputs when dist extensions are absent", async () => {
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
  ...
  it("reports missing GreenchClaw SDK alias outputs when runtime stamps match HEAD", async () => {
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          ...
          [DIST_GREENCHCLAW_ALIAS_PACKAGE]:
            '{"name":"GreenchClaw","type":"module","exports":{"./plugin-sdk":"./plugin-sdk/index.js"}}\n',
          [DIST_GREENCHCLAW_ALIAS_PLUGIN_SDK_INDEX]:
      ...
      await fs.rm(resolvePath(tmp, DIST_GREENCHCLAW_ALIAS_PLUGIN_SDK_INDEX));
    ...
    await withTempDir({ prefix: "GreenchClaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          [DIFFS_PACKAGE]:
            '{"GreenchClaw":{"build":{"staticAssets":[{"source":"./assets/viewer-runtime.js","output":"assets/viewer-runtime.js"}]}}}\n',
