...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.GREENCHCLAW_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for GreenchClaw. You can keep running the web gateway without it: \`${formatCliCommand("GreenchClaw gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
