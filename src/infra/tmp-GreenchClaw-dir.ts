...
export const POSIX_GREENCHCLAW_TMP_DIR = "/tmp/GreenchClaw";
...
export type ResolvePreferredGreenchClawTmpDirOptions = {
...
export function resolvePreferredGreenchClawTmpDir(
  options: ResolvePreferredGreenchClawTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "GreenchClaw" : `GreenchClaw-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[GreenchClaw] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback GreenchClaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback GreenchClaw temp dir: ${fallbackPath}`);
    }
  ...
  const preferredDir = POSIX_GREENCHCLAW_TMP_DIR;
