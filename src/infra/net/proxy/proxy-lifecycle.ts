/**
 * High-level lifecycle management for GreenchClaw's operator-managed network
 * proxy routing.
 *
 * GreenchClaw does not spawn or configure the filtering proxy. When enabled, it
...
const PROXY_ACTIVE_KEYS = ["GREENCHCLAW_PROXY_ACTIVE", "GREENCHCLAW_PROXY_LOOPBACK_MODE"] as const;
...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    GREENCHCLAW_PROXY_ACTIVE: process.env["GREENCHCLAW_PROXY_ACTIVE"],
    GREENCHCLAW_PROXY_LOOPBACK_MODE: process.env["GREENCHCLAW_PROXY_LOOPBACK_MODE"],
  };
}
...
function applyProxyEnv(proxyUrl: string, loopbackMode: ProxyLoopbackMode): void {
  ...
  process.env["GREENCHCLAW_PROXY_LOOPBACK_MODE"] = loopbackMode;
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["GREENCHCLAW_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or GREENCHCLAW_PROXY_URL to an http:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["GREENCHCLAW_PROXY_ACTIVE"] !== "1") {
    return;
  }
