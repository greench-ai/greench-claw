...
import { BUNDLED_PLUGIN_ROOT_DIR } from "GreenchClaw/plugin-sdk/test-fixtures";
...
describe("Dockerfile", () => {
  ...
  it("uses full bookworm for build stages and slim bookworm for runtime", async () => {
    ...
    expect(dockerfile).toContain(
      'ARG GREENCHCLAW_NODE_BOOKWORM_IMAGE="node:24-bookworm@sha256:3a09aa6354567619221ef6c45a5051b671f953f0a1924d1f819ffb236e520e6b"',
    );
    expect(dockerfile).toContain(
      'ARG GREENCHCLAW_NODE_BOOKWORM_SLIM_IMAGE="node:24-bookworm-slim@sha256:e8e2e91b1378f83c5b2dd15f0247f34110e2fe895f6ca7719dbb780f929368eb"',
    );
    expect(dockerfile).toContain("FROM ${GREENCHCLAW_NODE_BOOKWORM_IMAGE} AS workspace-deps");
    expect(dockerfile).toContain("FROM ${GREENCHCLAW_NODE_BOOKWORM_IMAGE} AS build");
    expect(dockerfile).toContain("FROM ${GREENCHCLAW_NODE_BOOKWORM_SLIM_IMAGE} AS base-runtime");
    ...
    expect(dockerfile).not.toContain("GREENCHCLAW_VARIANT");
  });
  ...
  it("installs CA certificates in the slim runtime stage", async () => {
    ...
    const runtimeIndex = collapsed.indexOf(
      "FROM ${GREENCHCLAW_NODE_BOOKWORM_SLIM_IMAGE} AS base-runtime",
    );
  ...
  it("installs optional browser dependencies after pnpm install", async () => {
    ...
    const browserArgIndex = dockerfile.indexOf("ARG GREENCHCLAW_INSTALL_BROWSER");
  ...
  it("copies install workspace manifests before pnpm install", async () => {
    ...
    const extensionManifestIndex = dockerfile.indexOf(
      "COPY --from=workspace-deps /out/${GREENCHCLAW_BUNDLED_PLUGIN_DIR}/ ./${GREENCHCLAW_BUNDLED_PLUGIN_DIR}/",
    );
    ...
    expect(dockerfile).toContain(
      `if [ -f "/tmp/\${GREENCHCLAW_BUNDLED_PLUGIN_DIR}/$ext/package.json" ]; then`,
    );
  ...
  it("prunes runtime dependencies after the build stage", async () => {
    ...
    expect(dockerfile).toContain("ARG GREENCHCLAW_EXTENSIONS");
    expect(dockerfile).toContain("ARG GREENCHCLAW_BUNDLED_PLUGIN_DIR");
    expect(dockerfile).toContain(
      "Opt-in plugin dependencies at build time (space- or comma-separated directory names).",
    );
    expect(dockerfile).toContain(
      'Example: docker build --build-arg GREENCHCLAW_EXTENSIONS="diagnostics-otel,matrix" .',
    );
    expect(dockerfile).toContain(
      "RUN --mount=type=cache,id=GreenchClaw-pnpm-store,target=/root/.local/share/pnpm/store,sharing=locked \\",
    );
    ...
    expect(dockerfile).toContain(
      'GREENCHCLAW_EXTENSIONS="$GREENCHCLAW_EXTENSIONS" node scripts/prune-docker-plugin-dist.mjs',
    );
  ...
  it("does not override bundled plugin discovery in runtime images", async () => {
    const dockerfile = collapseDockerContinuations(await readFile(dockerfilePath, "utf8"));
    expect(dockerfile).toContain(`ARG GREENCHCLAW_BUNDLED_PLUGIN_DIR=${BUNDLED_PLUGIN_ROOT_DIR}`);
    expect(dockerfile).not.toMatch(/^\s*ENV\b[^\n]*\bGREENCHCLAW_BUNDLED_PLUGINS_DIR\b/m);
  });

  it("normalizes plugin and agent paths permissions in image layers", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
    expect(dockerfile).toContain(
      "RUN for dir in /app/${GREENCHCLAW_BUNDLED_PLUGIN_DIR} /app/.agent /app/.agents; do \\",
    );
    expect(dockerfile).toContain('find "$dir" -type d -exec chmod 755 {} +');
    expect(dockerfile).toContain('find "$dir" -type f -exec chmod 644 {} +');
  });

  it("Docker GPG fingerprint awk uses correct quoting for GREENCHCLAW_SANDBOX=1 build", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
  ...
  it("pre-creates the GreenchClaw home before switching to the node user", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
    const runtimeStageIndex = dockerfile.lastIndexOf("FROM base-runtime");
    const stateDirIndex = dockerfile.indexOf(
      "RUN install -d -m 0700 -o node -g node /home/node/.GreenchClaw && \\",
      runtimeStageIndex,
    );
    ...
    expect(dockerfile).not.toContain("mkdir -p /home/node/.GreenchClaw");
    expect(dockerfile).toContain(
      "stat -c '%U:%G %a' /home/node/.GreenchClaw | grep -qx 'node:node 700'",
    );
  });
});
