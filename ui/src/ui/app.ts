...
declare global {
  interface Window {
    __GREENCHCLAW_CONTROL_UI_BASE_PATH__?: string;
  }
}
...
export class GreenchClawApp extends LitElement {
  ...
  async toggleRealtimeTalk() {
    ...
    const session = new RealtimeTalkSession(
      ...
      this.sessionKey,
      {
        ...
        onTranscript: (entry) => {
          this.realtimeTalkTranscript = `${entry.role === "user" ? "You" : "GreenchClaw"}: ${entry.text}`;
        },
      },
